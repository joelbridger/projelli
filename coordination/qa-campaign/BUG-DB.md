...
| QA-45 | P1 | **Shared client notes stuck on 'Loading' forever** (MatterNotesEditorWrapper.tsx:52): ensureMatterSync rejects (key fetch/sync/crypto fail), .then has no .catch → loading stays true, permanent spinner instead of the locked/no-access panel. | Codex swallow-sweep (static) | FIX LANE: lp/swallow-batch |
| QA-46 | P1 | **Live co-edit sync never re-arms after socket drop** (MatterSyncClient.ts:288/338): on ws close / failed push it sets offline but no reconnect loop + unsent Yjs updates not queued → teammates silently stop receiving changes until an outside restart. | Codex swallow-sweep (static) | FIX LANE: lp/swallow-batch |
| QA-47 | P1 | **Meeting notes exist but screen says 'notes pending' after DocxEditor chunk-load fail** (MeetingEntry.tsx:115/378): dynamic import fails → DocxEditorComp null → falls through to false 'pending'. Same class as the merged flake. Fix: track load error + retry. | Codex swallow-sweep (static) | FIX LANE: lp/swallow-batch |
| QA-48 | P1 | **Calendar failure treated as 'no meetings today' → auto-prep silently dies** (TodaysMeetingsStrip.tsx:98/147, useMeetingAutoprep.ts:97): calendarListEvents fails → events=[] → no Today strip, no prep briefs, no error. Fix: calendar error state + retry, don't equate fail with empty. | Codex swallow-sweep (static) | FIX LANE: lp/swallow-batch |
...
| QA-68 | DOWNGRADED (was P0? investigate) — no cross-workspace content leak found | **Live-verified on Azure bench-2 with real cloud AI (OpenAI key, fast inference): RAG/Ask does NOT leak content across workspaces.** First corrected the record: the "global `~/.keepance`" assumption in CLAUDE.md/QA-62 is OUTDATED for the current tip — `store::dataset_path()` (`src-tauri/src/commands/rag/store/mod.rs:331`) computes the LanceDB dataset as `workspace_data_dir(workspace_root).join("vectors")`, i.e. a hidden `.lantern/vectors` folder INSIDE each workspace root, confirmed live on disk (Workspace A: `.../AdvisorPrepHeroSample/.lantern/vectors/` with 86+ LanceDB versions; a freshly-created empty Workspace B got its OWN brand-new `.../QA-Workspace-B-ragleak/.lantern/vectors/` with only 2 versions and zero indexed content). The Rust-side active-workspace pointer (`RagState.workspace_root: Mutex<Option<PathBuf>>`, set by `rag_set_workspace` and read by `require_workspace()` before any query) is a single per-process value, not per-matter. **Repro exactly matching QA-62's scenario:** created client "RagLeak Probe Client" in Workspace A with a document containing a distinctive marker (`RAGLEAK-SECRET-MARKER: ...Roth Conversion Plan... $2.4 million... 2040...`), confirmed it was genuinely retrievable in Workspace A via direct `invoke('rag_retrieve', {scope:{kind:'allMatters'}})` (score 0.93). Then reproduced the QA-62 stale-Client-Map bug via a "reopen last workspace" reload pointed at a fresh empty Workspace B (title bar correctly switched to "QA-Workspace-B-ragleak", but the Client Map/sidebar still showed all 5 stale Workspace-A clients including RagLeak Probe Client — same defect, corroborated via a second trigger path beyond the original Recent-Projects click). **With the app in exactly this confused state**, clicked into the stale "RagLeak Probe Client" and asked the Ask panel (Files-only mode, scoped "This client") the exact probe question ("What is the target balance and year in the Roth Conversion Plan document... RAGLEAK-SECRET-MARKER") — response: **"I couldn't find anything about that in your documents."** Confirmed at the Rust level too: a direct `rag_retrieve` AllMatters call in this same state returned `[]` (empty), proving the backend had genuinely re-pointed at Workspace B's own (empty) dataset, not silently still serving A's. **Verdict: RAG is workspace-scoped by construction; QA-62's UI-level stale-client bug does NOT cascade into a content-level leak.** QA-62 itself (wrong client roster shown) remains real and now independently corroborated via a 2nd repro path — fix that per QA-62's existing recommendation. **Bonus finding while investigating (separate, smaller bug, not filed with its own ID yet):** newly-created documents got indexed under the WRONG `matter_id` — RagLeak Probe Client's own doc (`folderPaths` correctly under its own folder) was tagged in the RAG manifest/chunks with `matter_id: "matter_sample_garcia_v_meridian"` (an orphaned pre-existing sample matter not even visible in the current Client Map) instead of its real id `matter_a72690b1-...`. This is why "This client"-scoped Ask found nothing for the new doc even in Workspace A itself (matter-id prefilter mismatch), separate from the cross-workspace question — worth a dedicated follow-up ticket (indexer matter-resolution bug, could cause a DIFFERENT client within the SAME workspace to see content that isn't theirs if "This client" scoping is trusted). Evidence: `docs/evidence/ragleak-20260705/` (42 screenshots, numbered walkthrough). Tip tested: `fef898c2` (bench-2, cloud AI/OpenAI mode). | ragleak (bench-2), live-verified, cloud-AI mode | DOWNGRADED — no P0 leak; QA-62 still open; new matter-mistagging bug needs its own ticket |
