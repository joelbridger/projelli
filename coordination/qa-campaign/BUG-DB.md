...
| QA-91 | P1 (confirms/strengthens QA-81 — now proven with a REAL 2-person meeting) | **Confirmed live: the in-meeting Notice Card never actually appears to a second real participant, and the failure mode is non-deterministic (sometimes hangs silently forever, sometimes fails explicitly after ~9 min).** Set up a genuinely live Microsoft Teams meeting ("Meet now" via teams.live.com, real join URL), started it from the Legion app's "Record this meeting?" flow with consent given, then joined the SAME meeting as a second, distinct participant — "Sarah Morgan (guest observer)" — via an anonymous guest join in a separate browser session, admitted by the host. Confirmed via the in-meeting Participants panel ("In this meeting (2)": Sarah Morgan + Jameson Daines/Organizer) and the meeting chat log (only the standard "was invited to the meeting" system line) that **no notice-card bot/participant ever joined, and no recording-notice message ever appeared anywhere in the meeting UI visible to the second participant** — so a real attendee would have zero indication the call is being recorded. On the recorder side, this run's failure surfaced differently than QA-81's silent-forever hang: after ~9 minutes the widget updated to an explicit **"Notice card couldn't join. Say the notice aloud."** — a real, user-visible failure message (better UX than QA-81's silent hang) but the underlying join still failed. The recording/transcript pipeline itself worked fine throughout (11 min audio captured, notes generated normally) — this is isolated to the notice-card-joins-the-call mechanism. Two different failure presentations (silent hang in QA-81 vs. explicit failure here) for the same underlying join failure suggest the WebView2-based join is flaky/non-deterministic, not a single fixed bug — worth root-causing the actual join mechanism rather than just the error-surfacing path. | winsmoke qa90-91 job, live-verified with a REAL 2-person Teams meeting (Legion host + anonymous guest via server Chrome) | NEW |
