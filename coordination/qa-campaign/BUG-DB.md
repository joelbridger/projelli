...
| QA-80 | P1 (trust-breaker — core "ask about your data" promise) | **Ask never finds/cites content from a client's ORIGINAL, bulk-seeded documents — only from files created/added interactively during a live app session.** Live core-flow test on `origin/lantern-plus` tip: asked multiple real, specific questions (CRD number, advisory-agreement effective date, "what documents do you have") scoped to "This client" for TWO different pre-existing demo clients (Caldwell Jennifer, Diaz Michelle) whose real PDF/docx files (Investment Advisory Agreement, Form ADV, Schwab statements, etc.) are visibly present and render correctly when opened directly. Every one of these questions returned "Nothing found in your files — General guidance" instead of a cited answer, reproduced across: both file-access-consent states (passive RAG and active "let the AI search on its own"), both .docx and .pdf source types, and BOTH cloud providers (Anthropic and OpenAI/ChatGPT — confirmed by removing the Anthropic key so only OpenAI remained and re-testing). By contrast, Ask reliably found and correctly cited: a `.docx` I created fresh in the live app this session, a CRM record freshly imported via the Wealthbox connector this session (`crm:contact:66158026`, "Verified against source"), and a meeting transcript recorded this session. `rag reconcile` logs show `reused=91` chunks with `reindexed=0` on every boot — consistent with only the live-session-created content ever having been embedded, while the bulk pre-seeded practice files (the content that actually represents "your existing client files" for a real advisor) were seemingly never indexed in the first place. This directly contradicts the core "connect your files -> ask cited questions" promise for anything except brand-new content. | winsmoke core-flow demo-readiness run, live-verified (2 clients × 3+ phrasings × both consent modes × both cloud providers, all consistently failing; contrasted against 3 different content types that DO cite correctly) | NEW |
| QA-81 | P1/P2 (silent hang, matches QA-79 swallow-class in the same file) | **The in-meeting "Notice Card" (the feature that joins a pasted Teams/Zoom link to show a recording notice inside the call) gets stuck on "Notice card joining" forever, with zero user-facing error, while the backend logs a real crash: `failed to create webview: WebView2 error: WindowsError(HRESULT(0x8007139F), "The group or resource is not in the correct state to perform the requested operation.")`.** Pasted a well-formed (but non-live) Teams meeting URL into "Record this meeting?", checked consent, started recording — the floating recorder widget showed "Notice card joining" continuously for the full ~70s+ recording with no progress, no retry, and no failure state ever surfacing in the UI; the recording itself completed normally (audio saved, transcript/summary generated fine) but the notice-card promise was never confirmed or denied to the user. `noticeCard/supervisor.ts` and `noticeCard/tauriDriver.ts` are two of the files already carrying grandfathered async-swallow debt from QA-79 (unlogged `void` promises / empty catches) — this stuck-silent behavior is consistent with that same debt class actually manifesting, not just a theoretical lint-rule violation. Could not test against a REAL live Teams call (no second party / real Teams infra on this bench), so can't rule out "the URL wasn't a real meeting" as a contributing factor — but a real product should surface a clear "couldn't join — check the link" error rather than hang silently, and the webview-creation error is real regardless of the URL's validity. | winsmoke core-flow demo-readiness run, live-verified (backend log + UI both captured) | NEW |
| QA-91 | P1 (confirms/strengthens QA-81 — now proven with a REAL 2-person meeting, and root-caused one step further) | **Confirmed live: the in-meeting Notice Card never actually appears to a second real participant. Root-caused further than QA-81: the notice-card automation never even reaches the Teams lobby — it fails before ever requesting to join, consistent with QA-81's `WebView2 error` at webview-creation time, not a missed lobby-admit.** Set up a genuinely live Microsoft Teams meeting ("Meet now" via teams.live.com, real join URL) twice, started recording from the Legion app's "Record this meeting?" flow both times, and joined the SAME meeting as a second, distinct participant — "Sarah Morgan (guest observer)" — via an anonymous guest join in a separate browser session, admitted by the host. **Run 1:** confirmed via the in-meeting Participants panel ("In this meeting (2)": Sarah Morgan + Jameson Daines/Organizer) and the meeting chat log (only the standard "was invited to the meeting" system line) that no notice-card bot/participant ever joined and no recording-notice message ever appeared — a real attendee would have zero indication the call is being recorded. **Run 2 (redo, per coordinator intel that the Notice Card is a companion GUEST participant that must be separately admitted from the host's lobby):** this time watched the HOST's own Teams lobby/People panel directly for a separate join request from the notice-card bot — polled 9 times over ~135 seconds while the recorder showed "Notice card joining" — **zero lobby/admit requests ever appeared from any bot**, confirming the automation never gets far enough to even ask Teams to join, not that an admit request was sitting unattended. The recorder failed faster this run (4:35 vs ~9 min previously) with the same explicit message: **"Notice card couldn't join. Say the notice aloud."** The recording/transcript pipeline itself worked fine both runs — this bug is isolated to the notice-card-joins-the-call mechanism, specifically at (or before) WebView2 creation, matching QA-81's backend crash log exactly. Two different failure *timings* (silent hang in QA-81's original test vs. explicit failure at 4:35-9min here) for the same underlying cause suggest non-deterministic timing before the same failure, not two different bugs. | winsmoke qa90-91 job, live-verified with a REAL 2-person Teams meeting across 2 separate runs (Legion host + anonymous guest via server Chrome), including a direct host-side lobby watch that ruled out "unadmitted guest" as the cause | NEW |

**QA-90 (ops task, not a bug — pre-download verification): PASS.** Verified the bundled Local AI model ("Advisor Prep Hero Local AI", Qwen3-4B-Instruct-2507 Q4_K_M GGUF, ~2.4GB) is fully downloaded and ready on the Legion bench: confirmed on-disk (`C:\Users\james\AppData\Roaming\lantern\models\qwen3-4b-instruct-2507\`, file size 2,497,280,736 bytes, SHA256 matching `manifest.json` exactly, `"status": "ready"`), confirmed the in-app Settings > AI & Privacy card shows "✓ Installed and ready", and confirmed Local AI answers a real test prompt end-to-end through the Ask feature while "On this computer only" mode was active (network lockdown auto-enabled, zero cloud calls) — got a correct, fast response. Since the model was already fully present from a prior session, there was no live download to trigger this time; nothing to report as broken — the pre-download state itself is verified solid for demo use.
