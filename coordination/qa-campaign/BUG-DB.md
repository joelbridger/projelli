...
| QA-80 | P1 (QA-91 layer 2 — new bug uncovered by the QA-91 fix) | **Notice Card still never joins a real live Teams meeting, now for a different, identified reason: `page-unrecognized` (Teams-adapter selector drift vs. the current Teams join UI).** Re-verified the QA-91 fix (tip `4cafb72f`, shared WebView2 browser-args) on the Legion against a real, live `teams.live.com` meeting, 3 separate attempts. The original `0x8007139F` WebView2 window-creation crash this fix targeted **does not reproduce** — `msedgewebview2.exe` process count briefly rises above baseline during each attempt (12→14→13→12), confirming the companion window IS created and torn down, not failing to open. But the card still never reaches the host's lobby: the Participants panel stayed at "In this meeting (1)" the entire time across all 3 attempts, no admit/knock request ever appeared, and each attempt's recording widget showed "Notice card couldn't join. Say the notice aloud." The client's own `.consent-ledger.json` records the machine-readable reason for all 3: `"reason": "page-unrecognized"` ("the join page didn't match the adapter (web-client drift)" per `noticeCardTypes.ts`), each failing at a consistent **~28-29 seconds** after notice-context is logged — a fast, deterministic page-recognition timeout, not the multi-minute join-timeout from the original report. Likely cause: the current `teams.live.com` (personal/consumer) pre-join flow is more convoluted than the adapter expects — hit real friction driving it manually too (a "No camera is connected" panel, a recurring "Allow Teams to access your mic" nag, an explicit audio-mode radio choice, and a "Join now" button that silently no-ops on a first click) — any of which could make the adapter's page-matching selectors miss. Fix should start from the `page-unrecognized` reason path in the Teams adapter (likely `noticeCard/tauriDriver.ts` or a Teams-specific adapter file) and the current `teams.live.com` join-page DOM, not from a fresh repro. Full evidence: `coordination/qa-campaign/evidence/legion-qa91-verify/REPORT.md`. | cc-lantern-legionverify (Legion, live Teams meeting, 3x reproduced) | NEW |
| QA-81 | P1 (QA-80/91 layer 3 — root cause of `page-unrecognized` now directly proven) | **Found and directly proven why the Notice Card hits `page-unrecognized` (QA-80): its hidden companion webview gets stuck on Teams' "Continue on this browser / Join on the Teams app" chooser page, a step BEFORE the actual meeting prejoin screen the `lp/qa91b-teams-adapter` fix (tip `f7847f63`) correctly re-targeted.** Retested tip `f7847f63` (contains `lp/qa91b-teams-adapter` + `lp/sidebar-archived`) on the Legion against a fresh real live Teams meeting, 3 more attempts — **all 3 still failed**, same `page-unrecognized` reason in `.consent-ledger.json`, same ~29s timing, no lobby knock ever seen by the host (6/6 total failures across both retest rounds). This time, instead of only inferring the failure from symptoms, I found the Notice Card's own hidden webview is reachable as a separate CDP target on the same debug port the app already exposes, and read its live content mid-attempt with a small standalone read-only script (not part of the app, deleted after use): its URL was `teams.live.com/dl/launcher/launcher.html?...`, title "Join conversation", page text **"Join your Teams meeting / Continue on this browser / Join on the Teams app"** — a browser-vs-native-app interstitial, not the prejoin screen at all. The card's automation never clicks "Continue on this browser," so it never reaches `[data-tid="calling-prejoin-screen"]`, and `detectPhase` correctly (if unhelpfully) returns `loading` forever for this page, exactly reproducing the timeout. Plausible reason the qa91b DOM capture never saw this page: it was captured from the server's already-warmed, already-signed-in shared Chrome profile (which has likely already "decided" to continue in-browser from many past sessions and so skips the chooser), while the Notice Card's webview is a fresh, isolated guest profile every single time and sees the chooser fresh on every attempt. **Fix should teach the adapter/runner to recognize this launcher page (URL contains `/dl/launcher/launcher.html`, or text contains "Continue on this browser") and click that button before ever asking `detectPhase` to recognize the prejoin** — this is an earlier, additional page, not a flaw in the qa91b selector work itself. Secondary, smaller finding: some failed attempts this round showed "notes couldn't be written" in the Meetings list (not seen in the pre-fix round) — not investigated, flagging for the next pass. Full evidence + screenshot of the actual stuck page: `coordination/qa-campaign/evidence/legion-qa91-retest2/REPORT.md`. | cc-lantern-legionverify (Legion, live Teams meeting, 3x reproduced, root cause directly captured from the companion webview) | NEW |
| QA-82 | P1 (QA-81 layer 4 — BREAKTHROUGH: card now genuinely joins and is seen live, but app doesn't recognize its own success) | **The launcher fix (tip `bb3d68a1`, 3 layers) works: the Notice Card now reliably reaches the lobby, gets admitted, and genuinely becomes visible to a real second participant — proven live with a real two-person Teams meeting, not inferred.** Used a genuinely separate second participant this round (Chrome Guest mode, launched on the Legion's own local desktop via the pyautogui agent, not the shared server Chrome) — solves the "single shared profile" limitation disclosed as unsolved in QA-80/81. Two solo attempts first (`-7`, `-8`) each timed out at exactly 2:00 with `join-timeout` (not the launcher-page `page-unrecognized` from QA-81) — proving the card now sits in the lobby waiting, past the launcher. Third attempt (`-9`), with the guest browser ready: host's Participants panel showed **"Waiting in lobby (2)"** — both "Recording Notice (Guest)" and the guest observer — admitted both; **the guest's own browser then showed a visible, clearly-labeled, non-black "Recording Notice (Guest)" tile** (screenshot proof, not inferred). **But ~28 seconds after that same admission, `.consent-ledger.json` logs `notice-card-failed` with reason `page-unrecognized` again** (a different meaning this time — not the launcher, but whatever DOM state the adapter expects post-admission, which this join flow doesn't produce) — the app force-closed the companion window, the tile **vanished from the guest's own view** moments after appearing, and the app told the presenter "Notice card couldn't join" despite having just succeeded. Net effect for a live demo: Step 5 would show the tile flicker in then disappear ~30s later while the presenter's own screen incorrectly reports failure — not yet safe to run live. Fix target is narrow and follows the same pattern as QA-81: `detectPhase`'s selectors for the `'admitted'` phase don't match the real post-admission page for this join flow (anonymous/guest join via the rewritten `/v2/` URL) — same playbook (capture the real DOM at that moment, retarget selectors) that resolved the launcher and prejoin phases in the last two rounds. Secondary, smaller finding: the host's participants panel showed 2 extra stale "Recording Notice (Guest)" rows marked "Leaving..." left over from the two earlier solo attempts, still present when the fresh one joined — flagging in case more than one companion-window instance can be alive across attempts. Full evidence: `coordination/qa-campaign/evidence/legion-qa91-retest3/REPORT.md`. | cc-lantern-legionverify (Legion, live 2-person Teams meeting, genuinely separate second participant, root cause directly captured) | NEW |
| QA-83 | P1 (demo-critical regression — the exact "clean sidebar" fix from QA-91-retest2 no longer holds after a full app restart) | **The sidebar-archived-clients fix (`lp/sidebar-archived`, verified PASS twice already at tips `f7847f63` and `bb3d68a1`) regresses after a full app restart: the left rail shows all 42 archived test clients again, even though the main Clients table still correctly shows only the 3 real households + a collapsed "Archived clients (42)" row.** Found incidentally while verifying the F4 staging job (step-3 PDF trigger kit): restarted the Legion app (kill `lantern.exe` + `msedgewebview2.exe`, `Start-ScheduledTask KeepanceDev` — the same restart the demo runbook's own pre-flight checklist and the newly-built step-3 trigger both rely on to force a full reindex) to capture the "Indexing PDFs: X/Y" banner. The banner appeared and worked correctly (see the F4 write-up in `coordination/qa-campaign/evidence/legion-staging/REPORT.md`), but screenshots taken both mid-boot and well after boot finished (several minutes later, not a timing/race artifact) both show the sidebar listing "Caldwell, Jennifer / Diaz, Michelle / Diaz, Sandra / Ellison, Robert & Margaret / Foster, Ronald & Linda / Greer, Carol & Anthony / Hollings Family / Jennings, Carol / Jennings, Robert / ..." — all archived names — while the main content pane correctly shows "3 clients, 3 folders indexed" and the same 3 real households. This means `Spine.tsx`'s data source reverts to the unfiltered `useMatters()` (or an equivalent stale/unfiltered read) specifically on a **fresh app launch**, even though the non-archived filtering clearly works elsewhere (the main table) and evidently works right after an in-app archive action (per the retest2/retest3 verifications, which did not involve a full process restart in between). **This is demo-critical because BOTH the runbook's own pre-flight checklist ("fully restart the app once... before anyone arrives") AND the new step-3 PDF-trigger-kit technique (F4) rely on exactly this restart to work** — meaning a presenter who follows either of those steps would see the sidebar bug live, right before or during the demo. Screenshots: `coordination/qa-campaign/evidence/legion-staging/screenshots/04-f4-DEFINITIVE-indexing-pdfs-17-of-36-banner.jpg` (mid-boot, archived names already visible in the sidebar) and `05-f4-after-index-done-sidebar-regression-found.jpg` (well after boot, same archived names, unchanged). Fix should re-examine whether `Spine.tsx` reads matters from a source that's populated synchronously at launch (before the archived-filter/localStorage-derived state is ready) vs. `MattersHome.tsx`'s `useNonArchivedMatters()`, which is either populated later or filters correctly regardless of load order. | cc-lantern-legionverify (Legion, found live during F4 staging verification, reproduced on 2 separate screenshots minutes apart) | NEW |
