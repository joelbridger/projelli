...
### R2 — kill the standalone clients list; the rail is THE client home (REWORKED per Jameson 2026-07-07)
Jameson's later decision supersedes the earlier list-restructure items: the separate clients-list screen in the main window GOES AWAY. The left rail's expandable clients list is the one client home, gaining a permanent "All Clients" entry pinned ABOVE the individual clients (this replaces and renames the Whole Book view). Selecting "All Clients" → the main window shows the practice-wide table (the old whole-book view) with the one-line rows built for this round: Ask/Documents/Email/Meetings/Activity buttons beside the name, 3-dot menu (Archive + sensible shortcuts) at row end, comfortable whitespace, light-line tab styling, a New Client button in its header. Selecting an individual client → their client-map/tabs view (R3 semantics). No Clients/Whole-Book toggle anywhere — the rail entry IS the switch.

### R3 — navigation model (Tier B, the deepest thinking round)
Left-rail client search bar (top of the expandable list — advisors have 100+ clients). Client Map tab semantics: no selection → blank canvas + "Click a client on the left"; with selection → straight to THAT client's map. There is NO clients-list screen anymore (R2) — "All Clients" in the rail shows the practice-wide table. Fix B3. Remove the "only" scope badge from map top-right (the rail selection replaces its job). **App-wide Back button** (history stack for cross-surface jumps: Ask→source→Documents→back to the same Ask conversation).

### R4 — Client Map depth (Tier B)
Source-click routes per B1. Documents-tab toolbar persistence (feedback line 10, recovered by the cross-check sweep): opening a document must NOT hide the New Tab / toolbar buttons — the toolbar stays put.  Edit history: every bullet edit recorded (who/what/when) and reflected against its source — advisors need an audit trail. Confirmation modals: remove-section AND remove-bullet. Add-bullet on every section (pre-made + custom). Sync: circle-arrows icon next to the client name that re-scans documents and refreshes the map + "last updated" evidence beside it. Export/Share: branded (Advisor Prep Hero) PDF/Word of the map.
