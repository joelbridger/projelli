...
- **Frozen (branch/service/infra names, different dirs):** `keepance-3.0` branch references (many files), `keepance-backend.service`/`keepance-firm-backend*` (RUNBOOK.md), `keepance-wt-*` / `lantern-backups` / `lantern-jump-feasibility` directory names (REPO-MAP-CURRENT.md, feasibility docs).
