...
2. **Zoom pass (same session):** log into the Zoom demo account (creds `~/lantern-coordination/demo-creds/zoom-account.md`, browser), host an instant meeting, join from a second browser context, record it in the app the same way. Verify both channels contain the real call audio + transcription + note lands. This confirms the capture mechanism on Zoom (Teams already confirmed; Meet awaits Jameson's account tap).
