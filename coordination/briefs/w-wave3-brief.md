...
## ANCHOR CORRECTIONS (coordinator drift-check vs lantern-plus @307bcbde, 2026-07-03 — trust these over the plan's cites)
1. ⚠️ SEMANTIC: the internal workspace-data folder RENAMED `.keepance` → `.lantern` (main line's rename Phase 1, absorbed). Wherever the plan says to write workspace-data (consent ledger, transcription-queue state, Task 12c's meeting-types.json) under a literal `.keepance/...` path: instead resolve through the new seam `src-tauri/src/commands/data_dir.rs` — `workspace_data_dir()` / `resolve_workspace_relative()`. NEVER hardcode either folder name (the module doc says exactly this). Meeting ARTIFACT folders (<matter>/Meetings/...) are ordinary workspace files — unaffected.
2. `rag/store.rs` is now a DIRECTORY: the transcript source_type allowlist the plan cites at store.rs:189 lives at `rag/store/write.rs:75` (see also `rag/transcript.rs`). Behavior unchanged.
3. `lib.rs`: `transcribe_audio` registration is now at ~line 117 (not 99-100); "register beside it" still applies.
4. All other anchors verified line-exact or within a couple lines (voice.rs:38 30s cap, audit/store.rs:453 append, parakeet.rs:79, sidecars/mod.rs:31, connector/mod.rs:20 5-arg signature incl. the plan's own no-wrapper caveat, clientMap types.ts:32+160, VoiceCapture.ts:45/87, HUB_TABS insertion point, ClientMapHubTab union).
