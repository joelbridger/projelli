/* Projelli shared site navigation injector.
 * Removes any existing <nav> at the top of <body> and inserts the canonical
 * homepage-style nav. Skipped if the page already has .projelli-nav (which
 * the homepage itself does not, so no double-render).
 */
(function () {
  if (document.querySelector('.projelli-nav')) return;
  ...
  // Logo SVG markup. Trusted constant (no user input).
  var LOGO_SVG = '<svg viewBox="0 0 2697 727" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
  ...
  function buildLogo() {
    ...
    a.href = '/';
    a.className = 'projelli-nav-logo';
    a.setAttribute('aria-label', 'Projelli home');
    // Parse trusted, hardcoded SVG markup safely (no user input involved).
    var doc = new DOMParser().parseFromString(LOGO_SVG, 'image/svg+xml');
    var svg = doc.documentElement;
    if (svg && svg.tagName.toLowerCase() === 'svg') {
      a.appendChild(document.importNode(svg, true));
    } else {
      // Fallback: text logo if SVG parsing somehow fails.
      a.textContent = 'Projelli';
      a.style.fontWeight = '700';
      a.style.color = '#111F35';
  ...
  function buildNav() {
    ...
    inner.className = 'projelli-nav-inner';
