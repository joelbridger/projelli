...
| **API Key Storage** | OS Keychain (Tauri, `KeychainService` Tauri backend) → `localStorage` fallback (base64-encoded, browser/dev only). NB: a legacy `useApiKeys` path still mirrors the raw key into `localStorage` (`apiKey_<provider>`) — a known wart that should be routed through `KeychainService`; the intended store is the keychain. |
