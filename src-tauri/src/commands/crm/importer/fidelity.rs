...
pub fn build_fidelity_report(
    ...
    let mut per_type = BTreeMap::<String, TypeFidelity>::new();
    ...
    let affected_households = affected_attachment_households
        ...
        .cloned()
        .collect::<BTreeSet<_>>();
    let attachment_accounting_complete = affected_households.iter().all(|household| {
        attachments.iter().any(|record| {
            &record.household_ref == household
                && matches!(
                    record.status,
                    AttachmentStatus::Exported | AttachmentStatus::Gap
                )
        })
    });
    ...
    let in_flight_workflows_complete = checklists
        .iter()
        .all(|item| item.decision != ChecklistDecision::Pending);
