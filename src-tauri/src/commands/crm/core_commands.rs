...
/// Native-only seal for the fixed local Hendricks walkthrough. It has no
/// provider path and persists its signed envelope only in SQLCipher.
#[tauri::command]
pub async fn crm_hendricks_review_capability_seal(
    state: State<'_, CrmState>,
    manifest: Value,
) -> Result<Value, String> {
    let workspace = workspace(&state).await?;
    tokio::task::spawn_blocking(move || {
        CrmCoreStore::open(&workspace)?.seal_hendricks_review_capability(&manifest)
    })
    .await
    .map_err(|error| error.to_string())?
    .map_err(|error| error.to_string())
