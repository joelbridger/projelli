...
use crate::commands::{
    crm::{core_model::VerifiedMailboxIdentity, core_store::CrmCoreStore},
    mail::{
...
/// Native evidence after a future sealed adapter has resolved a real mailbox.
/// This has no provider transport, OAuth, or renderer constructor.
#[derive(Debug)]
...
/// A record read from the existing SQLCipher CRM store. Identity-only fields
/// are intentional: this type cannot carry recipient or draft content.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(super) struct VerifiedMailboxRecord(VerifiedMailboxIdentity);

impl VerifiedMailboxRecord {
    pub(super) fn from_identity(identity: VerifiedMailboxIdentity) -> Self {
        Self(identity)
    }
    ...
    pub(super) fn pre_dispatch_matches(
        &self,
        ...
        self.0.workspace_handle == workspace.native_handle()
            ...
            && self.0.canonical_address == resolved_address
...
fn current_mailbox(
    ...
    transaction.query_row(
        ...
        decode_identity,
    ).optional().context("load current verified mailbox").map(|row| row.map(VerifiedMailboxRecord))
}

/// Creates the one current mailbox identity for a workspace/provider. Existing
/// records are accepted only when every identity and generation value is exact;
/// a second account or copied record fails closed.
pub(super) fn register_current_verified_mailbox(
    ...
    store.m4_shared_foundation_transaction(|transaction| {
        if let Some(existing) = current_mailbox(transaction, workspace, credential.provider())? {
            if existing.pre_dispatch_matches(workspace, credential, &evidence.canonical_address)
                && existing.0.display_label == evidence.display_label()
            {
                return Ok(existing);
            }
        ...
        let identity = VerifiedMailboxIdentity {
            ...
            mailbox_handle: mailbox_handle(workspace, credential),
            provider_subject: credential.subject().as_str().to_string(),
            canonical_address: evidence.canonical_address.clone(),
            ...
            verified_source: evidence.verified_source.clone(),
            version: 1,
            status: MailboxState::Verified.as_db().to_string(),
        };
        transaction.execute(
        ...
        Ok(VerifiedMailboxRecord(identity))
    })
}
...
pub(super) fn transition_mailbox(
    ...
    store.m4_shared_foundation_transaction(|transaction| {
        ...
        if current.version() != expected_version || !current.state()?.can_transition_to(next) {
            bail!("invalid verified-mailbox transition or stale version")
        }
        transaction.execute(
            "UPDATE verified_mailboxes SET status=?1,version=version+1 WHERE workspace_handle=?2 AND provider=?3 AND version=?4",
            params![next.as_db(), workspace.native_handle(), provider.as_db(), expected_version],
        )?;
...
mod tests {
    ...
    fn m4_shared_foundation_refuses_wrong_workspace_provider_address_generation_and_second_account()
    {
        ...
        .unwrap();
        assert!(!record.pre_dispatch_matches(
        ...
        assert!(!record.pre_dispatch_matches(&workspace, &credential, "other@example.com"));
        assert!(!record.pre_dispatch_matches(
