...
impl GoogleOAuth {
    ...
    async fn send(&self, request: reqwest::RequestBuilder) -> anyhow::Result<reqwest::Response> {
        let Some((policy, operation)) = self.network_policy.as_ref() else {
            ...
            #[cfg(not(test))]
            anyhow::bail!("Google OAuth requires a NetworkPolicy before it can make a request");
        };
        let authorized =
            crate::commands::connector_network::authorize_url(policy, operation, &self.base)?;
    ...
    pub async fn exchange_code(
        ...
        let resp = self
            .send(self.http.post(&self.base).form(&[
...
mod tests {
    ...
    fn build_auth_url_contains_required_params() {
        ...
        assert!(
            url.contains("gmail.compose"),
            "gmail.compose scope missing from URL: {url}"
        );
        assert!(
    ...
    fn store_or_rollback_on_cancel_never_stores_when_cancelled_before_the_call() {
        ...
        assert_eq!(
            stored, None,
            "a canceled flow must leave NO stored credential"
        );
    ...
    fn store_or_rollback_on_cancel_deletes_the_credential_if_cancel_lands_right_after_the_store() {
        ...
        assert!(
            result.is_err(),
            "a late cancel must still abort the connect"
        );
        assert_eq!(result.unwrap_err(), "cancelled");
    ...
    fn store_or_rollback_on_cancel_restores_a_prior_value_instead_of_only_deleting() {
        ...
        let cancel = AtomicBool::new(false);
        let fake_keychain: RefCell<Option<String>> =
            RefCell::new(Some("old-working-token".to_string()));
        ...
        assert!(
            result.is_err(),
            "a late cancel must still abort the reconnect"
        );
        assert_eq!(
    ...
    fn store_or_rollback_on_cancel_propagates_a_real_store_error_without_deleting() {
        ...
        assert!(
            !deleted,
            "nothing was stored, so delete must not run for an unrelated store failure"
        );
    }
    ...
    async fn gmail_live_smoke() {
        ...
        if let Ok(code) = std::env::var("GMAIL_CODE") {
            ...
            let secret = std::env::var("LANTERN_GMAIL_CLIENT_SECRET")
                .expect("set LANTERN_GMAIL_CLIENT_SECRET");
