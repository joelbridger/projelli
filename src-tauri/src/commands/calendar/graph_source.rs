    ...
    pub fn new(policy: crate::network_policy::NetworkPolicy) -> Self {
        ...
        Self::new_with_base(
            ...
            move || {
                ...
                async move { super::commands::fresh_ms_access_token(&policy).await }
            },
            policy,
        )
    }
        ...
        Fut: std::future::Future<Output = Result<String, String>> + Send + 'static,
    {
        ...
        Self {
            ...
            operation: {
                ...
                #[cfg(not(test))]
                {
                    crate::network_policy::OUTLOOK_CALENDAR_SYNC
                }
            },
        }
    }
...
impl CalendarSource for GraphCalendarSource {
    ...
    async fn fetch_events(
        ...
        let self_emails = fetch_self_emails(
            &self.base_url,
            &self.http,
            &access,
            &self.policy,
...
async fn fetch_self_emails(
    ...
    let Ok(authorized) = crate::commands::connector_network::authorize_url(policy, operation, &url)
    else {
        return Vec::new();
    };
    let Ok(resp) =
        crate::commands::connector_network::await_authorized(policy, &authorized, async {
            Ok(http.get(&url).bearer_auth(access).send().await?)
        })
        .await
    else {
        return Vec::new();
...
mod tests {
    ...
    fn extracts_join_url_from_online_meeting_then_legacy_then_none() {
        ...
        let legacy =
            serde_json::json!({ "onlineMeetingUrl": "https://teams.microsoft.com/legacy" });
        assert_eq!(
        ...
        assert_eq!(
            extract_graph_join_url(&serde_json::json!({ "onlineMeeting": { "joinUrl": "" } })),
            None
        );
        assert_eq!(
            extract_graph_join_url(&serde_json::json!({ "subject": "no link" })),
            None
        );
    }
    ...
    async fn fetches_maps_and_pages_calendar_view() {
        ...
        let source = GraphCalendarSource::new_with_base(
            server.uri(),
        ...
        assert!(
            events[1].self_declined,
            "responseStatus declined maps to self_declined"
        );
    }
    ...
    async fn keeps_all_attendees_when_the_self_lookup_fails_rather_than_erroring_the_whole_fetch() {
        ...
        assert_eq!(
            events[0].attendees.len(),
            1,
            "unfiltered when self-email can't be determined"
        );
    }
    ...
    fn map_graph_event_filters_on_either_mail_or_upn_when_they_differ() {
        ...
        let self_emails = vec![
            "adv@firm.com".to_string(),
            "advisor@firm.onmicrosoft.com".to_string(),
        ];
