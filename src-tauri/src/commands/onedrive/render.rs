use crate::commands::onedrive::model::{is_pending_pdf, is_supported_office_or_text};
...
mod tests {
    ...
    #[test]
    fn pdf_is_pending_not_unsupported() {
        assert_eq!(
            downloaded_document_bytes_to_text("motion.pdf", b"%PDF").unwrap(),
            RenderedDocument::PendingPdf
        );
    }

    #[test]
    fn utf8_text_extracts() {
        assert_eq!(
            downloaded_document_bytes_to_text("note.txt", b"hello").unwrap(),
            RenderedDocument::Text("hello".to_string())
        );
    }
}
