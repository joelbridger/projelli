...
The runner started and owned its own Vite server after proving every non-evidence file still matched the product build. This includes the pre-development asset generator and its inputs. It refused to run if port 5174 was already occupied, so another checkout could not supply the screen.

The audit store used a private, unlocked Secret Service keychain on a disposable private D-Bus session without reading or changing Jameson’s real keychain. The private bus and keychain ended with the run. Audit persistence was not bypassed. The CRM core used the project launcher’s documented deterministic headless test key; its SQLCipher store remained encrypted.

## Files

- `packaged-restart-drive.log` — runner-owned transcript, replaced on every attempt so a failure cannot leave an older PASS log in place.
