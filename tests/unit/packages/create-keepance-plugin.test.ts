// Smoke test for the create-keepance-plugin CLI.
//
// Verifies that:
...
//      after wiring the local @keepance/plugin-api into node_modules.
//
// `--no-install` is used because the @keepance/plugin-api package is not
...
const CLI_BIN = resolve(REPO_ROOT, 'packages', 'create-keepance-plugin', 'bin', 'create.js');
...
describe('create-keepance-plugin CLI', () => {
  beforeAll(() => {
  ...
  it('scaffolds the expected file tree with placeholders substituted', () => {
    ...
    try {
      ...
      expect(pkgJson.devDependencies['@keepance/plugin-api']).toBeDefined();
  ...
  it('the scaffolded plugin builds with vite to a non-empty ES module bundle', () => {
    ...
    try {
      ...
      mkdirSync(join(target, 'node_modules', '@keepance'), { recursive: true });
      symlinkSync(
        PLUGIN_API_DIR,
        join(target, 'node_modules', '@keepance', 'plugin-api'),
        'dir',
      );
