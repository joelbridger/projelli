...
describe('deriveCompleteness', () => {
  ...
  it('is getting-there with a moderate base', () => {
    expect(deriveCompleteness([sec([known('a'), known('b'), known('c'), known('d'), known('e')])], [gap('ask one', 'standing')]).level).toBe('getting-there');
  });
  ...
  it('routes items into know vs assuming and passes the section-targeted ask list through', () => {
    const r = deriveCompleteness([sec([known('a'), assumed('b')])], [gap('what is the deadline?', 'upcoming')]);
    expect(r.know.map((i) => i.id)).toEqual(['a']);
    expect(r.assuming.map((i) => i.id)).toEqual(['b']);
    expect(r.ask).toEqual([{ text: 'what is the deadline?', sectionKey: 'upcoming' }]);
  });
});
