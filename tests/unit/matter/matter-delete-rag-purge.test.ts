/**
 * BUG-040 — deleting a matter must purge its chunks from the local RAG index,
 * so its content can't resurface through all-matters retrieval (which applies
 * no matter filter). This guards the wiring: deleteMatter -> ragDeleteMatter(id).
 */
