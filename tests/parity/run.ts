...
try {
  ...
  if (!(await portReady(5174))) {
    ...
    while (!(await portReady(5174))) {
      if (Date.now() > end) fail('Vite did not start on port 5174');
      await delay(200);
    }
  }
  desktop = start(
    ...
    { ...process.env, LANTERN_DEV_BRIDGE_PORT: String(port) }
  );
  await waitForBridge();
  ...
  for (const feature of FEATURES.filter((feature) => !feature.pending))
    results.push({
      id: feature.id,
      name: feature.name,
      area: feature.area,
      verdict: feature.verdict,
      status: 'FAILING',
      detail: `Runner setup failed: ${infrastructureError}`,
    });
  for (const feature of FEATURES.filter((feature) => feature.pending))
    results.push({
      id: feature.id,
