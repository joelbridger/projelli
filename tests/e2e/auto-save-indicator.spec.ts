...
test.describe('Auto-save indicator (UX-17)', () => {
  test('shows "Saved" (idle state) when no file is open', async ({ page }) => {
    ...
    // The indicator lives in MainPanel's toolbar, which only renders on the
    // standalone editor surface (sidebarActiveTab === 'files') — not the
    // default Client Map landing surface. See helpers/test-utils.ts.
  ...
  test('cycles to "Unsaved changes" when a tab is dirty', async ({ page }) => {
    ...
    await openStandaloneFile(
      page,
      '/test-workspace/autosave-dirty.md',
      'autosave-dirty.md',
      '# Autosave dirty test'
    );
