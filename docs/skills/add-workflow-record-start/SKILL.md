...
Do not reorder existing entries. The Workflows host calls the public
`mountWorkflowRecordStarts()` doorway only for a workflow request and supplies
the matching household's `matterId` when it is known.
...
Run the public extension-point test, the record-start feature tests, the live
workflow lifecycle harness, both TypeScript checks, boundaries, architecture
