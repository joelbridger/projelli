# Keepance v1.5, Release Tracking
...
- **Branded start screen** (`brand/KeepanceLogo.tsx`, `brand/GradientGlow.tsx`), the v1.5 launch copy can reuse this
...
- Windows: `Keepance_1.5.0_x64-setup.exe` (NSIS, Azure-signed) + `Keepance_1.5.0_x64_en-US.msi` (MSI, Azure-signed)
- macOS Apple Silicon: `Keepance_1.5.0_aarch64.dmg` (Developer ID signed, unnotarized)
- macOS Intel: `Keepance_1.5.0_x64.dmg` (Developer ID signed, unnotarized)
...
3. **MCP (Flag 2):**
   ...
   - Install the `.mcpb` into Claude Desktop. Ask Claude "search my Keepance workspace for X" and confirm it returns results.
   - Ask Claude to write a file via MCP. Confirm the Keepance approval modal surfaces and that Approve actually writes the file.
...
7. **Overall:** use Keepance for real work for ~24-48 hours. File any regressions as issues on `keepance/keepance`. Once you're happy with dogfood, Phase 8 tags `v1.5.0` as the real release and promotes the draft.
...
| `97ed333` | M4 (1/3), Real Keepance MCP server binary (JSON-RPC 2.0 over stdio) |
...
- No em dashes in any Keepance copy (`feedback_no_em_dashes.md`)
