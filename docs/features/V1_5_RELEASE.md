...
| M6 | Voice input via whisper.cpp | ✅ Done, real engine verified end-to-end (2026-07-04) | New `src-tauri/src/commands/voice.rs` exposes `voice_sidecar_available()` + `transcribe_audio(wav_bytes, model?)`, delegating to `ParakeetSidecar::transcribe` (`src-tauri/src/sidecars/parakeet.rs`) — the same code path `transcribe_meeting` (Meetings feature) already used. **2026-07-04 correction:** the original implementation above (`--stdin`/`-` args) was written before any real engine existed and was never actually tested against one — a real build of `ggml-org/whisper.cpp` has no stdin mode at all. Rewrote the contract to the real, verified one: WAV bytes are written to a temp file, the engine is invoked as `-f <file> -np -nt -m <model-path>` (verified against a locally-built `whisper-cli --help` and a real transcription of its own `samples/jfk.wav`). Model tiers (tiny/base/small) resolve to real bundled ggml files with an honest fallback (`resolve_model_path`) when a tier isn't staged — `small` (466 MB) isn't bundled today, an install-size trade-off; `tiny` + `base` are. 16 Rust tests in `parakeet.rs` (arg-building against the real contract, tier resolution + fallback ordering, temp-file write/cleanup via a stub engine) + 1 opt-in test that runs the actual whisper-cli binary, gated behind env vars since CI/gate don't vendor a real engine for hermetic unit tests. **CI staging, fixed:** `scripts/fetch-voice-models.sh` (pinned tiny/base ggml models + SHA256) + `scripts/build-voice-sidecar.sh` (builds whisper.cpp from source, statically linked so there are no sibling DLLs to stage) replace the old `VOICE_SIDECAR_URL`-gated no-op steps in `release.yml` that never actually ran — voice shipped disabled in every installer from M6's original 2026-04 landing until this fix. Frontend (unchanged by this fix): `VoiceCapture` wraps `getUserMedia` + `MediaRecorder` + an AudioContext re-encoding pass into 16 kHz mono 16-bit WAV. `usePressToTalk` registers `Ctrl+Shift+Space` (insert into focused input) and `Ctrl+Shift+N` (save transcript to `Inbox/note-<ISO>.md`); `PressToTalkIndicator` renders a pulsing mic badge while recording. New Voice settings category (schema-driven) surfaces enable toggle, model select (tiny/base/small), read-only hotkey displays, and a status pill that reports "Voice ready / Sidecar missing / Mic permission denied". |
...
**Flag 4 (offline voice + local models) is complete, and voice now actually works.** Q7 ships the Ollama provider so founders with a laptop and a model pulled locally get free, zero-network inference wired into the same chat surface as Claude/OpenAI/Gemini. M6 ships the voice input stack: press-to-talk inserts transcribed speech into whatever text field is focused; a second hotkey writes the transcript to `Inbox/`. Transcription runs through a bundled, statically-linked real whisper.cpp engine, invoked with its actual verified CLI contract (a real file path in, a real model file, a 30-second timeout) — not the `--stdin` mode M6 originally assumed, which the real engine has never had. Audio never leaves the machine. CI now builds and stages the real engine + tiny/base models on every platform (`scripts/build-voice-sidecar.sh` + `scripts/fetch-voice-models.sh`); the old `VOICE_SIDECAR_URL`-gated no-op is gone.
