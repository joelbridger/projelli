...
2. **High — a timed-out file can leave STALE CITATIONS.** The timeout path only bumps counters; it does not delete the file's old chunks. If a file changed and then hung, Advisor Prep Hero can still answer from the old indexed version. For a product whose promise is trustworthy citations, a stale citation is worse than a missing one.
