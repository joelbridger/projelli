...
|---|---|---|
| Logical access (CC6.1) | AI provider keys and store-encryption keys held in the OS keychain (`keychain.rs`); no shared cloud datastore of customer content to access. | No written access-control policy; no formal access reviews; firm-tier identity/SSO and per-seat access (designed in `spikes/firm-sync/DECISION.md`) not yet built. |
