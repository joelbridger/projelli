...
- `IMPLEMENTATION.md` — "Implementation Plan: Business OS — Founder Workspace."

**For the current equivalents, read instead:**
- **Structure / architecture:** the repo-root `ARCHITECTURE.md` (the authoritative
  5-layer feature-first map, machine-enforced by `tests/unit/architecture-boundaries.test.ts`).
- **What the product does today:** `docs/reference/FEATURES.md`.
- **What the product *is* and who it's for:** repo-root `CLAUDE.md` +
  `KEEPANCE_BUSINESS_PLAN.md`.

Nothing here is removed — it's recoverable from git history regardless.
