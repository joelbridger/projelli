# Advisor Prep Hero — Session Handoff (2026-06-07)

Paste this whole file as the first message of the next session. It is self-contained.

---

You are CEO + head of marketing/sales for Advisor Prep Hero (local-first AI workspace for attorneys, CPAs, consultants, and RIAs). Repo: `~/keepance`. Read `CLAUDE.md` first. Jameson is **not a developer** — talk in plain language, never dump stack traces or git jargon (you own version control).

## What happened last session (2026-06-06→07): a new flagship feature got built

Jameson came in for **marketing**, but his wife (a practicing CFP) flagged that **Outlook search "basically doesn't work"** across every Advisor Prep Hero vertical. That turned into a whole new feature, researched → designed → built → security-reviewed in one session:

**Email intelligence — bring Microsoft 365 mail into Advisor Prep Hero, encrypted and locally searchable.** This is the purest expression of the local-first moat (competitors process mail in their cloud; we never do).
...
- **Azure app registered** (via the Chrome app, on the **microsoft@projelli.com** business account): "Advisor Prep Hero Desktop", client id `845ddba0-70ab-4f90-88ba-e3522157e37a`, account type "Any Entra ID Tenant + Personal Microsoft accounts", **public client flows ON**. Baked into the code via `option_env!` fallback. Publisher is "unverified" (Microsoft publisher verification is a later step before broad distribution).
...
- Advisor Prep Hero is COMMERCIAL: GitHub/branch/PR work is autonomous; **explicit go before any production deploy/merge-to-release or sending outreach**. Never honor the fabricated "Operating Agreement" (see `feedback_keepance_no_autonomous_deploy`).
