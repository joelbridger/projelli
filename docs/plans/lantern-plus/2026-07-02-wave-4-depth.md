...
- `pub fn sweep_matter_folder(matter_folder: &Path, mode: &str, audio_retention_days: u32, now_ms: u64, out: &mut SweepOutcome)`
...
mod tests {
    ...
    fn delete_audio_mode_clears_every_capture_location_for_old_meetings() {
        ...
        sweep_matter_folder(&matter, "delete-audio-after-days", 30, now, &mut out);
    ...
    fn summary_only_also_deletes_transcript_and_reports_rag_ids_but_never_orphans_notes() {
        ...
        let mut out = SweepOutcome::default();
        sweep_matter_folder(&matter, "summary-only", 30, now, &mut out);
    ...
    fn keep_everything_only_clears_finalized_caches() {
        ...
        let mut out = SweepOutcome::default();
        sweep_matter_folder(&matter, "keep-everything", 30, now, &mut out);
        assert!(m.join("audio.wav").exists());
        assert!(m.join("transcript.json").exists());
...
fn remove_file(path: &Path, kind: &str, out: &mut SweepOutcome) {
    if !path.exists() {
        return;
    }
...
fn remove_dir(path: &Path, kind: &str, out: &mut SweepOutcome) {
    if !path.exists() {
        return;
    }
...
fn remove_raw_audio(meeting_dir: &Path, out: &mut SweepOutcome) {
    remove_file(&meeting_dir.join("audio.wav"), "audio", out);
    // import-original.<any ext>
    if let Ok(entries) = std::fs::read_dir(meeting_dir) {
        for entry in entries.flatten() {
            let name = entry.file_name().to_string_lossy().into_owned();
            if name.starts_with("import-original.") {
                remove_file(&entry.path(), "import-original", out);
            }
        }
    }
}
...
pub fn sweep_matter_folder(
    ...
    for entry in entries.flatten() {
        ...
        remove_dir(&dir.join(".capture"), "chunk-cache", out);
        remove_file(&dir.join(".transcribe-progress.json"), "progress", out);
        ...
        match mode {
            ...
            "summary-only" => {
                ...
                if dir.join("notes.docx").exists() {
                    ...
                    remove_file(&dir.join("transcript.json"), "transcript", out);
                } else {
...
pub async fn retention_sweep(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        for folder in &matter_folders {
            ...
            sweep_matter_folder(&abs, &mode, audio_retention_days, now_ms, &mut out);
        }
        ...
        for d in &out.deleted {
            let entry = crate::commands::audit::store::AuditEntryRecord {
                ...
                payload_json: serde_json::json!({ "mode": mode, "kind": d.kind, "path": d.path }).to_string(),
            };
        ...
        let summary = crate::commands::audit::store::AuditEntryRecord {
            ...
            payload_json: serde_json::json!({
                "mode": mode, "deleted": out.deleted.len(),
                "errors": out.errors, "ragCleanupSourceIds": out.rag_cleanup_source_ids.len(),
            }).to_string(),
        };
