...
**Resource flake — FIXED (not just documented):** `tests/security/intake-hosting.test.ts` "signs the compiled Vite output" spawned a real `tsc+vite` build inside vitest → `ERR_INSUFFICIENT_RESOURCES` under gate concurrency. Fixed by pre-building `intake-page/dist` serially in a new gate step + the test reuses it (`buildPage:false`) with a build-if-missing `beforeAll` fallback (`e210cc8b`). backend `bun test` "failure" was the same concurrency pressure (211/0 in isolation) — resolved by the same load reduction.

### Gate-fix round (after the first full gate, all now green independently)
Quality regressions the scoped per-lane tests missed, all fixed: typecheck:tests tuple guard; 23 ESLint findings (async `.catch`, i18n wrapping, void-expr) in `OnboardingTab`/`NewClientDialog` (`f5c21737`); 12 hard-coded colors → tokens; then the i18n follow-on: 9 new en.json keys (5 `intake.onboarding.*` + 4 `matter.new-client.*`) added to de.json + es.json with real translations, en-json namespace-inventory snapshot updated (intake 5, matter 326), security-test flake fixed (`e210cc8b`). Final gate attempt 3 running.
