...
- Ask-once suppression fires only when an item has an explicit `factMapping` (subject + factKind) and the client's active fact matches both fields exactly. Never guess from a filename, a label, or a document upload's expected type. A superseded fact (not currently active) does not suppress.
...
function resolveAskOnce(
  items: RequestItem[],           // draft items with factMapping-bearing descriptors, or the RequestItemDescriptor list from Lane 1
...
Pure function — no I/O, no store access, easy to test exhaustively. For each item with an explicit `factMapping`, check whether `matches` contains an entry with the same `subject` and `kind`. If yes, move it to `suppressed` (it will not be sealed into the client's checklist) and surface the "Already on file" status to the advisor in the review UI. Items without a `factMapping` (document uploads, readonly cards) always pass through untouched — never infer a mapping from a label or expected doc type.
