...
5. **Cross-request/wrong-class submissions are rejected, not mis-filed.** A submission carrying the onboarding request's ID cannot satisfy a standing item, and vice versa. An unknown `relayItemHandle`, a JSON body against a `file_upload`-class item, a file against a `json_fact`-class item, and an upload exceeding the descriptor's MIME/size/count policy are all flagged and remain **unacked** (assert no ack call reaches your mocked relay client for any of these cases).
