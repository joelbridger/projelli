...
- **Wave 3 Lane 1 (ingest+match — the ONLY Rust lane, the blocker): BUILT + reviewed, FIX ROUND IN FLIGHT.**
  ...
  - **UPDATE (fix now COMPLETE):** the fix round finished — commit **`4dc20449`** on `lp/intake-w3-1` (worktree `~/lp-w3-1`), tree clean, phrase `W3-LANE1-FIX-SENDER-FAILCLOSED` present, Codex claims green (vitest 91, cargo `commands::mail::graph` 23, tsc, eslint). **NOT yet lead-verified or merged** — that is the continuing session's FIRST task (§2.1–2.3): independently re-verify (don't trust the claim), read the 2 fixed spots, merge `--no-ff`, gate (serialize cargo), push.
  - The 2 findings that were fixed: **[P1]** M365 Graph sync didn't fetch `internetMessageHeaders` → every Outlook reply stored `authResult: missing` → matcher quarantines ALL M365 replies (feature quarantine-only for the primary provider). **[P2]** sender parse trusted the first `<...>` and ignored trailing text → `Evil <sarah@x> <attacker@evil>` parsed as sarah (spoof-gate bypass) → must fail closed.
