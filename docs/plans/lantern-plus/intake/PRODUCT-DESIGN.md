...
2. **Imported vendor/custodian PDFs** — the advisor drops in a Schwab (or any vendor) PDF. Fillable AcroForm PDFs are parsed into a field map; mapped fields become normal checklist items on the client page (one at a time, same experience), prefilled from existing client facts wherever we already know the answer (ask once, made visible — except the most sensitive values, which are never sent back out to the page; those items say "already on file, confirm or replace" instead, per ARCHITECTURE.md §9a); the filled PDF is regenerated and sealed like any other payload. Flat scanned PDFs get an honest degraded path (advisor-side field overlay, or attach-print-sign-photograph) rather than a fake one.
...
- **Custodian-grade: DocuSign.** The shipped connector reads envelope status today; **sending** envelopes is new build (WAVE-PLAN.md Wave 9), gated on vendor credentials. Honesty rule: a document routed through DocuSign transits DocuSign's cloud — that is what custodians accept and expect, and the Integration Honesty Card says it plainly rather than implying the E2EE story covers it.
...
- **Save/resume is automatic and invisible.** Progress saves after every item (encrypted; see ARCHITECTURE.md). Reopening the same link on any device resumes at the next incomplete item, and "done" checkmarks come from the secure record of what was actually submitted, never from anything a link holder could tamper with. No password, no "session expired."
...
2. **Sender authenticity — two distinct paths, not a sliding scale:** a message that passes email authentication (DKIM/SPF with DMARC alignment, as recorded by the advisor's mail provider) proceeds to normal proposal flow with the confidence tiers below. A message that fails or lacks authentication (a from-address alone is trivially forgeable) is **quarantined**: it appears as a separate manual-review card with nothing pre-selected, no one-click accept, and an unmissable warning ("this email did not prove it came from Sarah's account — verify with her before accepting anything"). Quarantine, not silence, because legitimate clients on misconfigured mail domains exist and must not vanish; the advisor stays the judge either way.
...
6. **Confidence tiers (authenticated mail only):** high (auto-checked in the proposal, one-click accept), medium (pre-selected but shows its reasoning line), low (listed unchecked). Quarantined mail never enters the tiers. Nothing is ever filed without the advisor's accept — the confirmation step is the product, not a speed bump, because a silently mis-filed SSN is the one mistake this feature can never make.
