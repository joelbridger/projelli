...
4. ~~**Matter-delete semantics (BUG-042)**~~ — ✅ **DECIDED + DONE (2026-06-21).** Jameson's call: **Delete = remove the matter + wipe the AI's memory of it, but KEEP the user's files** (Archive already = hide-and-keep; deleting documents would violate a law practice's retention duties). Closed the one real data gap: deleting a matter now also clears every email's durable "filed to this matter" tag (Rust `clear_message_matter_for_matter` + `mail_clear_matter_filings` command + JS wiring in `deleteMatter`), so a later sync can't resurface emails under a deleted matter. TDD'd both sides; the delete-confirm message is now honest. Files stay on disk and re-index as 'unassigned' (correct — they're still in the workspace).
