...
- ✅ Typed an instruction, "Suggest changes" enabled, ran it → **real OpenAI call returned 2 correct tracked changes**: "it's → its" (possessive) and "Payment are → Payments are" (subject-verb), shown as proper Word redlines in the page AND in the Review pane (author "Keepance AI"). **E4 PASS.**
