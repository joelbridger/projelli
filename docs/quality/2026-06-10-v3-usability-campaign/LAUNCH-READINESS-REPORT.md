...
2. **Firm backend deploy — needs your explicit go (NOT auto-deployed).** You authorized "publish + site deploy"; the firm backend deploy (production DB migration + new secrets `LEMONSQUEEZY_WEBHOOK_SECRET` from `~/.local/share/jameworld/keepance-firm-webhook-secret.env` + `FIRM_VARIANT_IDS=1769899`, service restart) is a separate production-infra action I did not take on my own. It's fully prepped: `docs/operations/2026-06-10-firm-provisioning.md` + `backend/deploy/RUNBOOK.md`. Safe to defer: the website Firm card is "Talk to us" (no self-serve path hits the new endpoints), and there are zero firm customers. Say the word and I deploy it.
