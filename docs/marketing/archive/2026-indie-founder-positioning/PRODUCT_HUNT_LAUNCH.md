...
> Shipped. .deb, .rpm, and .AppImage are all in every release. The CI pipeline (GitHub Actions) builds Mac (signed + notarized), Windows (Azure-signed), and Linux (.deb/.rpm/.AppImage) on every tagged release. Linux fell out of the cross-platform CI basically for free.
...
> Four providers, BYOK: Anthropic (Claude Sonnet, Opus, Haiku), OpenAI (GPT-4o, GPT-4o-mini, o1), Google Gemini (2.0 Flash, 2.0 Pro), and Ollama (any local model: llama3.2, mistral, qwen2.5, phi3, etc.). The cloud model lists auto-fetch from each provider's API so new releases show up automatically. Ollama auto-detects models from your local install. Use Ollama if you want zero API cost and fully-offline operation.

### 7. "Does the free trial actually work or is it crippled / nagware?"

> 30-day full-feature trial, no card required. For 30 days you get every Projelli feature: AI chat with your own keys (all 4 providers), all 15 workflow templates, unlimited workspaces, all panels, RAG over your workspace, MCP server, side-by-side AI editing, voice input, the works. After 30 days, AI chat sends and workflow runs pause until you activate a license; existing files stay fully readable forever. No nagware, no countdown popups during the trial, no fake "limited preview" gates.
...
### 13. "No image input? Even ChatGPT has multimodal."

> Right now Projelli supports image PASTE in the editor (drops a Markdown image link, saves to your workspace media folder), but doesn't send images to the AI for visual analysis yet. Multimodal AI input is on the post-launch backlog, ~30-45 days. For chart screenshots / UI feedback / visual analysis, use Claude.ai or ChatGPT directly in the meantime. I'd rather ship multimodal correctly than ship it half-done.

### 14. "Can the AI read my PDFs?"

> PDFs render in the viewer but the text isn't extracted into chat context yet. PDF chat is on the post-launch backlog (same engineering category as multimodal). Workaround for now: copy-paste the relevant text into chat. The RAG system (Memory) does index Markdown / text files in your workspace already, so any text content the AI already sees from "@workspace" queries.

### 15. "Where's mobile / iPad?"

> Desktop only. Mac (signed + notarized), Windows (Azure-signed), Linux (.deb/.rpm/.AppImage). Mobile would require either cloud sync (which compromises local-first) or local-network sync (a meaningful product on its own). I might build a read-only mobile companion in 2026 once the desktop product has paying customers; full mobile is post-year-1. If you need mobile-first, Notion or Bear are better fits today.

### 16. "Does it work with Claude Desktop / Cursor / Zed?"

> Yes — Projelli ships its own MCP server bundled as a `.mcpb` Desktop Extension (one-click install from Settings → Integrations). Once installed, your Projelli workspace becomes a queryable tool inside Claude Desktop, Cursor, Zed, or any MCP-compatible client. They get five tools: list_workspace_files, read_workspace_file, search_workspace (uses the same RAG retrieval as in-app), write_workspace_file (with confirmation), and get_memory_facts. Most apps consume MCP. Projelli serves it.

### 17. "Why no plugin system like Obsidian?"

> Obsidian's plugin model is amazing for power users; it also creates the version-fragmentation problem where "install these 12 plugins to get my setup" is the on-ramp. Projelli ships workflow templates + AI integration + RAG + memory + MCP in the box, no plugin assembly required. Different bet. If your Obsidian setup with 15 plugins is working for you, don't switch. If you want a single coherent app that does the AI workspace stuff out of the box, Projelli.

### 18. "Can I import my Notion / Obsidian content?"

> Notion → Projelli: Notion's built-in export creates Markdown — drop those exports into your Projelli workspace folder, done. Obsidian vaults are already plain Markdown in folders, so just point Projelli at the vault folder and it works. We don't have a fancy one-click importer yet but the "plain Markdown in folders" format means existing tools hand off cleanly. Importing other tools (Mem, Reflect, Roam) requires their export flow first; varies by tool.
