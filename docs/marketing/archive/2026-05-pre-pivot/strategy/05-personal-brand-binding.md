...
|---|---|---|---|
| **Brand voice (Projelli)** | `@projelliproject` on X, `projelli` on IH, `projelli` press kit author, blog author = "the team behind Projelli" | Daily (X), weekly (blog), as-needed (IH/Reddit replies) | Marketing engine, daily community presence, replies to launch threads |
...
**What `@projelliproject` never posts:**
