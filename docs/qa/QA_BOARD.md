...
| bench pass 3 PART 1.3 — OneDrive "Sync now" total silence (HIGH, regression *behind* pass-2's honest error on the same path) | OneDrive connector (TS + Rust command/event) | Claude (Sonnet 5) | fix/onedrive-sync-silence | 🟢 fixed (pushed, awaiting lead merge) | No single breaking commit — checked every commit `16043fab..b163540b` touching any OneDrive TS/Rust file: zero. Root cause is structural, not a regression from one diff: `runSync()`'s first step, `oneDriveListFolders()` (recursive, sequential, un-paginated-progress folder-tree walk used for client-folder auto-linking), ran before `onedrive_sync` ever emitted a progress event, had no cancellation, and no bound beyond each Graph call's own 60s timeout — pass-2's test happened to fail fast at token refresh (masking this), pass-3's had a working token and walked straight into the silent, unbounded phase. Fix: Rust `tokio::time::timeout` deadline on both `onedrive_list_folders`/`onedrive_sync` (command can never hang the frontend forever) + folder-discovery now honors the same cancel flag as sync (Stop actually interrupts it); frontend `localSyncing` state covers the whole operation (instant spinner) + a client-side `withOneDriveTimeout` backstop kept strictly above the matching Rust deadline (2 Codex review rounds — round 1 caught a same-magnitude timeout that could false-positive a legit long sync, round 2 caught a stale "Imported N files" message lingering next to a folder-discovery stop, both fixed + regression-tested). Verify: typecheck + typecheck:tests clean, `lint:gate` net-new 0, scoped `cargo test --lib commands::onedrive` 38/38 passed (3 new: cancel-before-start, cancel-mid-walk, outer-timeout-bounds-a-stall), full pre-push vitest 5298 passed/6 skipped (1 unrelated pre-existing OCR-asset gap in a fresh worktree); 7 new frontend tests covering the settle-guarantee, discovery-phase spinner, honest-stop, and stale-report-on-stop. HEAD `5b62cbbe`. |
