...
| bench pass 2 #10 — "Reopen last workspace" ignored | WorkspaceSelector / boot lifecycle (TS) | Claude (Sonnet 5) | fix/boot-auto-resume | 🟢 fixed (pushed, awaiting lead merge) | typecheck clean, `lint:gate` net-new 0, 9 hook tests + 3 timeout-fallback tests + 3 store tests green (Codex-found duplicate-reopen regression, and a post-merge review round's hang-timeout + locked-vault-fallback findings, all covered); full pre-push vitest 5208/5214 passed; 2 Codex review rounds + 1 independent post-merge review round, all addressed |
| bench pass 3 PART 1.3 — OneDrive "Sync now" total silence (HIGH, regression *behind* pass-2's honest error on the same path) | OneDrive connector (TS + Rust command/event) | Claude (Sonnet 5) | fix/onedrive-sync-silence | 🟢 fixed (pushed, awaiting lead merge — round-2 lead review found 2 more P2s in the settle/cancel contract, both closed) | No single breaking commit — checked every commit `16043fab..b163540b` touching any OneDrive TS/Rust file: zero. Root cause is structural, not a regression from one diff: `runSync()`'s first step, `oneDriveListFolders()` (recursive, sequential, un-paginated-progress folder-tree walk used for client-folder auto-linking), ran before `onedrive_sync` ever emitted a progress event, had no cancellation, and no bound beyond each Graph call's own 60s timeout — pass-2's test happened to fail fast at token refresh (masking this), pass-3's had a working token and walked straight into the silent, unbounded phase. Fix: Rust `tokio::time::timeout` deadline on both `onedrive_list_folders`/`onedrive_sync` (command can never hang the frontend forever) + folder-discovery now honors the same cancel flag as sync (Stop actually interrupts it); frontend `localSyncing` state covers the whole operation (instant spinner) + a client-side `withOneDriveTimeout` backstop kept strictly above the matching Rust deadline. **Round-1 Codex (2 rounds):** a same-magnitude timeout that could false-positive a legit long sync, and a stale "Imported N files" message lingering next to a folder-discovery stop — both fixed. **Round-2 lead review (this round, on @5b62cbbe):** (P2-a) a stuck Rust `{status:'syncing'}` event left over from before a genuine stall could out-live the frontend timeout backstop — `syncing` stayed derived-true (spinner stuck, button stuck disabled) even after `localSyncing` cleared, because the catch only called `setError()` and never touched `progress`; fixed by force-setting `progress` to `{status:'error'}` on any non-cancelled failure. (P2-b) `collect_folders`'s cancel check only ran at the top of each loop iteration, so a Stop landing during the very LAST item's in-flight `list_children` call had no next iteration to catch it — the walk returned `Ok(())` as if Stop was never clicked, and `onedrive_sync` (called right after) unconditionally resets `state.cancel` at its own start, silently erasing the click and starting the sync anyway; fixed by re-checking `cancel` immediately AFTER the await too. Both round-2 fixes verified red→green (reverted the fix, confirmed the exact assertion/test failure the review described, restored the fix, confirmed pass) before landing. Verify: typecheck + typecheck:tests clean, `lint:gate` net-new 0, scoped `cargo test --lib commands::onedrive` 39/39 passed (4 new Rust regression tests total), full pre-push vitest green (8 new frontend tests total: settle-guarantee, discovery-phase spinner, honest-stop, stale-report-on-stop, stuck-`'syncing'`-signal). HEAD `234dc649` (round-1) → round-2 commit pending push. |
