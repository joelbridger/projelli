...
Observation (NOT a product bug): the bench's Advisor Prep HeroTest chat defaults to **gpt-3.5-turbo**, which OpenAI now returns HTTP 400 for — so live chat sends in that workspace fail (the app handles it correctly: logs the failure, shows an error, no crash). The earlier working AI used gpt-4o (workflow) / the redline model. To live-confirm BUG-082 token/cost, switch the chat model to an available one (e.g. gpt-4o) first.
...
## Hunt update #9 (2026-06-22) — systematic Codex audit of ALL 51 workflow templates (the BUG-096 bug-class)
Rather than hand-drive 49 more multi-field interviews, ran a Codex static audit of **every** template under `src/features/workflows/engine/templates/` for the whole "broken/impossible workflow" class: (1) every interview field `type` is one `InterviewForm` renders (now text/textarea/select/multiselect), (2) every `{{placeholder}}` in every prompt maps to a real field id or a valid engine output key, (3) required selects/multiselects have non-empty options, (4) no duplicate/dangling field/step ids. Codex built an in-memory TypeScript AST checker to scan them all.
- **Result: ZERO high-severity bugs across all 51 templates.** BUG-096 (multiselect) was the ONLY un-completable workflow, and it's fixed. The whole workflow surface is otherwise sound — this gives high confidence without driving each by hand.
- **6 low-severity prompt-quality findings (→ BUG-097, being fixed by Codex on a `wf-tokens` worktree):** a handful of templates contain non-`\w+` brace tokens the engine's `{{\w+}}` regex can't substitute, so a raw `{{...}}` could leak verbatim into a generated document — e.g. `{{#if toaRequired}}`/`{{/if}}` (legal CitationFormatter + EngagementLetterDrafter), `{{taxYear minus 1}}` (tax QuarterlyEstimateReminder), and several `{{field — instruction}}` tokens (advisor RegBI/RegSP/BooksRecords). Not un-completable — a polish/trust issue in a legal-doc product. Fix: replace each with a bare `{{field}}` + plain-English instruction (or a real computed field), plus a regression test asserting every template token is a bare identifier mapping to a real field. **BUG-097 SHIPPED + merged + pushed (`97a8606`).** BUG-096 + BUG-097 + the eslint-drift fix all live on keepance-3.0; bench synced to HEAD; workspace reopened; gate green (vitest 3725 / cargo 597).

## Hunt update #10 (2026-06-22) — env note: dev-proxy "Failed to fetch" + a positive workflow-failure-audit confirmation
Tried to LIVE-confirm the BUG-097 fix by running the **Engagement Letter with AI Disclosure Clause** workflow (which had a `{{#if additionalTerms}}` conversion) with `additionalTerms` left blank (the omit path), to inspect the output `.docx` for any raw `{{}}`.
- **Result: the workflow's AI call FAILED with "Failed to fetch"** — so no output document was produced to inspect. Same error a chat send hit. **Root cause = environment, NOT a product bug:** the Evidence Gap Analyzer + Privilege Log Drafter workflows SUCCEEDED with gpt-4o earlier today; both AI "Failed to fetch" errors appeared only AFTER I reloaded the page (to reset a `chatContextTokenLimit` test value) + reopened the workspace via the WebFS folder picker. That points at the **Vite dev-server OpenAI proxy** getting into a bad state post-reload (the dev server proxies OpenAI to avoid browser CORS; the real Tauri desktop app fetches OpenAI directly with no proxy, so this path doesn't exist in production). To resume live AI testing on the bench, the dev server (Advisor Prep HeroDev task / `run-dev.bat`) likely needs a restart.
- **Positive confirmation (BUG-079 class, at the WORKFLOW level):** the failed run audited correctly — `AI Request Sent` (egress, gpt-4o) at 12:13:04, then `Workflow Failed` (gpt-4o, reason `"Failed to fetch"`, `auditPersistenceStatus: saved`) at 12:13:14. A failed AI call is logged as an honest FAILURE with the real reason — NOT a false success — and the app did not crash. (Previously confirmed for chat sends; now confirmed for workflow runs too.)
- **BUG-097 verification status:** PROVEN by its regression test (`tests/unit/workflow-template-tokens.test.ts` scans every template prompt and asserts no non-bare token survives — green) + code inspection (the `{{#if}}` blocks are gone, replaced with plain instructions referencing real `{{toaRequired}}`/`{{additionalTerms}}` fields). The LIVE output-document inspection is the one piece still pending, blocked only by the dev-proxy "Failed to fetch" above — not by the fix.
- **Bench test-pollution cleaned/noted:** the AI Assistant chat in Advisor Prep HeroTest accumulated ~101K tokens of junk history from earlier over-limit tests; combined with the dev-proxy issue, that chat is a poor vehicle for a clean cited-answer test. A FRESH chat (small context) + a working model (the picker offers GPT-4/4.1/Turbo — confirmed it populates fine; Radix dropdowns need a REAL desktop-agent click, synthetic clicks don't open them) is the way to live-confirm the headline cited-answer feature next session.

## Hunt update #11 (2026-06-22) — found + fixed BUG-098 LIVE: the headline cited-answer feature was 100% broken on Windows
After restarting the bench dev server (the hunt #10 dev-proxy blocker — Vite/node had died, leaving the Rust app orphaned; clean restart fixed it), I drove the headline feature. Discovery that the **AI Assistant chat (Ctrl+Shift+A) does NOT do RAG** — retrieval lives in the dedicated **Search/Ask** surface (`useAsk.ts`) — and that the Ask surface picks its model by key-presence with the OpenAIProvider default (`gpt-4o`), independent of the chat model picker. So the real headline test is on Search, not chat.

- **BUG-098 FOUND + (TS layer) FIXED + RE-CONFIRMED LIVE — the 4th real bug from live Windows driving (after BUG-093/094/096).** Every in-corpus Search/Ask question returned a CORRECT retrieval-grounded answer but was flagged **"Not cited from your files"** with zero citation chips — the central promise broken on Windows. Root cause: the RAG store holds each document TWICE under two path spellings (`C:/Advisor Prep HeroTest\file` from the full-workspace index vs `C:\Advisor Prep HeroTest\file` from the file watcher); `resolveCitationTarget` saw two "different" paths for one file → `uniquePaths.size > 1` → dropped every citation. Also doubles top-K (crowding real docs out — why a 2-part question's deadline half hallucinated). Confirmed via the captured OpenAI request body (two source headers per file) + a direct `rag_retrieve` (two chunks/file, distinct ids). TS fix: separator-normalize the resolver's collision check (genuine cross-folder collisions still fail closed). LIVE proof: same engagement-letter question post-fix → GREEN chip `[1]` + "Answered over your own files. Every cited claim can be checked against the source." + "Source found → fee-agreement.md §0". Rust root-cause layer (canonicalize stored paths + `INDEX_VERSION` bump → drop/rebuild migration) IN PROGRESS via Codex — needs a Rust recompile + bench re-index to confirm the duplicate is gone.
- **Token/cost audit (BUG-082 full) ✅** — a successful `gpt-4.1` chat send populated the cost chip ("$0.02 this chat · $0.02 today"), confirming persisted token/cost (the piece pending since hunt #2/#3).
- **Citation honesty held throughout** — even pre-fix, the ungrounded/uncited answers correctly carried the "Not cited — verify this" warning (no fabricated answer presented as trusted); the fix flips correctly-grounded answers from that warning to the green "Source found" state.

## Hunt update #12 (2026-06-22) — BUG-098 Rust layer reverted (exposed pre-existing BUG-099 indexing hang); P2/P3 confirmed live
- **BUG-098 Rust layer REVERTED (`e11d421`), TS layer stands as the shipped fix.** Live bench testing of the Rust store-canonicalization + `INDEX_VERSION` bump showed the forced re-index never completes and OOM-crashes the app on launch. Root cause is **BUG-099** (pre-existing): one workspace file hangs the indexer at 20/21, so the version-gated rebuild can't finish → migration retries → drop/rebuild churn → memory runaway → `0xc0000409` abort. Reverted the Rust commit to keep the branch safe; `git diff bfb29d1 HEAD -- src-tauri/` is empty. The headline cited-answer feature stays fixed by the TS resolver (live-verified green earlier this session). Store-side dedup deferred until BUG-099 is fixed + a memory-safe migration is designed and bench-verified.
- **BUG-099 filed (pre-existing indexing hang at 20/21).** Was masked at v10 (store already built); fatal only under the forced rebuild. Prerequisite for re-landing the BUG-098 Rust dedup.
- **P2 / BUG-097 ✅ live-confirmed** — ran "Engagement Letter with AI Disclosure Clause" with Additional terms BLANK; the generated .docx has ZERO raw `{{}}` tokens (checked word/document.xml + all parts) and is a clean, well-formed letter with no dangling/empty "Additional terms" section.
- **P3 / trash→restore ✅ live-confirmed** — deleting a file moves it to `.trash` (soft delete, confirm dialog, recoverable); Restore moves it back to its folder and clears `.trash`. Verified on disk both directions.
- **Token/cost audit (BUG-082 full) ✅** — a successful gpt-4.1 chat send populated the cost chip ($0.02 this chat / today).
