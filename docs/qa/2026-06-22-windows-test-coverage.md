...
- **BUG-098 FOUND + (TS layer) FIXED + RE-CONFIRMED LIVE — the 4th real bug from live Windows driving (after BUG-093/094/096).** Every in-corpus Search/Ask question returned a CORRECT retrieval-grounded answer but was flagged **"Not cited from your files"** with zero citation chips — the central promise broken on Windows. Root cause: the RAG store holds each document TWICE under two path spellings (`C:/KeepanceTest\file` from the full-workspace index vs `C:\KeepanceTest\file` from the file watcher); `resolveCitationTarget` saw two "different" paths for one file → `uniquePaths.size > 1` → dropped every citation. Also doubles top-K (crowding real docs out — why a 2-part question's deadline half hallucinated). Confirmed via the captured OpenAI request body (two source headers per file) + a direct `rag_retrieve` (two chunks/file, distinct ids). TS fix: separator-normalize the resolver's collision check (genuine cross-folder collisions still fail closed). LIVE proof: same engagement-letter question post-fix → GREEN chip `[1]` + "Answered over your own files. Every cited claim can be checked against the source." + "Source found → fee-agreement.md §0". Rust root-cause layer (canonicalize stored paths + `INDEX_VERSION` bump → drop/rebuild migration) IN PROGRESS via Codex — needs a Rust recompile + bench re-index to confirm the duplicate is gone.
