...
**Advisor Prep Hero privacy and security overview, for financial advisors and their compliance teams.**

*This is a plain-language summary of how Advisor Prep Hero handles data. Every claim below reflects how the app actually works today. We would rather tell you straight than over-promise.*

---

## The short version

Advisor Prep Hero runs on your own computer. Your client files, the search index it builds, and the "Client Map" it creates all stay on your machine. Nothing about your clients is stored on a Advisor Prep Hero server. The only time your client information leaves your computer is when you choose to use a cloud AI, and in that case it goes straight from your computer to the AI provider on your own account, not through a Advisor Prep Hero server.

*(This sheet describes the two ways a solo or small-firm advisor runs Advisor Prep Hero: fully on your own machine, or with your own AI key.)*

## What stays on your computer

- **Your files and notes.** They remain ordinary files in a folder you choose. Advisor Prep Hero does not upload or sync them anywhere.
- **The search index.** Advisor Prep Hero reads your documents and builds a private search index on your machine so it can find things fast. That index, and the AI "embeddings" behind it, are created on your computer and stored on your computer.
- **The Client Map.** The profile Advisor Prep Hero builds for each client household lives in the app on your machine.
...
This is the part advisors and compliance teams care about most, so here it is clearly. Advisor Prep Hero gives you two ways to run the AI:
...
2. **Bring-your-own-key mode.** You connect your own account with an AI provider (such as Anthropic, OpenAI, or Google) using your own key. When you ask a question, the relevant text is sent **directly from your computer to that provider**, on your account, to get the answer. It does **not** pass through any Advisor Prep Hero server. We never see it.
...
## What Advisor Prep Hero (the company) can and cannot see
...
When you connect Microsoft 365, Gmail, or another mailbox, Advisor Prep Hero talks **directly** to your email provider from your computer. Your email does not route through any Advisor Prep Hero server.

## For your compliance officer

Advisor Prep Hero is designed so that **client data stays under the advisor's control.** Practically, that means:
- There is no Advisor Prep Hero cloud holding your clients' files, so there is one fewer outside vendor to vet, monitor, and worry about under data-protection rules such as Reg S-P.
