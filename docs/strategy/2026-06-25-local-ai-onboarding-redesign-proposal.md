# Onboarding redesign — make "Keepance Local AI" the primary private path (Ticket 6, part 2)
...
Make **Keepance Local AI the front-and-center "keep everything private" choice** in onboarding, and demote the Ollama setup to an **Advanced** option for power users.

Concretely:
1. The **"Local-only / Most private"** choice in onboarding leads to a single, friendly **"Get Keepance Local AI"** step — one button, we download it for you in the background, no Ollama, no terminal commands.
...
3. **Ollama moves to Settings → Advanced** (for the rare user who already runs their own local model and wants to point Keepance at it). It stops being the face of "private."
4. The marketing page `keepance.com/local-model-setup` gets repositioned: a new primary "Keepance Local AI (built in)" story up top, with the existing Ollama guide kept as the "advanced / bring-your-own" section. *(Do this after the feature ships, so the page doesn't promise something not yet downloadable.)*
...
**D3 — Default selection.** Today **Cloud (bring-your-own-key)** is the visual default ("Recommended"). For a privacy-first product aimed at advisors, do we make **Keepance Local AI the recommended default**, or keep cloud as default and present local as the equal/privacy choice? (I lean: make local the recommended default for the privacy story, cloud clearly offered for users who want maximum capability. This is a brand/positioning call — your area.)

**D4 — The three-way vs two-way choice.** Right now it's Local-only / Cloud / Decide-later. With a built-in engine, I'd keep three: **Keepance Local AI (private)** / **Your own cloud key (most capable)** / **Decide later**. Confirm that's the shape you want.
...
- Keep the local-AI piece as a small, reusable component (the "Get Keepance Local AI" step + the choice logic), and
...
- **Done (shipped):** the engine, the in-chat picker, the Settings "Download Keepance Local AI" control, the app-wide progress banner. So the building blocks for the onboarding step already exist and are tested.
