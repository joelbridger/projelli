# Keepance: Independent Four-Vertical Review (Post-V2-Overhaul)
...
**Verified, and this is the most important *accuracy* finding.** The product's honest position, stated correctly on `/ai-workspace-privacy/`, is: Keepance's servers never see your data; with a **cloud** key the prompt still goes to your chosen AI provider (so the provider sees it); only a **local** model means nothing leaves the machine. But three vertical pages quietly drop that nuance at the headline:
...
- **Consulting:** "Keepance sidesteps the clause entirely / there's no upload." A client's "no uploading work product to AI services" clause is about whether the text reaches a third-party AI at all. With a cloud key, it does. Only a local model truly sidesteps that clause.

The **advisor page gets this right** consistently ("there is no Keepance server in the middle" kept carefully separate from "nothing leaves your machine," which it reserves for local models). Use the advisor page as the model for the other three.
...
> 1. **Keep Heppner everywhere it appears (9 files).** It is the single best authority for Keepance's entire thesis: a federal court just held that pouring your work into consumer cloud AI can destroy privilege.
...
> 3. **Keepance's characterization ("consumer AI use without attorney direction may offer no privilege protection") is accurate and appropriately hedged** ("may," "without attorney direction" tracks the Kovel point). Keep it.
...
- Advisor: a **CCO-ready "how Keepance fits your Reg S-P program"** memo.
...
- **`/fits-your-stack/`** and the "what Keepance is not" framing earn trust by telling buyers what you do not do.
