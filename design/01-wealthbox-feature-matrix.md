...
| Org Admin cross-workspace user management | Centralized admin console managing users across many workspaces | Premier Enterprise | UNVERIFIED | not mentioned | REPLICATE — firm overview is a read-only view of the existing firm-admin rails: workspaces, members, seats, roles, and access. It deliberately does not create a second permissions system. |
