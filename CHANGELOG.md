...
- **UX quick-wins — Batch B (small logic + a11y), each with a test.**
  - **Honest, reactive egress trust badge (UX-01, P0).** `useActiveEgressProvider` no longer claims a saved-default provider that has no key, and re-resolves the moment a key changes mid-session (a `keepance:egress-config-change` event from `useApiKeys` + the cross-tab `storage` event); when nothing is configured it returns a `'none'` sentinel that `EgressIndicator` renders as a neutral "No AI connected" badge. The TrustBar badge is now a `role="status"` live region.
  ...
  - **`.keepance` hidden (UX-21).** The internal dot-prefixed config folder is hidden from the file grid + tree via a shared `visibleNodes` filter (same convention the workspace selector uses).
