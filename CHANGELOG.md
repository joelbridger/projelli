...
### Fixed
- **Deleting a matter no longer leaves "ghost" emails tagged to it (BUG-042).** Matter-delete semantics are now settled: deleting a matter removes it and wipes the AI's memory of it (its search index entries and its email filings), but keeps your actual document files on disk — Archive is the "hide but keep everything" option; Delete never destroys documents. Previously, emails you'd filed to a matter kept that filing in the mail database, so the next email sync re-tagged them to a matter that no longer existed. Now those filings are cleared on delete (and the cleared emails simply re-index as unassigned, like the files). New Rust `EncryptedMailStore::clear_message_matter_for_matter` + `mail_clear_matter_filings` command, wired into `matterStore.deleteMatter` next to the existing index purge. Files: `src-tauri/src/commands/mail/store.rs`, `src-tauri/src/commands/mail/mod.rs`, `src-tauri/src/lib.rs`, `src/platform/utils/mail-commands.ts`, `src/platform/matter/matterStore.ts`; Tests: `src-tauri` `enc_clear_message_matter_for_matter_clears_only_that_matters_filings`, `tests/unit/matter/matter-delete-rag-purge.test.ts`.
