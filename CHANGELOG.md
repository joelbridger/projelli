...
- **UX quick-wins — Batch B (small logic + a11y), each with a test.**
  ...
  - **`.keepance` hidden (UX-21).** ONLY Keepance's internal `.keepance` config folder is hidden from the file grid + tree (via a shared `visibleNodes` filter); ordinary dotfiles like `.gitignore` keep their existing behaviour and still respect the "Show Hidden Files" setting. The desktop `TauriFSBackend.list` used to drop EVERY dotfile before the UI saw it (so "Show Hidden Files" could never reveal `.gitignore`); it now drops only `.keepance` (never recursing into it) and keeps `.trash`, matching the WebFS backend and letting the UI/settings layer decide.
