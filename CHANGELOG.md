...
### Fixed
- **Privacy/egress banner no longer goes stale on a confidentiality mode-switch (B-PRIV-1, MAJOR).** The inline Search/Ask egress banner resolved its provider ONCE on mount, so switching from "On this computer only" (local-only) to Cloud mode mid-session left the banner falsely claiming "On your machine. Nothing leaves … No prompt or file is sent over the network" while the query actually went to the cloud — two egress indicators contradicting each other on one screen. `useAsk` now subscribes to the confidentiality mode and re-resolves the active provider whenever the mode changes, and pins the badge to the engine each send actually resolves to, so the banner can never claim "nothing leaves" while a query goes to the cloud. Files: `src/features/ask/useAsk.ts`, `src/features/ask/Ask.tsx`. Regression test: `tests/unit/ask/egress-banner-mode-switch.test.tsx` (drives the real EgressIndicator + egress logic + reactive settings store through a mid-session mode flip).
