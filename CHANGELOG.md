...
- **Word editor: four data-loss / corruption bugs fixed (fix/docx-dataloss).** Found in a QA hunt driving the real Word editor; all four could silently lose or corrupt an advisor's edits to a real client document.
  ...
  - Tests: 15 new Rust unit tests for C3 (table accept/reject-all, single-revision-in-table resolve, raw-XML markup preservation, plus 6 from the two Codex-review rounds: missing-`w:id`, stray `delText`, and the early-exit/double-count edge cases) + 2 new Rust integration tests (final-clean strips a malformed id-less table deletion and a stray delText), 4 new Rust tests + 1 wire-level test for C4, 3 new TS unit tests for `atParagraphStart`, 4 new DocxEditor component tests for C1/C2/C3-P2 (unmount-before-blur, export-before-blur, concurrent-op-ordering, table-only-revisions Accept All enablement), 2 new `docx-dom.ts` `countRevisions` tests, 2 new `writeCoordinator.resetPath` tests. i18n snapshot updated for the one new locale key (`media.docx-editor.concurrent-edit-conflict`).
