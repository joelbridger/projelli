...
### Added
- **Control-day feature-map contract checks.** The renderer now requires the complete generated control contract, including the separate 238-row V1 requirement universe (151 Wealthbox, 63 Jump, 24 shared) and its separate 21-row outside-V1 list. It fails clearly on missing, invented, duplicate, malformed, or mismatched control data; keeps those rows out of the 69 journey and 9 foundation feature cards; retains the accepted totals in its stable page marker; and verifies its exact HTML document shape against the protected publisher parser.
- **Lantern board demo videos.** The Demo area now has a light-theme “Demo Videos” sub-tab beside the walkthrough, with the cited-answer and Client Map clips, native playback controls, plain-language captions, and the demo-video-engine note. Video details stay data-driven in `docs/board/board-data.json`; the private board host serves the clips from `/demo-videos/`.
...
### Fixed
- **Lantern Feature Map control renderer.** It now requires the supplied SHA-256 `input_hash`, shows its exact short prefix, retains the full value in a machine-readable page marker, rejects malformed/duplicate control records, keeps outside-V1 wording honest, and has browser-style checks for filtering and comments. Count-chip contrast now meets the light-theme accessibility target.
