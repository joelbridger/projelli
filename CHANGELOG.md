...
- **Word editor: four data-loss / corruption bugs fixed (fix/docx-dataloss).** Found in a QA hunt driving the real Word editor; all four could silently lose or corrupt an advisor's edits to a real client document.
  ...
  - **"Accept All" / "Reject All" silently left deleted text inside tables (CLUSTER-C3, confidentiality).** Tables round-trip as unparsed raw XML (`BlockContent::Raw`), which the paragraph-level accept/reject walk never looked inside — so a deleted client name inside a table survived "Accept All" and stayed in the saved `.docx`. A new stream-level raw-XML resolver (`resolve_raw_xml`, shared with the existing final-clean export path) now walks `w:ins`/`w:del` inside preserved blocks too, for both single-revision accept/reject and accept/reject-all. An independent Codex review then caught a regression the first version of this fix introduced: sharing the resolver made a MALFORMED `w:del`/`w:ins` with no `w:id` attribute at all silently un-matchable, so a third-party/malformed deletion could survive a bulk accept where the original scrubber used to strip it unconditionally as a fail-closed guarantee. Fixed with an explicit `treat_missing_id_as_match` parameter: bulk callers (accept-all/reject-all, final-clean export) pass `true` (fail-closed — never leaves deleted text behind); a single targeted `resolve_revision` passes `false` (can't safely claim an unidentifiable element as the one it was asked to resolve). File: `src-tauri/crates/lantern-docx/src/resolve.rs` (+ `scrub.rs` now delegates to the shared resolver instead of duplicating the logic).
  ...
  - Tests: 9 new Rust unit tests for C3 (table accept/reject-all, single-revision-in-table resolve, raw-XML markup preservation, plus 3 for the Codex-caught missing-`w:id` regression) + 1 new Rust integration test (final-clean strips a malformed id-less table deletion), 4 new Rust tests + 1 wire-level test for C4, 3 new TS unit tests for `atParagraphStart`, 3 new DocxEditor component tests for C1/C2 (unmount-before-blur, export-before-blur, concurrent-op-ordering), 2 new `writeCoordinator.resetPath` tests. i18n snapshot updated for the one new locale key (`media.docx-editor.concurrent-edit-conflict`).
