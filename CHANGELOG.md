...
### Fixed
- **Privacy/egress banner is honest across a mode-switch AND at send time (B-PRIV-1, MAJOR).** The inline Search/Ask egress banner resolved its provider ONCE on mount, so switching from "On this computer only" (local-only) to Cloud mode mid-session left the banner falsely claiming "On your machine. Nothing leaves … No prompt or file is sent over the network" while the query actually went to the cloud. The fix closes every window where the banner could lie:
  - `useAsk` now subscribes to the confidentiality mode and re-resolves the active provider whenever the mode changes.
  - The resolved provider is TAGGED with the mode it was resolved under, and the displayed value is derived SYNCHRONOUSLY in render: if the tag doesn't match the current mode (or it's unresolved) the badge shows a neutral "checking" state. So the instant the mode changes — in the same render, before any effect runs — a provider resolved under the old mode (e.g. a local provider) can no longer paint under the new mode, closing the one-frame stale-badge window.
  - During the async re-resolution window the badge stays on that neutral "checking — nothing leaves while this checks" state.
  - At send time the badge is pinned (mode-tagged) to the engine the send actually resolved to via `flushSync`, so the banner DOM reflects the real destination BEFORE `sendMessage`/`sendMessageStreaming` runs (a plain `setState` could be batched and not repaint in time) — even if the badge was still "checking" at click time.
  Files: `src/features/ask/useAsk.ts`, `src/features/ask/Ask.tsx`. Regression test: `tests/unit/ask/egress-banner-mode-switch.test.tsx` drives the REAL resolver + EgressIndicator + egress logic + reactive settings store: it flips the mode, proves (via a `resolveEgress` spy) the indicator is never even asked to render a local provider under Direct mode (the one-frame guarantee), and — with the resolver held mid-flight — clicks Search and asserts via a `sendMessageStreaming` spy that the banner already shows the real cloud destination at the instant the network call begins.
