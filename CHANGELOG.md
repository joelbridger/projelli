...
### Fixed
- **Privacy/egress banner is honest across a mode-switch AND at send time (B-PRIV-1, MAJOR).** The inline Search/Ask egress banner resolved its provider ONCE on mount, so switching from "On this computer only" (local-only) to Cloud mode mid-session left the banner falsely claiming "On your machine. Nothing leaves … No prompt or file is sent over the network" while the query actually went to the cloud. The fix closes three windows where the banner could lie:
  - `useAsk` now subscribes to the confidentiality mode and re-resolves the active provider whenever the mode changes.
  - During the async re-resolution window the badge is blanked to a neutral "checking — nothing leaves while this checks" state, so a stale local provider + the new Direct mode can never render as "nothing leaves".
  - At send time the badge is pinned to the engine the send actually resolved to via `flushSync`, so the banner DOM reflects the real destination BEFORE `sendMessage`/`sendMessageStreaming` runs (a plain `setState` could be batched and not repaint in time) — even if the badge was still "checking" at click time.
  Files: `src/features/ask/useAsk.ts`, `src/features/ask/Ask.tsx`. Regression test: `tests/unit/ask/egress-banner-mode-switch.test.tsx` drives the REAL resolver + EgressIndicator + egress logic + reactive settings store through a mode flip, and — with the resolver held mid-flight — clicks Search and asserts via a `sendMessageStreaming` spy that the banner already shows the real cloud destination at the instant the network call begins.
