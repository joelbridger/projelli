...
### Added (ui/reimagine branch — Wave A, 2026-07-03 UX evaluation)
- **Shared `AiConsentPrompt` component (`src/ui/kp/AiConsentPrompt.tsx`).** One in-place "needs-you" banner (question + primary action + optional note), built on the existing `Callout`/`Button` shell, so AI-privacy consent moments look and behave the same everywhere instead of three inconsistent mechanisms.
  - **Client Map build gate** (`MatterHub.tsx` + `useClientMap.ts`): the personal-install confidentiality-choice error now renders as "Ready to build the [Client] Client Map — Build map · Uses your cloud AI" instead of a bare sentence pointing at Settings; one click records the choice and retries the build. New `needsConfidentialityChoice` field on `useClientMap`'s return.
  - **Docx "Revise with AI" gate** (`DocxEditor.tsx` + `DocxRedlineControls.tsx`): same treatment — "AI redline needs a one-time privacy choice — Enable cloud AI".

### Fixed (ui/reimagine branch — Wave A)
- **Onboarding icon duplication (`LottiePlayer.tsx`).** React StrictMode's dev-only mount→cleanup→mount double-invoke let a shared cancellation ref get reset by the second mount, un-cancelling the first mount's still-pending `import('lottie-web')` and causing two `<svg>` instances to stack in the same container. Fixed by scoping the cancellation flag to each effect invocation's own closure.
- **Onboarding CTA cut off at 1200×800 (`OnboardingShell.tsx`, `IntroScene.tsx`).** Compressed hero/shell spacing so the "Go!" button is always visible without scrolling at the app's fixed onboarding window size.
- **Sidebar duplicate subtitle (`Spine.tsx`).** A matter row showed its client name twice (title + identical subtitle) whenever the matter had no separate name. Now shows a real secondary fact (folder count) or nothing.
- **Combined indexing count (`RagProgressBanner.tsx`).** Office/text and PDF indexing passes now fold into one honest total ("Indexing 378 files, 73 done · Nothing leaves your machine.") instead of two separate lines; excludes a finished PDF pass still lingering in its post-completion display grace period so it can't leak stale counts into an unrelated new run.
- **Trust-chip copy (`onboarding/v2/copy.ts`).** Rewritten to precise claims that never suggest Advisor Prep Hero itself is SOC 2 certified: "Your data stays on your computer" · "Encrypted on your device (AES-256)" · "Works with SOC 2-certified AI providers (your account)".
- **Reset-to-Defaults demotion (`SettingsContent.tsx`).** Moved the destructive, always-visible red button out of the permanent footer into the Advanced section's "Advanced" sub-tab.
