...
- **QA sweep round 4 — binary-file integrity + a workflow-model fix (Codex-reviewed).**
  ...
  - Tests: `tests/unit/fs/webfs-binary-copy.test.ts`, `tests/unit/fileOps/download-binary.test.tsx`, `tests/unit/utils/is-binary-file.test.ts`, `tests/unit/workflow/workflow-provider-resolution.test.ts`.
