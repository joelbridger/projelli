...
### Fixed
- **Client Documents search now spans the whole client, not just the currently open folder (B4b, bench finding).** In a client's Documents tab, typing in the search box only matched files that were direct children of whichever folder was currently open — a match in a sibling, parent, or nested subfolder never surfaced, and the count label ("N of M items") measured the wrong denominator. `DocumentGridView` now recursively flattens the already client-scoped tree (the same tree `scopeFileTreeToFolders` prunes to this client's own folders — never the raw workspace tree) whenever a search query is active, instead of filtering only the open folder's direct children; a cross-folder result shows its path context (e.g. "Contracts") so opening it doesn't feel like a teleport, and the count label now reads "N results" instead of the no-longer-meaningful "N of M items". Two issues an adversarial Codex review round caught and both fixed: (1) the path-context label could echo an ancestor WRAPPER folder's name that `scopeFileTreeToFolders` keeps purely for navigability — if a client's mapped folder happened to be nested under a folder coincidentally named after something outside its scope, that name could leak into a result card; now trimmed to start at the client's own folder (`createFolderFallback`), never above it. (2) the whole-tree flatten + sort is O(tree size) and is now memoized against the tree reference (not the search query), so it runs once per tree change instead of once per keystroke. Files: `src/features/documents/DocumentGridView.tsx`; tests: `tests/unit/documents/documents-grid-scoped-view.test.tsx` (cross-folder match within a client's scope, another client's file NEVER matches, honest empty state, ancestor-wrapper-name-leak regression), `tests/unit/reimagined-documents-home.test.tsx` (cross-folder match outside the open folder, path-context display, opening a cross-folder result). Verified with two independent Codex review rounds (round 1 clean; round 2 found the two issues above, both fixed and re-verified). TS-only; no Rust change.
