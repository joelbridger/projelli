...
### Fixed
- **Personal OneDrive document sync no longer sends an unsupported delta `$select`.** Consumer Microsoft accounts now use a plain OneDrive root delta call while business drives keep the selected-field delta request, and failed Graph requests log the rejected URL, status, and response body for diagnosis.
