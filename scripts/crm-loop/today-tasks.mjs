...
async function waitForEnabled(testid, seconds = 10) {
  const end = Date.now() + seconds * 1000;
  while (Date.now() < end) {
    if (await evaluate(`(() => { const element = document.querySelector('[data-testid="${testid}"]'); return Boolean(element && !element.disabled); })()`)) return;
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
...
if (!String(textAfterCreate).includes(taskTitle)) fail('created task is not in the list');
...
const task = records.find((record) => record.kind === 'task' && record.title === taskTitle);
