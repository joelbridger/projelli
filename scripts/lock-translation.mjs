...
if (!existsSync(localePath)) usage(`locale file not found: ${localePath}`);
...
for (let i = 0; i < parts.length - 1; i++) {
  ...
  if (!cursor || typeof cursor !== 'object' || !(seg in cursor)) {
    usage(`key not found in ${locale}.json: ${key} (missing segment "${seg}")`);
  }
  cursor = cursor[seg];
}

if (!cursor || typeof cursor !== 'object' || !(leaf in cursor)) {
  usage(`key not found in ${locale}.json: ${key}`);
}

if (typeof cursor[leaf] !== 'string') {
  usage(`key is not a leaf string in ${locale}.json: ${key}`);
}
...
process.stdout.write(`locked: ${locale}/${key}\n`);
