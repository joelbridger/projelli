...
const config = JSON.parse(fs.readFileSync(configPath, 'utf8')),
  inputs = JSON.parse(fs.readFileSync(inputPath, 'utf8')).rows,
  payload = JSON.parse(fs.readFileSync(payloadPath, 'utf8')).rows;
...
for (const [source, target] of Object.entries(config.bundle.resources)) {
  ...
  if (!row)
    throw new Error(`resolved resource missing from guarded inputs: ${source}`);
...
for (const row of payload) {
  ...
  if (!digest)
    throw new Error(`unexplained extracted payload file: ${row.logical_path}`);
...
for (const name of expected.keys())
  if (!seen.has(name)) throw new Error(`expected payload file absent: ${name}`);
