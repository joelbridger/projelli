/**
 * The relay never decrypts a wrapped matter key, but it must still refuse
 * arbitrary text. This is the fixed client/server wire envelope used by
...
export function decodeWrappedKeyEnvelope(value: unknown): Uint8Array | null {
  ...
  // The first byte of an uncompressed P-256 point is fixed. The remaining
  // cryptographic validity is checked by the recipient while unwrapping.
