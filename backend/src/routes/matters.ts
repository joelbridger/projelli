 ...
 */
export async function handleReleaseMatterStream(req: Request, store: Store, handle: string): Promise<Response> {
  const a = admin(req); if (!a.ok) return a.resp;
  const b = await readStrictV2Payload(req, ["stream_handle"]);
  if (!b || !isNonEmptyString(b.stream_handle, 64) || !streamHandle(b.stream_handle)) return error("invalid_v2_payload", 400);
  const m = sameOrgMatter(store, a.claims.org_id, handle); if (!m || m.status !== "active") return error("matter_not_found", 404);
  if (store.getMatterMember(handle, a.claims.sub)?.role !== "owner") return error("forbidden", 403);
  if (!store.releaseMatterStream(handle, b.stream_handle)) return error("stream_not_found", 404);
  store.audit({ org_id: a.claims.org_id, actor_user_id: a.claims.sub, action: "matter.stream.release", target: handle, detail: { op: "stream_release" } });
  return json({ ok: true });
}
...
export async function handleRemoveMatterMember(req: Request, store: Store, handle: string): Promise<Response> {
  const a=admin(req); if(!a.ok)return a.resp;
  const b=await readStrictV2Payload(req,["user_id"]);if(!b||!isNonEmptyString(b.user_id,64)||!sameOrgUser(store,a.claims.org_id,b.user_id))return error("invalid_v2_payload",400);
  const m=sameOrgMatter(store,a.claims.org_id,handle);if(!m)return error("matter_not_found",404); const removed=store.removeMatterMember(handle,b.user_id);const epoch=store.bumpMatterKeyEpoch(handle);store.deleteWrappedKeysForUser(handle,b.user_id);store.deleteWrappedKeysForEpoch(handle,m.key_epoch);
...
export async function handleSetWall(req: Request, store: Store, handle: string): Promise<Response> { const a=admin(req);if(!a.ok)return a.resp;const b=await readStrictV2Payload(req,["user_id"]);if(!b||!isNonEmptyString(b.user_id,64)||!sameOrgUser(store,a.claims.org_id,b.user_id))return error("invalid_v2_payload",400);const m=sameOrgMatter(store,a.claims.org_id,handle);if(!m)return error("matter_not_found",404);store.setEthicalWall({matter_handle:handle,user_id:b.user_id,org_id:a.claims.org_id,created_by:a.claims.sub});const epoch=store.bumpMatterKeyEpoch(handle);store.deleteWrappedKeysForUser(handle,b.user_id);store.deleteWrappedKeysForEpoch(handle,m.key_epoch);store.audit({org_id:a.claims.org_id,actor_user_id:a.claims.sub,action:"matter.wall.set",target:handle,detail:{op:"wall_set",epoch}});return json({ok:true,walled:true,key_epoch:epoch}); }
export async function handleClearWall(req: Request, store: Store, handle: string):Promise<Response> { const a=admin(req);if(!a.ok)return a.resp;const b=await readStrictV2Payload(req,["user_id"]);if(!b||!isNonEmptyString(b.user_id,64)||!sameOrgUser(store,a.claims.org_id,b.user_id))return error("invalid_v2_payload",400);const m=sameOrgMatter(store,a.claims.org_id,handle);if(!m)return error("matter_not_found",404);const cleared=store.clearEthicalWall(handle,b.user_id);store.audit({org_id:a.claims.org_id,actor_user_id:a.claims.sub,action:"matter.wall.clear",target:handle,detail:{op:"wall_clear",count:cleared?1:0}});return json({ok:true,cleared}); }
...
export function authorizeSyncConnect(req:Request,store:Store,tickets:SyncTicketStore=syncTickets):{ok:true;data:{matterHandle:string;streamHandle:string;orgId:string;userId:string;seatId:string;role:AccessTokenClaims["role"]}}|{ok:false;resp:Response} { const ticket=new URL(req.url).searchParams.get("ticket");if(!ticket)return {ok:false,resp:error("unauthorized",401,"missing_ticket")};const b=tickets.redeem(ticket);if(!b)return {ok:false,resp:error("unauthorized",401,"invalid_ticket")};const access=resolveAccess(store,{orgId:b.orgId,userId:b.userId,role:b.role},b.matterHandle);if(!access.allowed||!store.streamBelongsToMatter(b.streamHandle,b.matterHandle))return {ok:false,resp:error("forbidden",403)};return {ok:true,data:{matterHandle:b.matterHandle,streamHandle:b.streamHandle,orgId:b.orgId,userId:b.userId,seatId:b.seatId,role:b.role}}; }
