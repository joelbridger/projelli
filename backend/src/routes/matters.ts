...
export async function handleSetWall(req: Request, store: Store, handle: string): Promise<Response> { const a=admin(req);if(!a.ok)return a.resp;const b=await readStrictV2Payload(req,v2BodyKeys("setWall"));if(!b||!isNonEmptyString(b.user_id,64)||!sameOrgUser(store,a.claims.org_id,b.user_id))return error("invalid_v2_payload",400);const m=sameOrgMatter(store,a.claims.org_id,handle);if(!m)return error("matter_not_found",404);store.setEthicalWall({matter_handle:handle,user_id:b.user_id,org_id:a.claims.org_id,created_by:a.claims.sub});const epoch=store.bumpMatterKeyEpoch(handle);store.deleteWrappedKeysForUser(handle,b.user_id);store.deleteWrappedKeysForEpoch(handle,m.key_epoch);store.audit({org_id:a.claims.org_id,actor_user_id:a.claims.sub,action:"matter.wall.set",target:handle,detail:{op:"wall_set",epoch}});return json({ok:true,walled:true,key_epoch:epoch}); }
