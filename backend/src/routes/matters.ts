...
export async function handleAllocateStream(req:Request,store:Store,handle:string):Promise<Response> { if(await readStrictV2Payload(req,[])===null)return error("invalid_v2_payload",400);const gate=relayGate(req,store,handle,req.headers.get("x-seat-token"),"stream_allocate");if(!gate.ok)return gate.resp;if(gate.matter.status!=="active")return error("matter_not_active",409);return json({stream_handle:store.createMatterStream(handle)},201); }
