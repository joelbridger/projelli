...
export async function handleSyncTicket(req:Request,store:Store,handle:string,stream:string,ip:string,tickets:SyncTicketStore=syncTickets):Promise<Response> { const earlyAuth=authenticate(req);if(!earlyAuth.ok)return error("unauthorized",401,earlyAuth.reason);if(!req.headers.get("x-seat-token"))return error("seat_required",401);const b=await readStrictV2Payload(req,v2BodyKeys("syncTicket"));const since=b?.since;if(typeof since!=="number"||!Number.isSafeInteger(since)||since<0)return error("invalid_v2_payload",400);if(!streamHandle(stream)||!store.streamBelongsToMatter(stream,handle))return error("stream_not_found",404);const rl=rateLimit(ip,"relay_ticket",{max:config.relayRateLimitMax,windowSeconds:config.relayRateLimitWindowSeconds});if(!rl.ok)return error("rate_limited",429);const gate=relayGate(req,store,handle,req.headers.get("x-seat-token"),"ticket");if(!gate.ok)return gate.resp;const r=tickets.mint({matterHandle:handle,streamHandle:stream,orgId:gate.claims.org_id,userId:gate.claims.sub,seatId:gate.seatId,role:gate.claims.role,since});return json({ticket:r.ticket,expires_in_ms:r.expiresInMs}); }
