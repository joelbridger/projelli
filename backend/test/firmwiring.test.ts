...
describe("firm wiring preserved proofs",()=>{
  const cases:Array<[string,()=>Promise<void>|void]>=[
    ...
    ["member invite tells the desktop to release the current key when the invite is not walled",async()=>{const user=store.createUser({org_id:orgId,email:"release@wire.test",password_hash:"x",role:"member"});const r=await post(`/v2/firm/matters/${handle}/members/add`,{user_id:user.user_id,role:"editor"},admin);expect(r).toMatchObject({status:200,body:{key_epoch:store.getMatter(handle)!.key_epoch,key_release:"release_to_member"}})}],
    ["walled member invite explicitly withholds the current key",async()=>{const user=store.createUser({org_id:orgId,email:"walled-release@wire.test",password_hash:"x",role:"member"});await post(`/v2/firm/matters/${handle}/wall/set`,{user_id:user.user_id},admin);const r=await post(`/v2/firm/matters/${handle}/members/add`,{user_id:user.user_id,role:"editor"},admin);expect(r).toMatchObject({status:200,body:{key_release:"blocked_walled"}})}],
    ...
    ["removed v2 stream allocation route is not accepted",async()=>expect((await post(`/v2/firm/matters/${handle}/streams`,{},member)).status).toBe(404)],
    ...
    ["activation returns documented ok shape and records an activate audit",async()=>{const m=(await post("/v2/firm/matters",{},admin)).body;const r=await post(`/v2/firm/matters/${m.matter_handle}/activate`,{},admin);expect(r.body).toMatchObject({ok:true,matter_handle:m.matter_handle,status:"active"});expect(JSON.stringify(r.body)).not.toMatch(/client_name|matter_id/);expect(store.listAudit(orgId).some((event)=>event.action==="matter.activate"&&event.target===m.matter_handle)).toBe(true)}],
