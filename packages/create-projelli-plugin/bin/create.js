#!/usr/bin/env node
// create-projelli-plugin
//
// Scaffolds a new Projelli plugin from the bundled template. Designed to be
// run via `npx create-projelli-plugin <name>` or, in the monorepo, directly
...
function printUsage() {
  process.stdout.write(
    [
      'Usage: create-projelli-plugin <name> [options]',
      '',
      'Arguments:',
...
function fail(msg) {
  process.stderr.write(`create-projelli-plugin: ${msg}\n`);
  process.exit(1);
}
...
function printNextSteps(name, dirRelative, installed) {
  ...
  lines.push(
    ...
    '  Projelli plugins folder for sideloading. See:',
    '    https://projelli.com/docs/plugins/getting-started',
    '',
  );
  ...
  if (args.install) {
    ...
    if (!installed) {
      process.stderr.write(
        'create-projelli-plugin: npm install failed. You can re-run it manually.\n',
      );
    }
  }
