    ...
    return (
      ...
      >
        ...
        <IconButton
          ...
          style={{ color: 'var(--kp-side-fg-dim)', marginBottom: 'var(--kp-space-xs)' }}
        />
        {nav.map(({ id, label, Icon }) => (
          <button key={id} type="button" title={label} aria-current={active === id ? 'page' : undefined}
            data-testid={`spine-nav-collapsed-${id}`}
            onClick={() => onTabChange?.(id)}
            style={{ width: 38, height: 38, borderRadius: 'var(--radius-lg)', border: 0, cursor: 'pointer', color: active === id ? 'var(--kp-side-fg)' : 'var(--kp-side-fg-dim)', background: active === id ? 'var(--kp-side-active-bg)' : 'transparent' }}>
            ...
            return (
              <button key={id} type="button" ref={(el) => { tabRefs.current[id] = el; }}
                ...
                style={{
                  ...
                  fontWeight: on ? 'var(--kp-weight-semibold)' : 'var(--kp-weight-medium)', color: on ? 'var(--kp-side-fg)' : 'var(--kp-side-fg-dim)',
                  background: on ? 'var(--kp-side-active-bg)' : 'transparent',
                }}>
                {on && <span style={{ position: 'absolute', left: 3, width: 3, height: 18, borderRadius: 3, background: 'var(--kp-side-accent)' }} />}
