...
function getRailWidthStyle(railWidth: number | string | undefined): CSSProperties {
  const width = railWidth ?? 'var(--kp-rail-width)';
  ...
  const renderRow = (item: RailShellItem) => {
    ...
    return (
      <div
        ...
        className={cn(
          'group flex min-h-[42px] w-full items-center gap-2 rounded-md border border-transparent px-3 py-2.5 text-left text-[length:var(--kp-rail-row-title-font-size)] transition-colors',
        ...
        {item.content ? (
          ...
          <>
            ...
            <span className="flex min-w-0 flex-1 flex-col gap-0.5">
              ...
              {item.supportingText ? (
                <span className="truncate text-[length:var(--kp-rail-row-meta-font-size)] leading-snug text-[var(--kp-side-fg-dim)]">{item.supportingText}</span>
              ) : null}
            </span>
