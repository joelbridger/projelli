...
const EMPTY_RECEIPTS: Readonly<Record<string, NotesReviewReceipt>> =
  Object.freeze({});
...
export function NotesReviewPanel({
  rawItems = [],
  initialReceipts = EMPTY_RECEIPTS,
  blockedDestinations = {},
  onApprove,
}: NotesReviewPanelProps) {
  ...
  const [receipts, setReceipts] =
    useState<Record<string, NotesReviewReceipt>>(initialReceipts);
  ...
  const approve = async (item: NotesReviewItem) => {
    ...
    try {
      ...
      setReceipts((current) => ({
        ...current,
        [item.id]: receipt ?? missingReceipt(),
      }));
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : 'Could not approve this item. Nothing was sent.';
  ...
  return (
    <section
      data-testid="notes-review-panel"
      aria-label="Review proposed note items"
      style={{
    ...
    >
      ...
      <div style={{ display: 'grid', gap: 'var(--kp-space-sm)' }}>
        {items.map((item) => {
          ...
          const blockedReason = blockedDestinations[destination];
          return (
            <article
              key={item.id}
              data-testid={`notes-review-item-${item.id}`}
              style={{
            ...
            >
              ...
              >
                <div style={{ minWidth: 0 }}>
                  ...
                  <div
                    data-testid={`notes-review-detail-${item.id}`}
                    style={{
                      marginTop: 4,
                ...
                <Badge variant={completed ? 'success' : 'neutral'} size="sm">
                  {completed ? 'Approved' : 'Proposed'}
                </Badge>
              </div>
              ...
              {!completed && (
                ...
                >
                      ...
                      onChange={(event) => {
                        setDestinations((current) => ({
                          ...
                          [item.id]: event.target
                            .value as NotesReviewDestination,
                        }));
                      }}
                      style={{
                        minHeight: 30,
                  ...
                  {blockedReason && (
                    <span
                      data-testid={`notes-review-blocked-${item.id}`}
                      style={{
                        color: 'var(--color-muted-foreground)',
                        fontSize: 'var(--kp-font-xs)',
                      }}
              ...
              {receipt && (
                <div
                  data-testid={`notes-review-receipt-${item.id}`}
                  role="status"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
              ...
              {error && (
                <div
                  data-testid={`notes-review-error-${item.id}`}
                  role="alert"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
