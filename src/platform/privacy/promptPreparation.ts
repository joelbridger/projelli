...
function createStamp(): PreparationStamp {
  const stamp = {} as PreparationStamp;
  validStamps.add(stamp as object);
  return stamp;
}
...
export interface PreparedCloudRequest {
  ...
  preparationId: PreparationStamp;
...
export function setPreparationEnforcementMode(mode: PreparationEnforcementMode): void { enforcementMode = mode; }
export function getPreparationEnforcementMode(): PreparationEnforcementMode { return enforcementMode; }
...
export function assertCloudPreparation(stamp: unknown, provider: string): void {
  ...
  // This is intentionally a development backstop during the Wave 1 migration.
...
export function prepareCloudRequest(input: { prompt: string; systemPrompt?: string | undefined; parts?: PromptPart[] | undefined; attachmentBytes?: AttachmentBytes[] | undefined }): PreparationResult {
  ...
  return { status: 'ready', request: { prompt, systemPrompt: input.systemPrompt, attachmentBytes: input.attachmentBytes, findings, preparationId: createStamp(), attachmentDisposition: disposition } };
}
...
function redactedRequest(input: { prompt: string; systemPrompt?: string | undefined; attachmentBytes?: AttachmentBytes[] | undefined }, findings: SecretFinding[]): PreparedCloudRequest {
  return { prompt: redactText(input.prompt).text, systemPrompt: input.systemPrompt ? redactText(input.systemPrompt).text : undefined, attachmentBytes: input.attachmentBytes, findings, preparationId: createStamp(), attachmentDisposition: input.attachmentBytes?.length ? 'redacted_derivative' : 'none' };
}
...
function receipt(ctx: PreparedSendContext, decision: 'clean' | 'redacted_by_user' | 'cancelled' | 'blocked', request?: PreparedCloudRequest): void {
  ctx.onAuditLog?.(auditEventToEntry({ type: 'prompt_preparation', timestamp: new Date().toISOString(), payload: { surface: ctx.surface, destination: ctx.providerId, categories: (request?.findings ?? []).map(({ kind, count }) => ({ kind, count })), decision, attachmentDisposition: request?.attachmentDisposition ?? 'blocked' } }));
}
export async function sendPreparedMessageWithEgressAudit(ctx: PreparedSendContext): Promise<ProviderResponse> {
  ...
  return runWithEgressAudit({ ...ctx, operation: () => ctx.provider.sendMessage(chosen.request!.prompt, { ...ctx.options, ...(chosen.request!.systemPrompt ? { systemPrompt: chosen.request!.systemPrompt } : {}), ...(chosen.request!.attachmentBytes ? { attachmentBytes: chosen.request!.attachmentBytes } : {}), preparationStamp: chosen.request!.preparationId }) });
}
export async function sendPreparedStreamingWithEgressAudit(ctx: PreparedSendContext & { options: StreamOptions }): Promise<ProviderResponse> {
  ...
  return runWithEgressAudit({ ...ctx, operation: () => ctx.provider.sendMessageStreaming!(chosen.request!.prompt, { ...ctx.options, ...(chosen.request!.systemPrompt ? { systemPrompt: chosen.request!.systemPrompt } : {}), ...(chosen.request!.attachmentBytes ? { attachmentBytes: chosen.request!.attachmentBytes } : {}), preparationStamp: chosen.request!.preparationId }) });
}
export async function sendPreparedStructuredWithEgressAudit<T>(ctx: PreparedSendContext & { options: StructuredOutputOptions }): Promise<T> {
  ...
  receipt(ctx, chosen.decision, chosen.request); if (!chosen.request) throw new Error(chosen.decision === 'blocked' ? 'prompt_review_required' : 'prompt_send_cancelled');
  return runWithEgressAudit({ ...ctx, operation: () => ctx.provider.structuredOutput<T>(chosen.request!.prompt, { ...ctx.options, ...(chosen.request!.systemPrompt ? { systemPrompt: chosen.request!.systemPrompt } : {}), preparationStamp: chosen.request!.preparationId }) });
}
