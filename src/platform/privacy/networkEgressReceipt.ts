...
export function setNetworkEgressReceiptEmitter(
  next: AuditEmitter | null
): void {
  emitter = next;
}

function directionFor(operation: EgressOperation): NetworkEgressDirection {
  if (operation.category === 'local-ai' || operation.category === 'cloud-ai')
    return 'send';
  ...
  if (operation.category === 'external-client') return 'external-client-export';
  return 'sync';
}
...
function safeFailureCode(error: unknown): string | undefined {
  ...
  if (
    ...
    error.name === 'EgressDestinationNotAllowedError'
  ) {
...
export function buildNetworkEgressReceipt(
  ...
  return {
    ...
    destinationClass: operation.destination.userSelectedHost
      ? 'user-configured-host'
      : operation.id === 'local-loopback'
        ? 'literal-loopback'
        : 'service',
    destination: destination.hostname.toLowerCase(),
    direction: directionFor(operation),
    dataClasses: {
      content: operation.dataClasses.includes('content'),
      metadata: operation.dataClasses.includes('metadata'),
      credential: operation.dataClasses.includes('credential'),
      binaryDownload: operation.dataClasses.includes('binary-download'),
    },
...
export function recordNetworkEgressReceipt(
  receipt: NetworkEgressReceipt
): void {
  emitter?.({
