...
import { intakeOnboardingFolder, intakePdfFormFolder, intakeRequestFolder } from './intakeFiling';
...
describe('request filing identity', () => {
  it('keeps onboarding exactly where it has always been and separates standing folders', () => {
    ...
    expect(intakePdfFormFolder('/workspace/Sarah', 'bank-statements-b2')).toBe('/workspace/Sarah/Requests/bank-statements-b2/forms');
    expect(intakePdfFormFolder('/workspace/Sarah', 'bank-statements-b2')).not.toContain('/Requests/onboarding/');
  });
  ...
  it('uses safe generated names and rejects traversal', () => {
    ...
    expect(() => intakePdfFormFolder('/workspace/Sarah', '../onboarding')).toThrow();
  });
