...
describe('DocuSign broker capability clients', () => {
  it('uses the authenticated opaque capability contract without sending client data', async () => {
    ...
    await expect(createDocusignAuthorizationProvider({ intakeId: 'intake-1', seatToken: 'seat', accessToken: 'access', baseUrl: 'https://relay.test', templateId: 'approved-template' })()).resolves.toMatchObject({ accountId: 'account', baseUri: 'https://demo.docusign.net' });
