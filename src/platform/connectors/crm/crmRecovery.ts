...
function isNetworkLockdownError(error: unknown): boolean {
  return (
    typeof error === 'object' &&
    error !== null &&
    'code' in error &&
...
export async function rebuildConnectedCrmImports(): Promise<void> {
  ...
  for (const provider of connectedProviders) {
    try {
      ...
      // Privacy wins over recovery. Once the guarded doorway closes, stop
      // immediately and let the connector show the honest lockdown message.
      if (isNetworkLockdownError(error)) throw error;
