...
describe('rebuildConnectedCrmImports', () => {
  ...
  it('stops recovery when Network lockdown closes the guarded doorway', async () => {
    const lockdownError = Object.assign(new Error('internal policy detail'), {
      code: 'NETWORK_LOCKDOWN_BLOCKED',
    });
    controls.crmSyncAll.mockRejectedValue(lockdownError);

    await expect(rebuildConnectedCrmImports()).rejects.toBe(lockdownError);
    expect(controls.crmRebuildStore).toHaveBeenCalledTimes(1);
