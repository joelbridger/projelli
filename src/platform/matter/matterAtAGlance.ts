 ...
 * matterAtAGlance.ts
 *
 * AI at-a-glance generator for the Lantern matter hub. Given a matter id,
...
export async function buildResolvedProviderForGlance(): Promise<ResolvedGlanceProvider> {
  ...
  if (resolved) {
    ...
    if (apiKey) {
      ...
      // One front door: build through the shared factory (fix F2.2) so
      // At-a-Glance can never drift from Ask / redline on provider construction.
