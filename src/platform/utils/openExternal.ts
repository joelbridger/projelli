...
export async function openExternal(url: string): Promise<void> {
  ...
  if (isTauri) {
    ...
    try {
      ...
      // Importing the shell bridge can yield. Check only after that yield, at
      // the final possible moment before handing the URL to the operating
      // system, so an Offline Mode change during import wins.
    ...
    // Browser mode has no dynamic-import gap, but the policy check still sits
    // immediately before the navigation act.
    await assertEgressNavigationAllowed('external-navigation', url);
