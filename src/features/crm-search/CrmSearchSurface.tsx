...
function recordText(content: string): string {
  try {
    const record = JSON.parse(content) as Record<string, unknown>;
    const useful = ['name', 'title', 'body', 'text', 'label', 'facts', 'notes', 'accounts', 'members', 'externalParties', 'status', 'priority']
      .filter((key) => record[key] !== undefined)
      .reduce<Record<string, unknown>>((result, key) => ({ ...result, [key]: record[key] }), {});
...
export function CrmSearchSurface() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setSelected(null);
    try {
      setHits(await searchCrmRecords(live.workspaceRoot, query, scope || undefined));
    } catch (reason) {
