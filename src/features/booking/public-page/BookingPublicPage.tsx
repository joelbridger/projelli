...
export function BookingPublicPage({ branding, availability }: BookingPublicPageProps) {
  ...
  return (
    <main data-testid="booking-public-page" style={{ background: 'var(--color-secondary)', color: 'var(--kp-navy)', minHeight: '100%', padding: 24 }}>
      <div style={{ margin: '0 auto', maxWidth: 1080 }}>
        <header data-testid="booking-public-page-brand-header" style={{ alignItems: 'center', display: 'flex', gap: 12, marginBottom: 24 }}>
          <div aria-hidden style={{ alignItems: 'center', background: 'var(--kp-navy)', borderRadius: 10, color: 'var(--color-primary-foreground)', display: 'flex', fontSize: 18, fontWeight: 700, height: 42, justifyContent: 'center', width: 42 }}>
            {branding.firmMark}
          </div>
          <div>
        ...
        <div style={{ display: 'grid', gap: 20, gridTemplateColumns: 'minmax(240px, .78fr) minmax(0, 1.4fr)' }}>
          <aside data-testid="booking-public-page-advisor" style={{ ...cardStyle, padding: 24 }}>
            {branding.advisorPhotoUrl ? (
              ...
              <div aria-hidden style={{ alignItems: 'center', background: 'var(--kp-accent-soft)', borderRadius: '50%', color: 'var(--kp-navy)', display: 'flex', fontWeight: 700, height: 68, justifyContent: 'center', width: 68 }}>
            ...
            <p style={{ color: 'var(--color-muted-foreground)', lineHeight: 1.55, marginTop: 0 }}>{branding.advisorTitle}</p>
            <hr style={{ border: 0, borderTop: '1px solid var(--kp-divider)', margin: '22px 0' }} />
            <strong>{branding.meetingTitle}</strong>
            <p style={{ color: 'var(--color-muted-foreground)', lineHeight: 1.55 }}>{branding.meetingDescription}</p>
          </aside>
        ...
        <footer data-testid="booking-public-page-disclosure" style={{ color: 'var(--color-muted-foreground)', display: 'flex', fontSize: 12, gap: 12, justifyContent: 'space-between', marginTop: 20 }}>
...
function PageNotice({ body, testId, title }: { body: string; testId: string; title: string }) {
  return <div data-testid={testId} role="status" style={{ background: 'var(--color-secondary)', border: '1px solid var(--kp-divider)', borderRadius: 10, color: 'var(--color-muted-foreground)', padding: 18 }}><strong style={{ color: 'var(--kp-navy)', display: 'block', marginBottom: 6 }}>{title}</strong>{body}</div>;
}
...
const dateButtonStyle = (selected: boolean) => ({ background: selected ? 'var(--kp-navy)' : 'var(--color-background)', border: `1px solid ${selected ? 'var(--kp-navy)' : 'var(--color-border)'}`, borderRadius: 9, color: selected ? 'var(--color-primary-foreground)' : 'var(--kp-navy)', cursor: 'pointer', minHeight: 48, padding: '8px 10px' });
const slotButtonStyle = { background: 'var(--color-background)', border: '1px solid var(--kp-divider-strong)', borderRadius: 8, color: 'var(--kp-navy)', cursor: 'pointer', minHeight: 42, padding: '9px 12px' };
const inputStyle = { border: '1px solid var(--color-border)', borderRadius: 8, boxSizing: 'border-box' as const, display: 'block', marginTop: 6, padding: 10, width: '100%' };
