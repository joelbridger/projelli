...
- [x] The new destination appears in the CRM rail under Work as **Internal projects**. The base already has a separate legacy **Projects** item, so the new label avoids a misleading duplicate while the surface itself remains headed **Projects**.
...
at lines 737–742 and the CRM rail placement at lines 470–482. The screenshot
uses a populated internal-only project to make all required detail cues visible.
