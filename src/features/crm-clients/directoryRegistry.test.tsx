...
describe('client directory registries', () => {
  it('keeps frozen compatibility descriptors in stable order', () => {
    ...
    expect(directoryViewRegistry.map(({ id }) => id)).toEqual([
      'directory',
      'book',
    ]);
    const bookContext: DirectoryContext = {
      ...context,
      sort: { ...context.sort, value: 'book' },
    };
    const tabSwitch = directoryToolRegistry.find(
      ({ id }) => id === 'tab-switch'
    );
    const personDetails = directoryRailRegistry.find(
      ({ id }) => id === 'person-details'
    );
    if (!tabSwitch || !personDetails)
      throw new Error('Expected frozen compatibility descriptors');
    render(
      <>
        {tabSwitch.mount(bookContext)}
        {personDetails.mount(bookContext)}
      </>
    );
    expect([
      screen.queryByTestId('crm-directory-tab-households') !== null,
      screen.queryByTestId('crm-directory-person-panel') === null,
    ]).toEqual([true, true]);
  });

  it('mounts one dummy tool, action, rail, and view through their descriptors', () => {
    const tool: DirectoryToolDescriptor = {
      id: 'test-tool',
      order: 1,
      mount: () => <div data-testid="dummy-tool" />,
    };
    const action: DirectoryActionDescriptor = {
      id: 'test-action',
      order: 1,
      mount: () => <div data-testid="dummy-action" />,
    };
    const rail: DirectoryRailDescriptor = {
      id: 'test-rail',
      order: 1,
      mount: () => <div data-testid="dummy-rail" />,
    };
  ...
  it('rejects duplicate IDs and malformed descriptors clearly', () => {
    ...
    if (!tool || !action || !rail || !view)
      throw new Error('Expected compatibility descriptors');
    expect(() => {
      validateDirectoryToolDescriptors([...directoryToolRegistry, tool]);
    }).toThrow('duplicate id: view-switch');
    expect(() => {
      validateDirectoryActionDescriptors([...directoryActionRegistry, action]);
    }).toThrow('duplicate id: create-household');
    expect(() => {
      validateDirectoryRailDescriptors([...directoryRailRegistry, rail]);
    }).toThrow('duplicate id: person-details');
    expect(() => {
      validateDirectoryViewDescriptors([...directoryViewRegistry, view]);
    }).toThrow('duplicate id: directory');
    expect(() => {
      validateDirectoryToolDescriptors([{ ...tool, order: Number.NaN }]);
    }).toThrow('order must be finite: view-switch');
    expect(() => {
      validateDirectoryViewDescriptors([
        { ...view, mount: undefined as never },
      ]);
    }).toThrow('mount must be a function: directory');
  });
