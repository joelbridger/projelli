...
function CreateHouseholdAction({ context }: { context: DirectoryContext }) {
  ...
  return <>
    ...
    {creating ? <form data-testid="crm-create-household" onSubmit={(event) => { event.preventDefault(); const trimmed = name.trim(); if (!trimmed) return; void Promise.resolve(context.repository.createHousehold(trimmed)).then(() => { setName(''); setCreating(false); }).catch(() => { setCreating(true); }); }} style={{ display: 'flex', gap: 8, marginTop: 12 }}><input data-testid="crm-household-name" aria-label="Household name" value={name} onChange={(event) => { setName(event.target.value); }} placeholder="Household name" autoFocus /><Button data-testid="crm-household-save" type="submit">Create household</Button><Button variant="secondary" type="button" onClick={() => { setCreating(false); }}>Cancel</Button></form> : null}
  </>;
}

function DirectoryResults({ context }: { context: DirectoryContext }) {
  const households = useMemo(() => context.records.households.filter((household) => household.name.toLowerCase().includes(context.query.value.toLowerCase())), [context]);
  const people = useMemo(() => context.records.people.filter((person) => (!context.filters.externalOnly || person.external) && (!context.filters.needsVerification || !person.verifiedAt) && person.name.toLowerCase().includes(context.query.value.toLowerCase())), [context]);
  if (context.filters.tab === 'households') return <div style={{ display: 'grid', gap: 8, marginTop: 12 }}>{households.length ? households.map((household) => <Card key={household.id} variant="interactive" role="button" tabIndex={0} data-testid={`crm-directory-household-${household.id}`} onClick={() => { context.repository.openHousehold(household.id); }} onKeyDown={(event) => { if (event.key === 'Enter') context.repository.openHousehold(household.id); }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}><strong>{household.name}</strong><Badge variant="featured">{household.serviceTier}</Badge></div><div style={{ color: 'var(--color-slate-600)', marginTop: 4 }}>{household.lifecycle} · Owned by {household.primaryAdvisor} · {household.peopleCount} people</div></Card>) : <Card variant="raised">No households match this search.</Card>}</div>;
...
export const legacyDirectoryTools: readonly DirectoryToolDescriptor[] = [
  ...
  { id: 'tab-switch', order: 20, mount: (context) => context.sort.value !== 'book' ? <SegmentedToggle ariaLabel="Directory view" value={context.filters.tab} onChange={(value) => { context.filters.setTab(value); }} options={[{ value: 'households', label: 'Households' }, { value: 'people', label: 'People' }]} data-testid="crm-directory-tab" /> : null },
...
export const legacyDirectoryRails: readonly DirectoryRailDescriptor[] = [{ id: 'person-details', order: 10, mount: (context) => <SlidePanel open={context.selection.person !== null} onClose={() => { context.selection.setPerson(null); }} title={context.selection.person?.name} data-testid="crm-directory-person-panel">{context.selection.person ? <div><p><strong>Person roles:</strong> {context.selection.person.roles.join(', ') || 'None'}</p><p><strong>Household relationship:</strong> {context.selection.person.householdRole ?? 'Not a household member'}</p><p><strong>Channel:</strong> {context.selection.person.channel ?? 'No recipient channel'}</p><p><strong>Verification:</strong> {context.selection.person.verifiedAt ? `${context.selection.person.verifiedAt}${context.selection.person.verifiedBy ? ` by ${context.selection.person.verifiedBy}` : ''}` : 'Needs verification'}</p><Button size="sm" data-testid={`crm-review-recipient-${context.selection.person.id}`} onClick={() => { context.repository.reviewRecipient(context.selection.person?.id ?? ''); }}>Review recipient</Button></div> : null}</SlidePanel> }];
export const legacyDirectoryViews: readonly DirectoryViewDescriptor[] = [
  { id: 'directory', order: 10, mount: (context) => context.sort.value !== 'book' ? <DirectoryResults context={context} /> : null },
  { id: 'book', order: 20, mount: (context) => context.sort.value === 'book' ? <BookDirectoryView onOpenClient={(id) => { context.repository.openHousehold(id); }} /> : null },
];
