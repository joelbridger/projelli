...
import { ArrowLeft, ChevronDown, Clock3, Lock, Mail, Plus } from 'lucide-react';
import {
  Badge,
  Button,
  Card,
...
function syncLabel(household: HouseholdRecord, offlineMessage: string): string {
  if (household.syncState === 'last_synced') {
    return household.lastSyncedAt
      ? `Last synced ${household.lastSyncedAt}`
      : 'Last synced';
  }
...
export function HouseholdRecordSurface({
  ...
  const [adding, setAdding] = useState<'person' | 'account' | 'fact' | null>(
    null
  );
  ...
  const recordContext: HouseholdRecordShellContext = {
    ...
    openPanel: (panel) => {
      if (panel === 'metadata') setMetadataOpen(true);
      else setEditingHousehold(true);
    },
    setNoteAudience: (audience) => {
      setNoteAudience(audience);
      setAddOpen(false);
    },
    setAdding: (kind) => {
    ...
    renderLegacyClientMap: () => (
      <>
        ...
        />
        {getHouseholdRecordExtensions().map((extension) => {
          const candidate = household.extensionData?.[extension.dataKey];
          const value =
            candidate !== undefined && extension.validate(candidate)
              ? candidate
              : extension.defaultValue;
          return extension.renderSummary ? (
            <span key={extension.id}>
              {extension.renderSummary({
                household,
                value,
                save: async (next) => {
                  await onSaveHousehold?.({
  ...
  const headerActions = getHouseholdHeaderActions();
  const addActions = getHouseholdAddActions();
  const sections = getHouseholdSections();
    ...
    <section data-testid="crm-household-record">
      <header>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {onBack ? (
              <Button
                ...
                variant="secondary"
                iconLeft={ArrowLeft}
                data-testid="crm-household-back"
                onClick={onBack}
              >
                Directory
              </Button>
            ...
            {headerActions
              ...
              .map((action) => (
                <span key={action.id}>{action.mount(recordContext)}</span>
              ))}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
          ...
          <Badge
            variant={syncVariant[household.syncState]}
            data-testid="crm-household-sync-status"
          >
      ...
      <SegmentedToggle
        ...
        value={tab}
        onChange={setTab}
        options={householdTabRegistry.map(({ route, label }) => ({
          value: route,
          label,
        }))}
      ...
      {(() => {
        const selectedTab =
          householdTabRegistry.find((surface) => surface.route === tab) ??
          householdTabRegistry[0];
        ...
        return createElement(selectedTab.Component, {
          ...
          renderLegacySurface: (id: HouseholdTab) =>
            id === 'client_map'
              ? sections
      ...
      >
        <ContactEditor
          onSave={async (person) => {
            await onSaveHousehold?.({
              ...household,
              members: person.external
                ? household.members
                : [...household.members, person],
              externalParties: person.external
                ? [...household.externalParties, person]
      ...
      >
        {editingPerson ? (
          <ContactEditor
            ...
            onSave={async (person) => {
              const replace = (
                people: readonly import('./adapters').CrmPerson[]
              ) =>
                people.map((item) => (item.id === person.id ? person : item));
              await onSaveHousehold?.({
                ...household,
                members: replace(household.members),
                externalParties: replace(household.externalParties),
              });
              setEditingPerson(null);
            }}
            onRemove={async () => {
              await onSaveHousehold?.({
                ...
                members: household.members.filter(
                  (item) => item.id !== editingPerson.id
                ),
                externalParties: household.externalParties.filter(
                  (item) => item.id !== editingPerson.id
                ),
              });
              setEditingPerson(null);
      ...
      >
        <AccountEditor
          onSave={async (account) => {
            await onSaveHousehold?.({
              ...household,
              accounts: [...household.accounts, account],
            });
            setAdding(null);
          }}
        />
...
function HouseholdEditor({
  ...
  household: HouseholdRecord;
  onSave: (household: HouseholdRecord) => Promise<void> | void;
}) {
  ...
  return (
    <form
      data-testid="crm-household-editor"
      noValidate
      onSubmit={(event) => {
        ...
        const next = {
          ...household,
          name: cleanName,
          lifecycle: lifecycle.trim() || 'Active',
          primaryAdvisor: advisor.trim() || 'Unassigned',
          serviceTier: tier.trim() || 'Standard',
        };
        if (nextReview.trim()) next.nextReview = nextReview.trim();
        else delete next.nextReview;
        void onSave(next);
      }}
    ...
    >
      <label>
        ...
        <input
          data-testid="crm-household-edit-name"
          required
          aria-invalid={Boolean(nameError)}
          aria-describedby={
            nameError ? 'crm-household-edit-name-error' : undefined
          }
          style={nameError ? invalidFieldStyle : undefined}
          value={name}
          onChange={(event) => {
        ...
        {nameError ? (
          <span
            id="crm-household-edit-name-error"
            role="alert"
      ...
      <label>
        Lifecycle
        <input
          data-testid="crm-household-edit-lifecycle"
          value={lifecycle}
          onChange={(event) => {
      ...
      <label>
        Primary advisor
        <input
          data-testid="crm-household-edit-advisor"
          value={advisor}
          onChange={(event) => {
      ...
      <label>
        Service tier
        <input
          data-testid="crm-household-edit-tier"
          value={tier}
          onChange={(event) => {
      ...
      <label>
        Next review
        <input
          data-testid="crm-household-edit-review"
          value={nextReview}
          onChange={(event) => {
      ...
      <Button data-testid="crm-household-edit-save" type="submit">
        Save household
      </Button>
    </form>
  );
...
function FactEditor({
  ...
  onSave: (fact: import('./adapters').CrmFact) => Promise<void> | void;
}) {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<'label' | 'value' | 'asOf' | 'source', string>>
  >({});
  ...
  return (
    <form
      data-testid="crm-fact-editor"
      noValidate
      onSubmit={(event) => {
        event.preventDefault();
        const nextErrors: typeof errors = {};
        if (!label.trim())
          nextErrors.label = 'What the firm should remember is required.';
    ...
    >
      <label>
        What should the firm remember? (required)
        <input
          data-testid="crm-fact-label"
          required
          aria-invalid={Boolean(errors.label)}
          aria-describedby={errors.label ? 'crm-fact-label-error' : undefined}
          style={errors.label ? invalidFieldStyle : undefined}
          value={label}
          onChange={(event) => {
      ...
      <label>
        Value (required)
        <input
          data-testid="crm-fact-value"
          required
          aria-invalid={Boolean(errors.value)}
          aria-describedby={errors.value ? 'crm-fact-value-error' : undefined}
          style={errors.value ? invalidFieldStyle : undefined}
          value={value}
          onChange={(event) => {
      ...
      <label>
        True as of (required)
        <input
          data-testid="crm-fact-as-of"
          required
          aria-invalid={Boolean(errors.asOf)}
          aria-describedby={errors.asOf ? 'crm-fact-as-of-error' : undefined}
          style={errors.asOf ? invalidFieldStyle : undefined}
          type="date"
          value={asOf}
          onChange={(event) => {
      ...
      <label>
        Source (required)
        <input
          data-testid="crm-fact-source"
          required
          aria-invalid={Boolean(errors.source)}
          aria-describedby={errors.source ? 'crm-fact-source-error' : undefined}
          style={errors.source ? invalidFieldStyle : undefined}
          value={source}
          onChange={(event) => {
            setSource(event.target.value);
            clearError('source');
          }}
          placeholder="Tax return, meeting, or advisor"
        />
        {errors.source ? (
          <span
            id="crm-fact-source-error"
            role="alert"
            style={validationErrorStyle}
          >
            {errors.source}
      ...
      <label>
        Open this source (optional)
        <input
          data-testid="crm-fact-source-ref"
          value={sourceRef}
          onChange={(event) => {
            setSourceRef(event.target.value);
          }}
          placeholder="Workspace file, mail link, or web link"
        />
      </label>
      <Button data-testid="crm-fact-save" type="submit">
        Save fact
      </Button>
    </form>
  );
...
function AccountEditor({
  ...
  onSave: (account: import('./adapters').CrmAccount) => Promise<void> | void;
}) {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<'custodian' | 'type' | 'purpose', string>>
  >({});
  const clearError = (field: keyof typeof errors) => {
    setErrors((current) =>
      current[field] ? { ...current, [field]: undefined } : current
  ...
  return (
    <form
      data-testid="crm-account-editor"
      noValidate
      onSubmit={(event) => {
        event.preventDefault();
        const nextErrors: typeof errors = {};
        ...
        void onSave({
          ...
          ...(lastFour.trim() ? { lastFour: lastFour.trim().slice(-4) } : {}),
          status: 'Open',
        });
      }}
      style={{ display: 'grid', gap: 10 }}
    >
      <label>
        Custodian (required)
        <input
          data-testid="crm-account-custodian"
          required
          aria-invalid={Boolean(errors.custodian)}
          aria-describedby={
            errors.custodian ? 'crm-account-custodian-error' : undefined
          }
          style={errors.custodian ? invalidFieldStyle : undefined}
          value={custodian}
          onChange={(event) => {
        ...
        {errors.custodian ? (
          <span
            id="crm-account-custodian-error"
            role="alert"
            style={validationErrorStyle}
          >
            {errors.custodian}
      ...
      <label>
        ...
        <input
          ...
          aria-describedby={errors.type ? 'crm-account-type-error' : undefined}
          style={errors.type ? invalidFieldStyle : undefined}
          value={type}
          onChange={(event) => {
      ...
      <label>
        Purpose (required)
        <input
          data-testid="crm-account-purpose"
          required
          aria-invalid={Boolean(errors.purpose)}
          aria-describedby={
            errors.purpose ? 'crm-account-purpose-error' : undefined
          }
          style={errors.purpose ? invalidFieldStyle : undefined}
          value={purpose}
          onChange={(event) => {
      ...
      <label>
        Last four only (optional)
        <input
          data-testid="crm-account-last-four"
          inputMode="numeric"
          value={lastFour}
          onChange={(event) => {
      ...
      <Button data-testid="crm-account-save" type="submit">
        Save account
      </Button>
    </form>
  );
...
function ClientMap({
  ...
  household: HouseholdRecord;
  onEditPerson: (person: import('./adapters').CrmPerson) => void;
  onDeleteFact: (id: string) => Promise<void> | void;
}) {
  return (
    <div style={{ display: 'grid', gap: 12, marginTop: 14 }}>
          ...
          household.facts.map((fact) => (
            ...
            >
                ...
                {fact.sources.map((source) =>
                  source.ref ? (
                    <button
                      key={source.id}
                      type="button"
                      data-testid={`crm-fact-source-${source.id}`}
                    ...
                    <CiteChip
                      key={source.id}
                      docLabel={source.label}
                      quote={source.asOf ?? 'Source recorded'}
                    >
                      {source.label}
                    </CiteChip>
                  )
              ...
              <Button
                size="sm"
                variant="secondary"
                data-testid={`crm-fact-remove-${fact.id}`}
                onClick={() => {
                  void onDeleteFact(fact.id);
                }}
              >
                Remove fact
              </Button>
      ...
      <Card variant="raised" data-testid="crm-household-accounts">
        ...
        {household.accounts.map((account) => (
          <div
            key={account.id}
            data-testid={`crm-household-account-${account.id}`}
          >
            <strong>
      ...
      <Card variant="raised" data-testid="crm-household-people">
        ...
        <p>
          ...
          {household.members.length
            ? household.members.map((person) => (
                <span
                  key={person.id}
                  data-testid={`crm-household-person-${person.id}`}
                  style={{ display: 'block' }}
                >
                  <button
                    type="button"
                    data-testid={`crm-person-edit-${person.id}`}
                    onClick={() => {
                      onEditPerson(person);
                    }}
                  >
                  ...
                  {person.householdRole ? (
                    <span
                      data-testid={`crm-person-household-role-${person.id}`}
                    >
                      {' '}
                      ({person.householdRole})
                    </span>
                  ...
                  {person.roles.length ? (
                    <span data-testid={`crm-person-roles-${person.id}`}>
                      {' '}
                      · {person.roles.join(', ')}
                    </span>
                  ) : null}
                  {person.emails?.length ? (
                    <span data-testid={`crm-person-email-${person.id}`}>
                      {' '}
                      ·{' '}
                      {person.emails.find((email) => email.primary)?.address ??
                        person.emails[0]?.address}
                    </span>
                  ) : null}
                  {person.phones?.length ? (
                    <span data-testid={`crm-person-phone-${person.id}`}>
                      {' '}
                      ·{' '}
                      {person.phones.find((phone) => phone.primary)?.address ??
                        person.phones[0]?.address}
                    </span>
                  ) : null}
                  {person.addresses?.length ? (
                    <span
                      data-testid={`crm-person-address-${person.id}`}
                      style={{
                        display: 'block',
                        color: 'var(--color-slate-600)',
                      }}
                    >
                      {person.addresses.find((address) => address.primary)
                        ?.address ?? person.addresses[0]?.address}
                    </span>
                  ) : null}
                </span>
              ))
        ...
        <p>
          ...
          {household.externalParties.length
            ? household.externalParties.map((person) => (
                <span key={person.id} style={{ display: 'block' }}>
                  <button
                    type="button"
                    data-testid={`crm-person-edit-${person.id}`}
                    onClick={() => {
                  ...
                  {` · ${person.roles.join(', ') || 'No role'} · ${person.verifiedAt ? 'Recipient verified' : 'Needs verification'}`}
                </span>
              ))
            : 'None'}
        </p>
      ...
      <Card variant="raised" data-testid="crm-household-metadata-summary">
        ...
        {household.tags?.length ? (
          <div
            style={{
              ...
              gap: 6,
              flexWrap: 'wrap',
              marginBottom: household.customFields?.length ? 12 : 0,
            }}
          >
            {household.tags.map((tag) => (
              <Badge
                key={tag}
                variant="featured"
                data-testid={`crm-household-tag-${tag}`}
              >
                {tag}
              </Badge>
            ))}
        ...
        {household.customFields?.length ? (
          ...
          >
            {household.customFields.map((field) => (
              <div
                key={field.id}
                data-testid={`crm-household-field-${field.id}`}
                style={{ display: 'contents' }}
              >
                <dt style={{ color: 'var(--color-slate-600)' }}>
                  {field.label}
                </dt>
                <dd style={{ margin: 0 }}>{field.value || 'Not set'}</dd>
              </div>
            ))}
          </dl>
        ) : null}
        {!household.tags?.length && !household.customFields?.length ? (
          <p>
            Use Fields and tags to add details that belong only to this
            household.
          </p>
        ) : null}
      </Card>
            ...
            .map((note) => (
              <p key={note.id} data-testid={`crm-note-${note.id}`}>
                {note.pinned ? (
                  <Badge
                    variant="featured"
                    data-testid={`crm-note-pinned-${note.id}`}
                  >
                    Pinned
                  </Badge>
                ) : null}{' '}
      ...
      <Card variant="raised">
        ...
        {tab === 'email' ? (
          ...
          <span>
            ...
            {isActivity
              ? t('crm.household.activity.empty-title')
              : 'No history yet.'}
          </span>
        )}
      </Card>
