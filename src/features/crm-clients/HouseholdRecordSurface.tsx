...
export function HouseholdRecordSurface({
  ...
  return (
    <section data-testid="crm-household-record">
      ...
      />
      {tab === 'client_map' ? (
        <ClientMap household={household} onEditPerson={setEditingPerson} onDeleteFact={async (id) => { await onSaveHousehold?.({ ...household, facts: household.facts.filter((fact) => fact.id !== id) }); }} />
      ) : (
        <ExistingSurface
          tab={tab}
          household={household}
