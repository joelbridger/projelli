...
import { buildCapacityTriage, dailyWorkReason, nextRecurringDue, type DailyWorkItem } from '@/platform/crm/tasks';
...
function Today({ workItems, firmMembers, approvals, activity, freshness, onNavigate, onCompleteWorkItem, onDecideApproval }: { workItems: readonly CrmDailyWorkItem[]; firmMembers: readonly CrmFirmMember[]; approvals: readonly CrmApproval[]; activity: readonly CrmActivity[]; freshness: CrmFreshnessState; onNavigate: (r: CrmHomeRoute) => void; onCompleteWorkItem: (item: CrmDailyWorkItem) => void | Promise<void>; onDecideApproval: (approval: CrmApproval, decision: 'approved' | 'rejected') => void | Promise<void> }) {
  ...
  return <Screen title="Today" description="Your morning plan" Icon={LayoutDashboard} action={<Button data-testid="crm-today-review" iconLeft={ClipboardList} onClick={() => { setReviewing(true); }}>Review today’s plan</Button>}>
    ...
    {open.length === 0 ? <section data-testid="crm-today-first-use" style={panelStyle}><strong>Your firm is ready to set up Today.</strong><p style={mutedStyle}>Add tasks or finish your import, and the work that needs attention first will appear here.</p><div style={{ display: 'flex', gap: 8 }}><Button data-testid="crm-today-new-task" onClick={() => { onNavigate('tasks'); }}>New task</Button><Button variant="secondary" data-testid="crm-today-open-migration" onClick={() => { onNavigate('migration'); }}>Open migration</Button></div></section> : <section data-testid="crm-today-triage" style={panelStyle}><strong>What needs attention first</strong><p style={mutedStyle}>Ordered from your saved tasks and workflow steps. Each item explains why it is here.</p>{open.map((item) => <div key={item.id} data-testid={`crm-today-task-${item.id}`} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, padding: '10px 0', borderTop: '1px solid var(--kp-border)' }}><span><strong>{item.title}</strong><span style={mutedStyle}> · {dailyWorkReason(item, today)} · {workLabel(item)}{workHousehold(item) ? ` · ${workHousehold(item)}` : ''}</span></span><Button size="sm" variant="secondary" data-testid={`crm-today-complete-${item.id}`} onClick={() => { void onCompleteWorkItem(item); }}>Complete</Button></div>)}<Button variant="secondary" data-testid="crm-today-review-inline" onClick={() => { setReviewing(true); }}>Review this order</Button></section>}
    {reviewing && <section data-testid="crm-today-review-panel" style={{ ...panelStyle, borderColor: 'var(--kp-assured)' }}><strong>How Today is ordered</strong><p style={mutedStyle}>Work with a date comes first, with overdue work first. Then Lantern puts blocked work and high-priority work ahead of other open items. It only uses saved CRM records and never moves work by itself.</p>{open.length === 0 ? <p style={mutedStyle}>Add a task or finish your import to start a plan.</p> : open.map((item) => <div key={item.id} style={{ display: 'flex', gap: 8, justifyContent: 'space-between', padding: '8px 0', borderTop: '1px solid var(--kp-border)' }}><span>{item.title}<span style={mutedStyle}> · {dailyWorkReason(item, today)}</span></span><Button size="sm" variant="secondary" data-testid={`crm-today-keep-${item.id}`} onClick={() => undefined}>Keep in Today</Button></div>)}</section>}
...
function Tasks({ tasks, workflowWorkItems, firmMembers, households, savedViews, freshness, onUpdateTask, onCompleteWorkflowWorkItem, onSaveView }: { tasks: readonly CrmTask[]; workflowWorkItems: readonly CrmWorkflowWorkItem[]; firmMembers: readonly CrmFirmMember[]; households: readonly { id: string; name: string }[]; savedViews: readonly CrmTaskSavedView[]; freshness: CrmFreshnessState; onUpdateTask: (task: CrmTask) => void | Promise<void>; onCompleteWorkflowWorkItem: (item: CrmWorkflowWorkItem) => void | Promise<void>; onSaveView: (view: CrmTaskSavedView) => void | Promise<void> }) {
  ...
  return <Screen title="Tasks" description="One work list for tasks and workflow steps" Icon={ListChecks} action={<Button data-testid="crm-task-new" iconLeft={Plus} onClick={newTask}>New task</Button>}>
    ...
    {tasks.length === 0 && workflowWorkItems.length === 0 && <section data-testid="crm-tasks-first-use" style={panelStyle}><strong>No work yet.</strong><p style={mutedStyle}>Create a task or start a workflow. A task can be for one client or the whole firm.</p></section>}
    <section data-testid="crm-task-triage-count" style={panelStyle}><strong>{plan.hasCapacitySignal ? `${String(plan.fitsToday.length)} of ${String(plan.ranked.length)} open items fit the first daily plan.` : 'Add active firm members to make a capacity-based daily plan.'}</strong></section><FreshnessBanner freshness={freshness} />
    {view === 'list' ? <div data-testid="crm-task-list" style={panelStyle}>{workItems.length === 0 ? <p>{tasks.length === 0 && workflowWorkItems.length === 0 ? 'No work yet.' : 'No work matches these filters.'}</p> : <>{filtered.map((task) => <TaskRow key={task.id} task={task} onComplete={() => { advance(task); }} onOpen={() => { setEditing(task); }} />)}{filteredWorkflowSteps.map((item) => <WorkflowWorkRow key={item.id} item={item} onComplete={() => { void onCompleteWorkflowWorkItem(item); }} />)}</>}</div> : <TaskBoard tasks={filtered} onOpen={setEditing} onMove={(task, status) => { void onUpdateTask({ ...task, status }); }} />}
    {editing && <TaskDetail task={editing} households={households} firmMembers={firmMembers} onClose={() => { setEditing(null); }} onSave={onUpdateTask} />}
  </Screen>;
}
...
function WorkflowWorkRow({ item, onComplete }: { item: CrmWorkflowWorkItem; onComplete: () => void }) {
  return <div data-testid={`crm-workflow-work-${item.id}`} style={{ display: 'flex', gap: 10, alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--kp-border)' }}><button data-testid={`crm-workflow-work-complete-${item.id}`} aria-label={`Complete ${item.title}`} onClick={onComplete}>□</button><span style={{ flex: 1 }}><strong>{item.title}</strong><span style={mutedStyle}> · Workflow step · {item.householdLabel} · {item.assigneeLabel ?? item.assigneeUserId ?? 'Unassigned'}</span></span></div>;
}
...
function TaskDetail({ task, households, firmMembers, onClose, onSave }: { task: CrmTask; households: readonly { id: string; name: string }[]; firmMembers: readonly CrmFirmMember[]; onClose: () => void; onSave: (task: CrmTask) => void | Promise<void> }) {
...
function PropagationReview({ offers, freshness, onApply, onUndo, undoReport }: { offers: readonly PropagationOffer[]; freshness: CrmFreshnessState; onApply: (offers: readonly PropagationApplyOffer[]) => void; onUndo: () => void; undoReport: string | null }) {
  ...
  const apply = () => {
    if (unresolved.length) { setOutcome('Choose keep or apply for every change before continuing. Nothing was changed.'); return; }
    ...
    onApply(payload);
    setOutcome(`${String(payload.flatMap((offer) => offer.acceptedDecisions).length)} workflow change${payload.flatMap((offer) => offer.acceptedDecisions).length === 1 ? '' : 's'} are ready to apply. Completed work and notes will not change.`);
  };
  const reportUndo = () => { onUndo(); };
  return <Screen title="Workflow update review" description="Choose how a template update should affect open household work" Icon={GitPullRequest} action={<Button data-testid="crm-propagation-approve-all" disabled={freshness.kind === 'offline' || freshness.kind === 'syncing' || freshness.kind === 'last-synced'} onClick={approveAll}>Apply all clear changes</Button>}>
    <FreshnessBanner freshness={freshness} />
    {(freshness.kind === 'offline' || freshness.kind === 'last-synced') && <div role="alert" style={{ ...panelStyle, borderColor: 'var(--kp-direct)' }}>{freshness.kind === 'offline' ? 'Reconnect before changing open household workflows. You can still read this review.' : 'Lantern is checking for the latest workflow updates before it can apply all of them.'}</div>}
    {unresolved.length > 0 && <div data-testid="crm-propagation-review-required" role="alert" style={{ ...panelStyle, borderColor: 'var(--kp-danger)' }}>Some changes need your choice. Compare each option, then choose Apply or Keep current.</div>}
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><span style={mutedStyle}>{draft.length} household{draft.length === 1 ? '' : 's'} to review</span><span style={mutedStyle}>{eligible.length} ready</span><span style={mutedStyle}>{unresolved.length} need your choice</span></div>
    {draft.map((offer) => <PropagationOfferCard key={offer.id} offer={offer} onDecision={setDecision} />)}
    <footer style={{ ...panelStyle, position: 'sticky', bottom: 0, display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }}><span>{decisions(draft).filter((decision) => decision.decision === 'accepted').length} change{decisions(draft).filter((decision) => decision.decision === 'accepted').length === 1 ? '' : 's'} ready across {draft.length} household{draft.length === 1 ? '' : 's'}</span><div style={{ display: 'flex', gap: 8 }}><Button variant="secondary" data-testid="crm-propagation-undo" onClick={reportUndo}>Undo last update</Button><Button data-testid="crm-propagation-apply" disabled={freshness.kind !== 'live' || unresolved.length > 0} onClick={apply}>Apply changes</Button></div></footer>{outcome && <p data-testid="crm-propagation-result" role="status">{outcome}</p>}{undoReport && <p data-testid="crm-propagation-undo-report" role="status">{undoReport}</p>}
  </Screen>;
}
...
function PropagationOfferCard({ offer, onDecision }: { offer: PropagationOffer; onDecision: (offerId: string, decisionId: string, decision: OfferDecision['decision']) => void }) {
  const changeCount = offer.steps.reduce((count, step) => count + step.decisions.length, 0);
  return <section data-testid={`crm-propagation-offer-${offer.id}`} style={panelStyle}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}><strong>{offer.householdLabel}: Template updated</strong><span style={mutedStyle}>{changeCount} change{changeCount === 1 ? '' : 's'} to review</span></div><p style={mutedStyle}>Choose what should change next. Work already done and its notes stay as they are.</p>{offer.steps.map((step) => <section key={step.id} data-testid={`crm-propagation-step-${step.id}`} style={{ borderTop: '1px solid var(--kp-border)', marginTop: 10, paddingTop: 10 }}><strong>{step.label}</strong>{step.decisions.map((decision) => <PropagationFieldRow key={decision.id} decision={decision} onDecision={(choice) => { onDecision(offer.id, decision.id, choice); }} />)}{step.newAssignmentOffer && <p style={mutedStyle}>New work after this update will go to {step.newAssignmentOffer.assigneeLabel}. This does not change work already assigned.</p>}<p style={mutedStyle}>{step.protectedProgress.status === 'completed' ? 'This step is complete. Its work and notes will not change.' : 'Any work already recorded on this step will stay as it is.'}</p></section>)}<details style={{ marginTop: 10 }}><summary>Details for support</summary><p style={mutedStyle}>Update: {offer.revisionLabel}. Record: {offer.id}. Step records: {offer.steps.map((step) => step.id).join(', ')}.</p></details></section>;
}

function PropagationFieldRow({ decision, onDecision }: { decision: OfferDecision; onDecision: (decision: OfferDecision['decision']) => void }) {
  const sentence = decision.field === 'due_offset' ? `Move due date from ${decision.before ?? 'the current date'} to ${decision.after}` : decision.field === 'default_assignee_role' ? `Send new work from ${decision.before ?? 'the current person'} to ${decision.after}` : decision.field === 'title' ? `Rename this step${decision.before ? ` from ${decision.before}` : ''} to ${decision.after}` : decision.field === 'required' ? `${decision.after === 'true' ? 'Make this step required' : 'Make this step optional'}` : decision.field === 'description' ? 'Update the step instructions' : decision.field === 'order' ? `Move this step to ${decision.after}` : `${decision.label}: ${decision.before ? `${decision.before} to ` : ''}${decision.after}`;
  return <div style={{ display: 'flex', gap: 8, padding: '8px 0', alignItems: 'start' }}><span><strong>{sentence}</strong><br /><span style={mutedStyle}>This is a future-work change. Work already done will stay as it is.</span></span><div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}><Button size="sm" variant={decision.decision === 'accepted' ? 'primary' : 'secondary'} data-testid={`crm-propagation-accept-${decision.id}`} onClick={() => { onDecision('accepted'); }}>Apply</Button><Button size="sm" variant={decision.decision === 'rejected' ? 'primary' : 'secondary'} data-testid={`crm-propagation-reject-${decision.id}`} onClick={() => { onDecision('rejected'); }}>Keep current</Button>{decision.decision === 'review_required' && <span data-testid={`crm-propagation-unresolved-${decision.id}`} style={mutedStyle}>Choose one</span>}</div></div>;
}
...
function LiveWorkflows({ data, households, onSave, onNavigate }: { data: LiveWorkflowData; households: readonly HouseholdChoice[]; onSave: (record: LiveCrmRecord) => Promise<unknown>; onNavigate: (route: CrmHomeRoute) => void }) {
  ...
  const [schedule, setSchedule] = useState<WorkflowScheduleDraft>({ frequency: 'annual', timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC', startsAt: new Date().toISOString().slice(0, 10), householdIds: [], enabled: false });
  const [outcomes, setOutcomes] = useState<Record<string, WorkflowStepOutcomeDraft[]>>({});
  ...
  const chooseStarter = (starter: typeof STARTER_WORKFLOWS[number]) => { setName(starter.name); setTitles([...starter.steps]); setCreating(true); };
  const create = async () => { await save(createTemplate(name, titles)); setCreating(false); };
  const start = async () => {
    if (!template) return;
  ...
  const publish = async () => {
    ...
    try {
      ...
      await onSave(updateWorkflowTemplate(update.template, { schedule, outcomes }));
  ...
  return <Screen title="Workflows" description="Steps your firm follows for each household" Icon={Workflow} action={<Button data-testid="crm-live-workflow-new-template" iconLeft={Plus} onClick={() => { setCreating(true); }}>New template</Button>}>
    ...
    {!template && !creating && <><section style={panelStyle}><strong>No workflow templates yet</strong><p style={mutedStyle}>Create your own set of steps, or start from a common firm workflow. Nothing is added until you choose one.</p><Button data-testid="crm-live-workflow-create-first" onClick={() => { setCreating(true); }}>Create a workflow</Button></section><section data-testid="crm-starter-workflow-library" style={panelStyle}><strong>Starter workflow library</strong><p style={mutedStyle}>These are editable starting points, not records in your firm.</p>{STARTER_WORKFLOWS.map((starter) => <div key={starter.id} style={{ borderTop: '1px solid var(--kp-border)', marginTop: 8, paddingTop: 8 }}><strong>{starter.name}</strong><p style={mutedStyle}>{starter.description}</p><Button size="sm" variant="secondary" data-testid={`crm-starter-workflow-${starter.id}`} onClick={() => { chooseStarter(starter); }}>Use this template</Button></div>)}</section></>}
    {creating && <section data-testid="crm-live-workflow-template-form" style={panelStyle}><strong>Create a workflow template</strong><p style={mutedStyle}>Start with the steps your firm follows. You can adjust them before publishing later.</p><label>Template name<input data-testid="crm-live-workflow-name" value={name} onChange={(event) => { setName(event.target.value); }} /></label>{titles.map((title, index) => <label key={index} style={{ display: 'block', marginTop: 8 }}>Step {String(index + 1)}<input data-testid={`crm-live-workflow-step-title-${String(index + 1)}`} value={title} onChange={(event) => { setTitles((current) => current.map((item, position) => position === index ? event.target.value : item)); }} /></label>)}<div style={{ display: 'flex', gap: 8, marginTop: 12 }}><Button data-testid="crm-live-workflow-create-template" onClick={() => { void create(); }}>Save template</Button><Button variant="secondary" onClick={() => { setCreating(false); }}>Cancel</Button></div></section>}
    {template && <><section data-testid="crm-live-workflow-template" style={panelStyle}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}><div><strong>{template.name}</strong><p style={mutedStyle}>{instances.filter((item) => item.status !== 'completed').length} open household workflow{instances.length === 1 ? '' : 's'}</p></div><span style={{ display: 'flex', gap: 8 }}><Button variant="secondary" data-testid="crm-live-workflow-open-propagation" onClick={() => { onNavigate('propagation'); }}>Review updates</Button><Button variant="secondary" data-testid="crm-live-workflow-edit-template" onClick={() => { setChangedTitle((template.steps[1] ?? template.steps[0])?.title ?? ''); setSchedule(template.schedule ?? schedule); setOutcomes(Object.fromEntries(template.steps.map((step) => [step.id, step.outcomes]))); setEditing(true); }}>Edit template</Button></span></div><ol>{template.steps.map((step) => <li key={step.id}>{step.title} · {step.role} · {step.dueOffset === 0 ? 'start day' : `day ${String(step.dueOffset + 1)}`} · {step.required ? 'required' : 'optional'}{step.outcomes.length ? ` · ${String(step.outcomes.length)} outcome${step.outcomes.length === 1 ? '' : 's'}` : ''}</li>)}</ol>{editing && <div data-testid="crm-live-workflow-update-form" style={{ ...panelStyle, background: 'var(--color-background)' }}><strong>Update this template</strong><p style={mutedStyle}>Template step changes ask before they touch open household work. Schedule and outcome choices guide new work from this point on.</p><label>Rename “{(template.steps[1] ?? template.steps[0])?.title}”<input data-testid="crm-live-workflow-change-title" value={changedTitle} onChange={(event) => { setChangedTitle(event.target.value); }} /></label><label style={{ display: 'block', marginTop: 8 }}>Add a step<input data-testid="crm-live-workflow-add-title" value={addedTitle} onChange={(event) => { setAddedTitle(event.target.value); }} /></label><section style={{ borderTop: '1px solid var(--kp-border)', marginTop: 12, paddingTop: 12 }}><strong>Scheduled workflow</strong><p style={mutedStyle}>When this time arrives, this app creates a workflow for the selected households. It never sends anything outside Lantern.</p><label><input data-testid="crm-live-workflow-schedule-enabled" type="checkbox" checked={schedule.enabled} onChange={(event) => { setSchedule((current) => ({ ...current, enabled: event.target.checked })); }} /> Start on a schedule</label><label style={{ display: 'block', marginTop: 8 }}>Repeat<select data-testid="crm-live-workflow-schedule-frequency" value={schedule.frequency} onChange={(event) => { setSchedule((current) => ({ ...current, frequency: event.target.value as WorkflowScheduleDraft['frequency'] })); }}>{['daily', 'weekly', 'monthly', 'quarterly', 'annual'].map((frequency) => <option key={frequency} value={frequency}>{frequency}</option>)}</select></label><label style={{ display: 'block', marginTop: 8 }}>First run<input data-testid="crm-live-workflow-schedule-starts-at" type="date" value={schedule.startsAt.slice(0, 10)} onChange={(event) => { setSchedule((current) => ({ ...current, startsAt: event.target.value })); }} /></label><label style={{ display: 'block', marginTop: 8 }}>Households<select data-testid="crm-live-workflow-schedule-households" multiple value={schedule.householdIds} onChange={(event) => { setSchedule((current) => ({ ...current, householdIds: Array.from(event.currentTarget.selectedOptions, (option) => option.value) })); }}>{households.map((household) => <option key={household.id} value={household.id}>{household.label}</option>)}</select></label></section><section style={{ borderTop: '1px solid var(--kp-border)', marginTop: 12, paddingTop: 12 }}><strong>Step outcomes and branching</strong><p style={mutedStyle}>A completed step can move work to another step, restart work at a step, or finish this workflow.</p>{template.steps.map((step) => <div key={step.id} style={{ marginTop: 8 }}><strong>{step.title}</strong>{(outcomes[step.id] ?? []).map((outcome) => <div key={outcome.id} style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}><input data-testid={`crm-live-workflow-outcome-label-${outcome.id}`} aria-label={`Outcome label for ${step.title}`} value={outcome.label} placeholder="Outcome, such as Approved" onChange={(event) => { editOutcome(step.id, outcome.id, { label: event.target.value }); }} /><select data-testid={`crm-live-workflow-outcome-action-${outcome.id}`} value={outcome.restartAtStepId ? `restart:${outcome.restartAtStepId}` : outcome.nextStepId ? `next:${outcome.nextStepId}` : 'complete'} onChange={(event) => { const [kind, target] = event.target.value.split(':'); editOutcome(step.id, outcome.id, kind === 'next' ? { nextStepId: target, restartAtStepId: undefined } : kind === 'restart' ? { restartAtStepId: target, nextStepId: undefined } : { nextStepId: undefined, restartAtStepId: undefined }); }}><option value="complete">Complete workflow</option>{template.steps.filter((candidate) => candidate.id !== step.id).map((candidate) => <option key={`next-${candidate.id}`} value={`next:${candidate.id}`}>Go to {candidate.title}</option>)}{template.steps.map((candidate) => <option key={`restart-${candidate.id}`} value={`restart:${candidate.id}`}>Restart at {candidate.title}</option>)}</select></div>)}<Button size="sm" variant="secondary" data-testid={`crm-live-workflow-add-outcome-${step.id}`} style={{ marginTop: 6 }} onClick={() => { addOutcome(step.id); }}>Add outcome</Button></div>)}</section><div style={{ display: 'flex', gap: 8, marginTop: 10 }}><Button data-testid="crm-live-workflow-publish" onClick={() => { void publish(); }}>Publish step update</Button><Button variant="secondary" data-testid="crm-live-workflow-save-settings" onClick={() => { void saveSettings(); }}>Save schedule and outcomes</Button></div></div>}</section>
      ...
      <section data-testid="crm-live-workflow-instances" style={panelStyle}><strong>Household workflows</strong>{instances.length === 0 ? <p style={mutedStyle}>Start this workflow for a household to see its steps here.</p> : instances.map((instance) => <LiveInstanceCard key={instance.id} instance={instance} onSave={save} />)}</section><section data-testid="crm-live-meeting-proposal" style={panelStyle}><strong>Propose a workflow from a meeting</strong><p style={mutedStyle}>Meeting content can suggest a workflow. It always waits for a person to approve before it starts.</p>{data.meetings.length === 0 ? <p style={mutedStyle}>No meeting activity is available yet.</p> : <><label>Meeting<select data-testid="crm-live-meeting-select" value={meetingId} onChange={(event) => { setMeetingId(event.target.value); }}><option value="">Choose a meeting</option>{data.meetings.map((meeting) => <option key={meeting.id} value={meeting.id}>{typeof meeting['summary'] === 'string' ? meeting['summary'] : 'Untitled meeting'}</option>)}</select></label><label style={{ display: 'block', marginTop: 8 }}>Household<select data-testid="crm-live-meeting-household" value={meetingHouseholdId} onChange={(event) => { setMeetingHouseholdId(event.target.value); }}><option value="">Choose a household</option>{households.map((household) => <option key={household.id} value={household.id}>{household.label}</option>)}</select></label><Button data-testid="crm-live-meeting-propose-workflow" style={{ marginTop: 8 }} disabled={!meetingId || !meetingHouseholdId} onClick={() => { const meeting = data.meetings.find((item) => item.id === meetingId); const household = households.find((item) => item.id === meetingHouseholdId); if (meeting && household) void save(createMeetingWorkflowProposal(meeting, template, household)); }}>Create proposal for approval</Button></>}</section></>}
  </Screen>;
}
...
function LiveInstanceCard({ instance, onSave }: { instance: LiveWorkflowInstance; onSave: (record: LiveCrmRecord) => Promise<unknown> }) {
  ...
  return <section data-testid={`crm-live-workflow-instance-${instance.id}`} style={{ borderTop: '1px solid var(--kp-border)', marginTop: 10, paddingTop: 10 }}><strong>{instance.householdLabel}</strong><span style={mutedStyle}> · {instance.status === 'completed' ? 'workflow complete' : `${String(steps.filter((step) => step.status === 'done').length)} of ${String(steps.length)} complete`}</span>{steps.map((step) => { const choices = instance.outcomesByStep?.[step.stepId] ?? []; return <div key={step.stepId} data-testid={`crm-live-workflow-instance-step-${step.stepId}`} style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', padding: '7px 0' }}><span>{step.status === 'done' ? '✓' : '□'}</span><strong>{liveStepTitle(instance, step.stepId)}</strong>{step.status === 'done' ? <span style={mutedStyle}>{step.outcome ? `Completed: ${step.outcome}. ` : ''}Completed work stays as it is.</span> : <><>{choices.length > 0 && <select data-testid={`crm-live-workflow-outcome-choice-${instance.id}-${step.stepId}`} value={outcomeIds[step.stepId] ?? ''} onChange={(event) => { setOutcomeIds((current) => ({ ...current, [step.stepId]: event.target.value })); }}><option value="">Choose an outcome</option>{choices.map((outcome) => <option key={outcome.id} value={outcome.id}>{outcome.label}</option>)}</select>}</><Button size="sm" variant="secondary" data-testid={`crm-live-workflow-complete-${instance.id}-${step.stepId}`} onClick={() => { void onSave(completeWorkflowStep(instance, step.stepId, outcomeIds[step.stepId])); }}>Complete step</Button></>}<Button size="sm" variant="secondary" data-testid={`crm-live-workflow-edit-local-${instance.id}-${step.stepId}`} onClick={() => { setEditingStep(step.stepId); setLocalTitle(liveStepTitle(instance, step.stepId)); }}>Edit for this household</Button>{editingStep === step.stepId && <span><input data-testid={`crm-live-workflow-local-title-${instance.id}-${step.stepId}`} value={localTitle} onChange={(event) => { setLocalTitle(event.target.value); }} /><Button size="sm" data-testid={`crm-live-workflow-local-save-${instance.id}-${step.stepId}`} onClick={() => { void onSave(renameWorkflowStepLocally(instance, step.stepId, localTitle)); setEditingStep(null); }}>Save</Button></span>}<div style={{ width: '100%' }}><textarea data-testid={`crm-live-workflow-step-note-${instance.id}-${step.stepId}`} value={notes[step.stepId] ?? ''} placeholder="Add an internal step comment" onChange={(event) => { setNotes((current) => ({ ...current, [step.stepId]: event.target.value })); }} /><Button size="sm" variant="secondary" data-testid={`crm-live-workflow-save-note-${instance.id}-${step.stepId}`} disabled={!notes[step.stepId]?.trim()} onClick={() => { void onSave(addWorkflowStepNote(instance, step.stepId, notes[step.stepId] ?? '')); setNotes((current) => ({ ...current, [step.stepId]: '' })); }}>Save comment</Button>{step.stepNotes && <p data-testid={`crm-live-workflow-step-notes-${instance.id}-${step.stepId}`} style={mutedStyle}>{step.stepNotes}</p>}</div></div>; })}</section>;
}
...
function LivePropagationReview({ data, onSave }: { data: LiveWorkflowData; onSave: (record: LiveCrmRecord) => Promise<unknown> }) {
  ...
  return <Screen title="Workflow update review" description="Choose how a template update should affect open household work" Icon={GitPullRequest} action={undefined}>
    {offers.length === 0 ? <section style={panelStyle}><strong>No workflow updates waiting for review</strong><p style={mutedStyle}>When you update a workflow template, each household’s open work will appear here for a simple choice.</p></section> : <><section style={panelStyle}><strong>Workflow updates ready to review</strong><p style={mutedStyle}>Choose what should change for each household. Work already done stays exactly as it is.</p></section>{offers.map((offer) => { const instance = instanceFor(offer); const grouped = new Map<string, LiveWorkflowOffer['engineOffer']['decisions']>(); for (const decision of offer.engineOffer.decisions) grouped.set(decision.stepId, [...(grouped.get(decision.stepId) ?? []), decision]); const changeCount = offer.engineOffer.decisions.length; const ready = !offer.engineOffer.requiresConcurrentHeadReview && !offer.engineOffer.decisions.some((decision) => decision.decision === 'review_required'); return <section key={offer.id} data-testid={`crm-live-propagation-offer-${offer.id}`} style={panelStyle}><strong>{offer.householdLabel}: Template updated</strong><p style={mutedStyle}>{changeCount} change{changeCount === 1 ? '' : 's'} to review. Work already done stays as it is.</p>{offer.engineOffer.requiresConcurrentHeadReview && <p role="alert" style={{ ...panelStyle, borderColor: 'var(--kp-direct)' }}>This workflow was changed in two places. Compare each choice before applying.</p>}{[...grouped.entries()].map(([stepId, decisions]) => <div key={stepId} style={{ borderTop: '1px solid var(--kp-border)', paddingTop: 9, marginTop: 9 }}><strong>{instance ? liveStepTitle(instance, stepId) : 'New workflow step'}</strong>{instance?.snapshot.steps[stepId]?.status === 'done' && <p style={mutedStyle}>This step is complete. Its completed work and notes will not change.</p>}{decisions.map((decision) => <div key={decision.id} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '6px 0' }}><span style={{ flex: 1 }}>{plainField(decision, instance ?? { snapshot: { steps: {} } } as LiveWorkflowInstance)}</span><Button size="sm" variant={decision.decision === 'accepted' ? 'primary' : 'secondary'} data-testid={`crm-live-propagation-accept-${decision.id}`} onClick={() => { void change(offer, decision.id, 'accepted'); }}>Apply</Button><Button size="sm" variant={decision.decision === 'rejected' ? 'primary' : 'secondary'} data-testid={`crm-live-propagation-reject-${decision.id}`} onClick={() => { void change(offer, decision.id, 'rejected'); }}>Keep current</Button></div>)}</div>)}<details style={{ marginTop: 10 }}><summary>Details for support</summary><p style={mutedStyle}>Update: {offer.revisionLabel}. Record: {offer.id}. Workflow: {offer.engineOffer.instanceId}.</p></details><div style={{ display: 'flex', gap: 8, marginTop: 12 }}><Button data-testid={`crm-live-propagation-apply-${offer.id}`} disabled={!ready} onClick={() => { void apply(offer); }}>Apply these changes</Button>{instance?.lastApplyEventId && <Button variant="secondary" data-testid={`crm-live-propagation-undo-${instance.id}`} onClick={() => { void undo(instance); }}>Undo last update</Button>}</div></section>; })}</>}
...
function Migration({ route, freshness, migration, onNavigate, actions }: { route: CrmHomeRoute; freshness: CrmFreshnessState; migration: CrmHomeAdapter['migration']; onNavigate: (route: CrmHomeRoute) => void; actions: CrmHomeAdapter['actions'] }) {
  ...
  if (route === 'fidelity') return <FidelityReport onNavigate={onNavigate} {...(migration.noteGaps ? { noteGaps: migration.noteGaps } : {})} {...(migration.report ? { report: migration.report } : {})} />;
  return <Screen title="Wealthbox migration" description="Bring your firm’s records over safely" Icon={Activity} action={<Button data-testid="crm-migration-fidelity" disabled={!migration.report} onClick={() => { onNavigate('fidelity'); }}>Review import report</Button>}><FreshnessBanner freshness={freshness} /><section style={panelStyle}><strong>{migration.report ? 'Import finished' : 'Connect the test source and import'}</strong><p style={mutedStyle}>{migration.report ? migration.report.message : 'This test source only contains made-up Northcrest firm data.'}</p><label style={{ display: 'block', marginBottom: 10 }}>Test source address <input data-testid="crm-migration-base-url" value={baseUrl} onChange={(event) => { setBaseUrl(event.target.value); }} style={{ display: 'block', width: 'min(620px, 100%)' }} /></label><Button data-testid="crm-migration-run-import" disabled={running} onClick={() => { void run(); }}>{running ? 'Importing…' : migration.report ? 'Run import again' : 'Run import'}</Button>{error && <p data-testid="crm-migration-error" role="alert">{error}</p>}</section><section style={panelStyle}><strong>Use both systems while you check the import</strong><p style={mutedStyle}>Lantern brings over only what it can read safely. When it cannot tell where an active workflow is, it asks your firm to decide instead of guessing.</p><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><Button variant="secondary" data-testid="crm-migration-archive" disabled={!migration.report} iconLeft={FileArchive} onClick={() => { onNavigate('archive-export'); }}>Save a backup</Button><Button variant="secondary" data-testid="crm-migration-rollback" disabled={!migration.report} iconLeft={Download} onClick={() => { onNavigate('rollback-export'); }}>Prepare a return file</Button><Button data-testid="crm-migration-start-parallel" disabled={parallel || !migration.report} onClick={() => { setParallel(true); }}>{parallel ? 'Both systems are in use' : 'Start using both systems'}</Button></div></section><section style={panelStyle}><strong>What your firm needs to decide</strong><p style={mutedStyle}>If something cannot be brought over, the report gives you a clear next step. Nothing is hidden behind technical error names.</p><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><Button variant="secondary" data-testid="crm-migration-workflow-fallback" disabled={!migration.report} onClick={() => { onNavigate('workflow-recreation'); }}>Rebuild active workflows</Button><Button variant="secondary" data-testid="crm-migration-attachment-fallback" disabled={!migration.report} onClick={() => { onNavigate('attachment-accounting'); }}>Account for attachments</Button></div></section></Screen>;
}

function FidelityReport({ onNavigate, report, noteGaps = [] }: { onNavigate: (route: CrmHomeRoute) => void; report?: MigrationFidelityReport; noteGaps?: readonly MigrationNoteGap[] }) {
  const [showNotes, setShowNotes] = useState(false);
  if (!report) return <Screen title="Import report" description="No import has run yet" Icon={Flag}><p>Run the import first.</p></Screen>;
  const skippedNotes = report.matrix.find((row) => row.sourceType === 'note')?.skipped ?? 0;
  const workflowNeedsAttention = report.workflows.pending;
  const attachmentNeedsAttention = report.attachments.unaccounted;
  const decisionCount = skippedNotes + workflowNeedsAttention + attachmentNeedsAttention;
  const labelFor = (sourceType: string) => ({ contact: 'Clients', note: 'Notes', task: 'Tasks', event: 'Events', opportunity: 'Opportunities', project: 'Projects', workflow_template: 'Workflow templates', workflow: 'Active workflows', custom_field: 'Custom fields', attachment: 'Attachments' }[sourceType] ?? sourceType.replaceAll('_', ' '));
  return <Screen title="Import report" description={new Date(report.generatedAt).toLocaleString()} Icon={Flag}>
    <section data-testid="crm-migration-decision-dashboard" style={panelStyle}>
      <strong>{decisionCount > 0 ? `Not ready to switch yet: ${String(decisionCount)} item${decisionCount === 1 ? '' : 's'} need your firm’s decision.` : 'Your import has no open migration decisions.'}</strong>
      <p style={mutedStyle}>Nothing is hidden. Resolve each item below before you switch systems.</p>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}>
        <section style={{ ...panelStyle, background: 'var(--color-background)' }}><strong>{String(skippedNotes)} note{skippedNotes === 1 ? '' : 's'} we could not bring over</strong><p style={mutedStyle}>These notes were not safely linked to a client. Check them in Wealthbox, then add any important note to the right household.</p><Button variant="secondary" data-testid="crm-migration-open-note-gaps" onClick={() => { setShowNotes((open) => !open); }}>{showNotes ? 'Close note list' : 'Review these notes'}</Button></section>
        <section style={{ ...panelStyle, background: 'var(--color-background)' }}><strong>{String(workflowNeedsAttention)} active workflow{workflowNeedsAttention === 1 ? '' : 's'} to rebuild</strong><p style={mutedStyle}>Choose the current step for each household, then rebuild its remaining work in Lantern.</p><Button variant="secondary" data-testid="crm-migration-workflow-fallback" onClick={() => { onNavigate('workflow-recreation'); }}>Rebuild these workflows</Button></section>
        <section style={{ ...panelStyle, background: 'var(--color-background)' }}><strong>{String(attachmentNeedsAttention)} attachment{attachmentNeedsAttention === 1 ? '' : 's'} to account for</strong><p style={mutedStyle}>Mark each attachment as exported, or explain what is missing and who owns the follow-up.</p><Button variant="secondary" data-testid="crm-migration-attachment-fallback" onClick={() => { onNavigate('attachment-accounting'); }}>Account for attachments</Button></section>
      </div>
      {showNotes && <section data-testid="crm-migration-note-gap-list" style={{ borderTop: '1px solid var(--kp-border)', marginTop: 12, paddingTop: 12 }}><strong>Notes to check</strong>{noteGaps.length === 0 ? <p style={mutedStyle}>{skippedNotes === 0 ? 'No notes need review.' : 'This import recorded the count but not note titles. Open the saved migration archive to find them.'}</p> : noteGaps.map((note) => <p key={note.id}><strong>{note.label}</strong><span style={mutedStyle}> · {note.reason}</span></p>)}</section>}
    </section>
    <section style={panelStyle}><strong>What came over</strong><p style={mutedStyle}>{report.message}</p><div data-testid="crm-fidelity-matrix">{report.matrix.map((row) => <section key={row.sourceType} data-testid={`crm-fidelity-row-${row.sourceType}`} style={{ borderTop: '1px solid var(--kp-border)', padding: '8px 0' }}><strong>{labelFor(row.sourceType)}</strong><span> · {row.fetched} found · {row.imported} imported · {row.skipped} not imported</span>{row.plainReason ? <p role="alert" style={mutedStyle}>{row.plainReason}</p> : null}</section>)}</div></section>
  </Screen>;
}
...
function WorkflowFallbackChecklist({ records, onRecord }: { records: readonly MigrationWorkflowChecklist[]; onRecord: (record: MigrationWorkflowChecklist) => void | Promise<void> }) {
  ...
  return <Screen title="Rebuild active workflows" description="Finish this before switching systems" Icon={Workflow}><p style={mutedStyle}>We could not safely read where these workflows are today. Review the evidence below, choose the current step, then rebuild the remaining work or explain why it cannot be rebuilt.</p>{drafts.map((record) => <section key={record.id} data-testid={`crm-workflow-checklist-${record.id}`} style={panelStyle}><strong>{record.clientLabel} · {record.sourceTemplateLabel}</strong><p style={mutedStyle}>What we found: {record.activityEvidence.join(' · ') || 'No readable history is available'}</p><label><input data-testid={`crm-workflow-evidence-${record.id}`} type="checkbox" checked={Boolean(record.evidenceReviewed)} onChange={(event) => { update(record.id, { evidenceReviewed: event.target.checked }); }} /> I reviewed what was available</label><label style={{ display: 'block', marginTop: 8 }}>Current step <select data-testid={`crm-workflow-step-${record.id}`} value={record.selectedCurrentStep ?? ''} onChange={(event) => { update(record.id, { selectedCurrentStep: event.target.value }); }}><option value="">Choose the current step</option>{record.availableSteps.map((step) => <option key={step}>{step}</option>)}</select></label><div style={{ display: 'flex', gap: 8, marginTop: 8 }}><Button size="sm" variant={record.decision === 'recreate' ? 'primary' : 'secondary'} onClick={() => { update(record.id, { decision: 'recreate' }); }}>Rebuild this workflow</Button><Button size="sm" variant={record.decision === 'gap' ? 'primary' : 'secondary'} onClick={() => { update(record.id, { decision: 'gap' }); }}>Explain why it cannot be rebuilt</Button></div>{record.decision === 'recreate' ? <label style={{ display: 'block', marginTop: 8 }}>New workflow name <input data-testid={`crm-workflow-instance-${record.id}`} value={record.resultingInstanceLabel ?? ''} onChange={(event) => { update(record.id, { resultingInstanceLabel: event.target.value }); }} /></label> : record.decision === 'gap' ? <label style={{ display: 'block', marginTop: 8 }}>Why it cannot be rebuilt <input data-testid={`crm-workflow-gap-${record.id}`} value={record.gapReason ?? ''} onChange={(event) => { update(record.id, { gapReason: event.target.value }); }} /></label> : null}<Button data-testid={`crm-workflow-record-${record.id}`} style={{ marginTop: 10 }} disabled={!complete(record)} onClick={() => { void recordChecklist(record); }}>Save this decision</Button>{saved.has(record.id) && <p data-testid={`crm-workflow-recorded-${record.id}`} role="status">Decision saved</p>}</section>)}</Screen>;
}
...
function ExportReadiness({ job, onCreate, onRetry }: { job: ExportJobStatus; onCreate: () => void; onRetry: () => void }) { const kind = job.kind; return <Screen title={`${kind === 'archive' ? 'Archive' : 'Rollback'} export`} description="Prepare an export without changing a connector account" Icon={kind === 'archive' ? FileArchive : Download}><section style={panelStyle}><strong>{job.status === 'ready' ? 'Ready to prepare' : job.status === 'preparing' ? 'Preparing export' : job.status === 'exported' ? 'Exported' : 'Failed — retry available'}</strong><ul>{kind === 'archive' ? <><li>Manifest present</li><li>Raw-capture checksums verified</li><li>Fidelity counts matched</li><li>Storage destination selected</li></> : <><li>Full check complete</li><li>Current report saved</li><li>Eligible Lantern changes counted</li><li>Destination format checked</li><li>Known unsupported items listed</li></>}</ul>{job.status === 'exported' ? <><p data-testid="crm-exported-status">Exported {job.exportedAt ?? 'at the recorded export time'} · {kind === 'archive' ? `Manifest ID: ${job.manifestId ?? 'missing from engine data'}` : `Reconciliation report: ${job.reconciliationReportId ?? 'missing from engine data'}`}</p>{job.filePath ? <p data-testid="crm-export-file" style={mutedStyle}>Saved file: {job.filePath}{typeof job.byteLength === 'number' ? ` · ${String(job.byteLength)} bytes` : ''}{job.sha256 ? ` · checksum ${job.sha256}` : ''}</p> : <p role="alert">The export status is saved, but no file location was recorded.</p>}</> : job.status === 'failed' ? <><p role="alert">{job.failureReason ?? 'The export failed. Nothing changed in the connector account.'}</p><Button data-testid="crm-export-retry" onClick={onRetry}>Retry {kind} export</Button></> : job.status === 'preparing' ? <p data-testid="crm-export-preparing" role="status">Preparing the export. This page will update when the CRM data engine records its result.</p> : <Button data-testid="crm-export-create" onClick={onCreate}>Create {kind} export</Button>}</section></Screen>; }
...
export function CrmHome({ adapter, preview = false, initialRoute }: CrmHomeProps) {
  ...
  const firmMembers: readonly CrmFirmMember[] = live.records.filter((record) => record.kind === 'firmDirectoryEntry' && record['active'] === true && typeof record['userId'] === 'string' && typeof record['displayName'] === 'string').map((record) => ({ userId: record['userId'] as string, displayName: record['displayName'] as string, ...(typeof record['title'] === 'string' ? { title: record['title'] } : {}) }));
  ...
  const liveTasks: readonly CrmTask[] = live.records.filter((record) => record.kind === 'task').map((record) => {
    ...
    return {
      ...
      ...(firmMembers.find((member) => member.userId === record['assigneeUserId']) ? { assigneeLabel: firmMembers.find((member) => member.userId === record['assigneeUserId'])?.displayName } : {}),
  ...
  const liveWorkflowWorkItems: readonly CrmWorkflowWorkItem[] = workflowRecords(live.records).instances.flatMap((instance) => Object.values(instance.snapshot.steps)
    .filter((step) => !step.hiddenByTemplateRemoval && step.status !== 'done')
    .map((step) => {
      const assigneeUserId = step.assigneeUserId ?? null;
      const member = firmMembers.find((candidate) => candidate.userId === assigneeUserId);
  ...
  const liveNoteGaps: readonly MigrationNoteGap[] = live.records.filter((record) => record.kind === 'migration_note_gap').map((record) => ({
    id: record.id,
    label: typeof record['label'] === 'string' ? record['label'] : 'Untitled note',
    reason: typeof record['reason'] === 'string' ? record['reason'] : 'This note could not be safely linked to a client.',
  }));
  ...
  const recordActivity = async (summary: string, task?: CrmTask, verb = 'task.updated') => {
    const now = new Date().toISOString();
    await live.save({ id: `activity-${crypto.randomUUID()}`, kind: 'activityEvent', matterId: 'firm_home', at: now, summary, actor: { userId: 'local-user', displayName: 'You' }, verb, targetRef: task ? { kind: 'task', id: task.id, ...(task.householdId ? { matterId: task.householdId } : {}) } : { kind: 'firmDoc', id: 'firm_home' }, ...(task?.householdId ? { householdId: task.householdId } : {}), payload: task ? { taskId: task.id, status: task.status } : {}, important: false });
  };
  const saveTask = async (task: CrmTask) => {
    ...
    const householdRef = householdId ? { kind: 'household', id: householdId, matterId: householdId } : null;
    await live.save({ id: task.id, kind: 'task', matterId: 'firm_home', title: task.title.trim(), body: task.body ?? '', assigneeUserId: task.assigneeUserId, status: task.status, ...(task.dueAt ? { due: task.dueAt } : {}), priority: task.priority, ...(task.recurrence ? { recurrence: task.recurrence } : {}), householdRef, contextRefs: householdRef ? [householdRef] : [], customFields: {} });
    await recordActivity(task.status === 'done' && previous?.status !== 'done' ? `Completed task: ${task.title}` : previous ? `Updated task: ${task.title}` : `Created task: ${task.title}`, { ...task, ...(householdId ? { householdId } : {}) }, task.status === 'done' && previous?.status !== 'done' ? 'task.completed' : previous ? 'task.updated' : 'task.created');
    if (task.status === 'done' && previous?.status !== 'done' && task.recurrence?.regenerateOnComplete) {
      ...
      await live.save({ id: child.id, kind: 'task', matterId: 'firm_home', title: child.title, body: child.body ?? '', assigneeUserId: child.assigneeUserId, status: 'open', ...(dueAt ? { due: dueAt } : {}), priority: child.priority, recurrence: child.recurrence, householdRef, contextRefs: householdRef ? [householdRef] : [], customFields: {} });
  ...
  const completeWorkflowWorkItem = async (item: CrmWorkflowWorkItem) => {
    const instance = workflowRecords(live.records).instances.find((candidate) => candidate.id === item.instanceId);
    if (!instance) throw new Error('That workflow step is no longer available.');
    await live.save(completeWorkflowStep(instance, item.stepId));
    const now = new Date().toISOString();
    await live.save({ id: `activity-${crypto.randomUUID()}`, kind: 'activityEvent', matterId: 'firm_home', at: now, summary: `Completed workflow step: ${item.title}`, actor: { userId: 'local-user', displayName: 'You' }, verb: 'workflow.step.done', targetRef: { kind: 'workflowInstance', id: item.instanceId, matterId: item.householdId }, householdId: item.householdId, payload: { stepId: item.stepId }, important: false });
  };
  const liveAdapter: CrmHomeAdapter = {
    ...
    savedTaskViews,
    migration: { noteGaps: liveNoteGaps, workflowChecklists: liveWorkflowChecklists, attachmentAccounting: liveAttachmentAccounting, exports: liveExports, ...(liveReport ? { report: liveReport } : {}) },
    actions: {
      updateTask: saveTask,
      ...
      decideApproval: async (approval, decision) => {
        ...
        let appliedEntityRef: { kind: 'workflowInstance'; id: string; matterId: string } | undefined;
        if (decision === 'approved' && record['proposalKind'] === 'workflow_launch') {
          const proposed = record['proposedMutation'] as { workflowTemplateId?: unknown } | undefined;
          const templateId = typeof proposed?.workflowTemplateId === 'string' ? proposed.workflowTemplateId : undefined;
          const householdId = householdIdFor(record);
          const template = templateId ? workflowRecords(live.records).templates.find((item) => item.id === templateId) : undefined;
          const household = householdId ? workflowHouseholds.find((item) => item.id === householdId) : undefined;
          if (!template || !household) throw new Error('This proposed workflow no longer has a template and household to start.');
          const instance = startWorkflow(template, household);
          await live.save(instance);
          appliedEntityRef = { kind: 'workflowInstance', id: instance.id, matterId: household.id };
        }
        await live.save({ ...record, state: decision, decidedAt, decidedBy: { userId: 'local-user', displayName: 'You' }, ...(appliedEntityRef ? { appliedEntityRef } : {}) });
  ...
  const workflowHouseholds = useMemo<HouseholdChoice[]>(() => live.records.filter((record) => record.kind === 'household').map((record) => ({ id: record.id, label: typeof record['name'] === 'string' ? record['name'] : typeof record['label'] === 'string' ? record['label'] : 'Untitled household' })), [live.records]);
