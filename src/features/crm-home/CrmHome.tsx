...
function Tasks({ tasks, households, savedViews, freshness, onUpdateTask, onSaveView }: { tasks: readonly CrmTask[]; households: readonly { id: string; name: string }[]; savedViews: readonly CrmTaskSavedView[]; freshness: CrmFreshnessState; onUpdateTask: (task: CrmTask) => void | Promise<void>; onSaveView: (view: CrmTaskSavedView) => void | Promise<void> }) {
  ...
  return <Screen title="Tasks" description="Commitments that fit real time" Icon={ListChecks} action={<Button data-testid="crm-task-new" iconLeft={Plus} onClick={newTask} disabled={households.length === 0}>New task</Button>}>
    ...
    {editing && <TaskDetail task={editing} households={households} onClose={() => { setEditing(null); }} onSave={async (task) => { await onUpdateTask(task); setEditing(null); }} />}
  </Screen>;
}
...
function TaskDetail({ task, households, onClose, onSave }: { task: CrmTask; households: readonly { id: string; name: string }[]; onClose: () => void; onSave: (task: CrmTask) => Promise<void> }) {
...
function Migration({ route, freshness, migration, onNavigate, actions }: { route: CrmHomeRoute; freshness: CrmFreshnessState; migration: CrmHomeAdapter['migration']; onNavigate: (route: CrmHomeRoute) => void; actions: CrmHomeAdapter['actions'] }) {
  ...
  if (route === 'workflow-recreation') return <WorkflowFallbackChecklist records={migration.workflowChecklists} onRecord={async (record) => { await actions.recordWorkflowChecklist?.(record); }} />;
  if (route === 'attachment-accounting') return <AttachmentFallbackChecklist records={migration.attachmentAccounting} onRecord={async (record) => { await actions.recordAttachmentAccounting?.(record); }} />;
...
function WorkflowFallbackChecklist({ records, onRecord }: { records: readonly MigrationWorkflowChecklist[]; onRecord: (record: MigrationWorkflowChecklist) => Promise<void> }) {
