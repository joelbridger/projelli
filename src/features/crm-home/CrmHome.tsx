...
const DEFAULT_TASKS: readonly CrmTask[] = [
...
const DEFAULT_OFFERS: readonly PropagationOffer[] = [
  {
    id: 'offer-henderson', householdLabel: 'Henderson household', revisionLabel: 'Welcome sequence refresh', state: 'ready',
    fields: [
      { id: 'due', label: 'Confirm recurring transfer · due offset', before: '+0 days', after: '+4 days', accepted: true },
      { id: 'role', label: 'Send welcome packet · owner role', before: 'CSA', after: 'Operations', accepted: true },
      { id: 'remove', label: 'Paper welcome kit · removal', after: 'Untouched step will be removed', accepted: true },
    ],
  },
  {
    id: 'offer-miller', householdLabel: 'Miller household', revisionLabel: 'Welcome sequence refresh', state: 'needs-decision', concurrentHeads: true,
    fields: [{ id: 'conflict', label: 'Send welcome packet · due offset', before: '+2 days', after: '+4 days', accepted: true }],
  },
];

const DEFAULT_FRESHNESS: CrmFreshnessState = { kind: 'live', lastSyncedAt: '10:42', lastFullCheckAt: 'yesterday 11:04' };
...
function PropagationReview({ offers, freshness, onApply, onUndo }: { offers: readonly PropagationOffer[]; freshness: CrmFreshnessState; onApply: (offers: readonly PropagationOffer[]) => void; onUndo: () => { restored: number; protectedCells: readonly string[] } }) {
  ...
  const eligible = draft.filter((offer) => offer.state === 'ready' && !offer.concurrentHeads);
  const toggle = (offerId: string, fieldId: string) => setDraft((current) => current.map((offer) => offer.id !== offerId ? offer : { ...offer, fields: offer.fields.map((field) => field.id !== fieldId ? field : { ...field, accepted: !field.accepted }) }));
  const approveAll = () => setDraft((current) => current.map((offer) => offer.state === 'ready' && !offer.concurrentHeads ? { ...offer, fields: offer.fields.map((field) => ({ ...field, accepted: true })) } : offer));
  const apply = () => { onApply(draft); setOutcome(`${draft.flatMap((offer) => offer.fields).filter((field) => field.accepted).length} derived fields applied. Progress was kept.`); };
  return <Screen title="Propagation review" description="One offer per open workflow instance" Icon={GitPullRequest} action={<Button data-testid="crm-propagation-approve-all" disabled={freshness.kind === 'offline' || freshness.kind === 'syncing' || freshness.kind === 'last-synced'} onClick={approveAll}>Approve all eligible</Button>}>
    ...
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><span style={mutedStyle}>All {draft.length}</span><span style={mutedStyle}>Ready: {eligible.length}</span><span style={mutedStyle}>Need a decision: {draft.filter((offer) => offer.concurrentHeads).length}</span></div>
    {draft.map((offer) => <PropagationOfferCard key={offer.id} offer={offer} onToggle={toggle} />)}
    <footer style={{ ...panelStyle, position: 'sticky', bottom: 0, display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }}><span>{draft.flatMap((offer) => offer.fields).filter((field) => field.accepted).length} accepted fields across {draft.length} instances</span><div style={{ display: 'flex', gap: 8 }}><Button variant="secondary" data-testid="crm-propagation-undo" onClick={() => { const result = onUndo(); setUndoReport(result.protectedCells.length ? `${result.restored} untouched derived cells restored. Protected cells kept: ${result.protectedCells.join(', ')}.` : `${result.restored} untouched derived cells restored. No protected cells needed to stay.`); }}>Undo last apply</Button><Button data-testid="crm-propagation-apply" disabled={freshness.kind !== 'live'} onClick={apply}>Apply</Button></div></footer>{outcome && <p data-testid="crm-propagation-result" role="status">{outcome}</p>}{undoReport && <p data-testid="crm-propagation-undo-report" role="status">{undoReport}</p>}
  </Screen>;
}

function PropagationOfferCard({ offer, onToggle }: { offer: PropagationOffer; onToggle: (offerId: string, fieldId: string) => void }) {
  return <section data-testid={`crm-propagation-offer-${offer.id}`} style={panelStyle}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}><strong>{offer.householdLabel} · one offer</strong><span style={mutedStyle}>{offer.revisionLabel}</span></div>{offer.concurrentHeads ? <div role="alert" style={{ marginTop: 8 }}><strong>Two concurrent updates need review.</strong><p style={mutedStyle}>Choose a resolved source value or keep the current derived value. There is no hidden merge editor.</p><Button variant="secondary" size="sm">Review concurrent updates</Button></div> : offer.fields.map((field) => <PropagationFieldRow key={field.id} field={field} onToggle={() => onToggle(offer.id, field.id)} />)}<p style={mutedStyle}>Progress, notes, completions, outcomes, and assignment history are protected.</p></section>;
}

function PropagationFieldRow({ field, onToggle }: { field: PropagationFieldDecision; onToggle: () => void }) {
  return <label style={{ display: 'flex', gap: 8, padding: '8px 0', borderTop: '1px solid var(--kp-border)', alignItems: 'start' }}><input data-testid={`crm-propagation-toggle-${field.id}`} type="checkbox" checked={field.accepted} onChange={onToggle} /><span><strong>{field.label}</strong><br /><span style={mutedStyle}>{field.before ? `${field.before} → ` : ''}{field.after} · {field.accepted ? 'Accept' : 'Reject'}</span></span></label>;
}
...
function Migration({ route, freshness, onNavigate }: { route: CrmHomeRoute; freshness: CrmFreshnessState; onNavigate: (route: CrmHomeRoute) => void }) {
  const [parallel, setParallel] = useState(false);
  const exportKind = route === 'archive-export' ? 'archive' : route === 'rollback-export' ? 'rollback' : null;
  const [exportState, setExportState] = useState<'ready' | 'preparing' | 'exported' | 'failed'>('ready');
  if (exportKind) return <ExportReadiness kind={exportKind} state={exportState} setState={setExportState} />;
  if (route === 'workflow-recreation') return <FallbackChecklist kind="workflow" />;
  if (route === 'attachment-accounting') return <FallbackChecklist kind="attachment" />;
...
function FallbackChecklist({ kind }: { kind: 'workflow' | 'attachment' }) { const [done, setDone] = useState(false); return <Screen title={kind === 'workflow' ? 'In-flight workflow re-creation' : 'Attachment accounting'} description="Required through cutover" Icon={kind === 'workflow' ? Workflow : FileArchive}><section style={panelStyle}>{kind === 'workflow' ? <><strong>Henderson household · Annual review</strong><p style={mutedStyle}>Source template · linked client · activity evidence · operator decision · resulting Lantern instance · unresolved trace gap</p><Button data-testid="crm-recreate-workflow" onClick={() => setDone(true)}>Start Lantern workflow</Button><p>{done ? 'Decision recorded. The chosen current step is ready to start.' : 'The source API does not supply open-workflow state. Choose the current step yourself.'}</p></> : <><strong>Henderson household</strong><p style={mutedStyle}>Every affected client needs exactly one status: Exported with source/operator, or Attachment gap with reason/owner.</p><Button data-testid="crm-attachment-gap" onClick={() => setDone(true)}>Record attachment gap</Button><p>{done ? 'Attachment gap recorded with owner.' : 'No absence is silently treated as no attachment.'}</p></>}</section></Screen>; }

function ExportReadiness({ kind, state, setState }: { kind: 'archive' | 'rollback'; state: 'ready' | 'preparing' | 'exported' | 'failed'; setState: (state: 'ready' | 'preparing' | 'exported' | 'failed') => void }) { return <Screen title={`${kind === 'archive' ? 'Archive' : 'Rollback'} export`} description="Prepare an export without changing a connector account" Icon={kind === 'archive' ? FileArchive : Download}><section style={panelStyle}><strong>{state === 'ready' ? 'Ready to prepare' : state === 'preparing' ? 'Preparing export' : state === 'exported' ? 'Exported' : 'Failed — retry available'}</strong><ul>{kind === 'archive' ? <><li>Manifest present</li><li>Raw-capture checksums verified</li><li>Fidelity counts matched</li><li>Storage destination selected</li></> : <><li>Full check complete</li><li>Current report saved</li><li>Eligible Lantern changes counted</li><li>Destination format checked</li><li>Known unsupported items listed</li></>}</ul>{state === 'exported' ? <p data-testid="crm-exported-status">Exported today · manifest/reconciliation report ready</p> : <Button data-testid="crm-export-create" onClick={() => { setState('preparing'); setTimeout(() => setState('exported'), 0); }}>Create {kind} export</Button>}</section></Screen>; }
...
export function CrmHome({ adapter, initialRoute = 'today' }: CrmHomeProps) {
