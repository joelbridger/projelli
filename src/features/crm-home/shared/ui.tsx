...
export function FreshnessBanner({ freshness }: { freshness: CrmFreshnessState }) {
  ...
  return <div data-testid="crm-freshness-banner" role="status" style={{ ...panelStyle, padding: 'var(--kp-space-sm)', borderColor: color, display: 'flex', gap: 'var(--kp-space-sm)', alignItems: 'center', flexWrap: 'wrap' }}><strong style={{ color }}>{marker}</strong><span style={mutedStyle}>{detail}</span></div>;
}
