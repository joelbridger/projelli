...
function LiveMigrationWizard() {
  ...
  const saveWorkflow = async (item: Checklist) => {
    ...
    if (sourceSteps.length < 2) {
      await live.save({ ...item, decision: 'gap', gapReason: `The source did not provide readable workflow steps, so ${BRAND.name} did not invent a replacement.`, recordedAt: new Date().toISOString() });
    ...
    setStatus(`Checklist recorded. The resulting workflow is now open in ${BRAND.name}.`);
    await live.reload();
  };
