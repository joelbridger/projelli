...
function NotificationInbox({ records, currentUserId, onOpen }: { records: readonly NotificationRecord[]; currentUserId: string; onOpen: (record: NotificationRecord) => void }) {
  ...
  return <section data-testid="crm-notification-inbox" style={panelStyle}>
    ...
    <p style={mutedStyle}>The relay can see the recipient, delivery and acknowledgement timing, ciphertext size band, and an opaque ID. It cannot read the notification’s content.</p>
    {mine.length === 0 ? <EmptyState icon={Bell} title="No notifications yet" body="Assignments, approval requests, workflow due dates, and mentions will appear here." /> : <div>{mine.map((record) => <article key={record.id} data-testid={`crm-notification-${record.id}`} style={{ borderTop: '1px solid var(--kp-border)', padding: '10px 0', opacity: read.has(record.id) ? 0.7 : 1 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}><strong>{notificationText(record)}</strong>{!read.has(record.id) && <span aria-label="Unread" style={{ color: 'var(--kp-assured)' }}>New</span>}</div><p style={{ ...mutedStyle, margin: '5px 0' }}>Recipient: you · sent {dateLabel(record.createdAt)} · ciphertext: {record.ciphertextBand ?? 'size not available'} · opaque ID: {record.opaqueId ?? record.id.slice(0, 12)}</p><Button size="sm" variant="secondary" data-testid={`crm-notification-open-${record.id}`} onClick={() => { const next = new Set(read).add(record.id); setRead(next); writeIds(currentUserId, next); onOpen(record); }}>Open</Button></article>)}</div>}
  </section>;
}
...
function MentionNote({ members, onSave }: { members: readonly { userId: string; displayName: string }[]; onSave: (body: string, mentioned: readonly { userId: string; displayName: string }[]) => Promise<void> }) {
  ...
  const save = async () => { setSaving(true); try { await onSave(body.trim(), members.filter((member) => mentions.includes(member.userId))); setBody(''); setMentions([]); } finally { setSaving(false); } };
  return <section data-testid="crm-activity-note" style={panelStyle}><h2 style={{ marginTop: 0 }}>Internal note</h2><p style={mutedStyle}>Choose people to notify before saving. This is an internal note, not an email.</p><textarea data-testid="crm-activity-note-body" aria-label="Internal note" value={body} onChange={(event) => setBody(event.target.value)} placeholder="Write a note for the firm" style={{ width: '100%', minHeight: 84 }} />
    ...
    {mentions.length > 0 && <p data-testid="crm-activity-mention-review" style={mutedStyle}>Will notify: {members.filter((member) => mentions.includes(member.userId)).map((member) => member.displayName).join(', ')}.</p>}<Button data-testid="crm-activity-note-save" disabled={!body.trim() || saving} iconLeft={MessageSquarePlus} onClick={() => { void save(); }}>{saving ? 'Saving…' : 'Save internal note'}</Button>
  </section>;
}
...
export function CrmActivitySurface() {
  ...
  const notifications = useMemo(() => live.records.flatMap((record): NotificationRecord[] => {
    ...
    return [{ id: record.id, recipientUserId: currentUserId, type: typeof row.payload?.type === 'string' ? row.payload.type : 'firm_notice', subject: typeof row.payload?.subjectRef === 'string' ? row.payload.subjectRef : undefined, createdAt: typeof row.createdAt === 'string' ? row.createdAt : undefined, opaqueId: typeof row.envelopeId === 'string' ? row.envelopeId : record.id, ciphertextBand: typeof row.ciphertextB64 === 'string' ? `${String(Math.round(atob(row.ciphertextB64).length / 1024) || 1)} KiB` : 'size not available', targetRef: typeof row.payload?.pointer?.referenceId === 'string' ? { kind: 'record', id: row.payload.pointer.referenceId } : undefined }];
  ...
  return <div data-testid="crm-activity-surface" style={{ padding: 'var(--kp-space-xl)', overflow: 'auto', width: '100%', display: 'grid', gap: 'var(--kp-space-md)', alignContent: 'start' }}><SurfaceHeader Icon={Clock3} title="Firm activity" description="A dated history of work across your firm" />{live.error && <p role="alert">Could not load firm activity: {live.error}</p>}<SurfaceToolbar><span style={mutedStyle}>Activity is read-only. It has no comments or reactions.</span></SurfaceToolbar><section data-testid="crm-firm-activity-feed" style={panelStyle}><h2 style={{ marginTop: 0 }}>Activity</h2>{activities.length === 0 ? <EmptyState icon={Clock3} title="No firm activity yet" body="Saved tasks, workflow work, notes, and other firm changes will appear here." /> : activities.map((record) => <article key={record.id} data-testid={`crm-firm-activity-${record.id}`} style={{ borderTop: '1px solid var(--kp-border)', padding: '10px 0' }}><strong>{activityLabel(record)}</strong><p style={{ ...mutedStyle, margin: '5px 0' }}>{dateLabel(record.at as string)}</p><Button size="sm" variant="secondary" data-testid={`crm-firm-activity-open-${record.id}`} onClick={() => setOpened(record)}>{sourceLabel(record)}</Button></article>)}</section><NotificationInbox records={notifications} currentUserId={currentUserId} onOpen={setOpened} /><MentionNote members={members} onSave={saveNote} />{opened && <section data-testid="crm-activity-source" style={panelStyle}><strong>Saved record</strong><p style={mutedStyle}>This item points to the record that created the activity.</p><p>Reference: {typeof opened.targetRef === 'object' && opened.targetRef ? `${String((opened.targetRef as { kind?: string }).kind ?? 'record')} / ${String((opened.targetRef as { id?: string }).id ?? opened.id)}` : opened.id}</p><Button size="sm" variant="secondary" onClick={() => setOpened(null)}>Close</Button></section>}</div>;
}
