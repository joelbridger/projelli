...
vi.mock('@/platform/crm/useLiveCrmRecords', () => ({
  useLiveCrmRecords: () => ({ records, error: null, save }),
}));
...
describe('OrgAdmin', () => {
  ...
  it('takes a local, restart-safe picture from the existing firm-admin rails', async () => {
    ...
    // The refresh fans out across several awaited round-trips; wait for the work
    // to actually finish rather than for a fixed number of microtasks.
    await waitFor(() => expect(save).toHaveBeenCalledWith(expect.objectContaining({ kind: 'firmWorkspaceSummary', name: 'Northcrest Advisory' })));
