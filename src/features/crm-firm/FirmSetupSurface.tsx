...
function FirmAccessSetup({ onSave }: { onSave: (record: LiveCrmRecord) => Promise<unknown> }) {
  const [memberName, setMemberName] = useState('New firm member'); const [teamName, setTeamName] = useState('Client service'); const [groupName, setGroupName] = useState('Private clients'); const [schedulingLink, setSchedulingLink] = useState('https://calendly.com/your-firm/intro'); const [saved, setSaved] = useState<string | null>(null);
  const save = async (kind: 'member' | 'team' | 'group' | 'permission' | 'service') => { const at = new Date().toISOString(); if (kind === 'member') await onSave({ id: `firm-member:${crypto.randomUUID()}`, kind: 'firmMember', matterId: 'firm_home', displayName: memberName.trim() || 'New firm member', role: 'member', active: true, updatedAt: at }); if (kind === 'team') await onSave({ id: `team:${crypto.randomUUID()}`, kind: 'team', matterId: 'firm_home', name: teamName.trim() || 'Client service', memberIds: [], updatedAt: at }); if (kind === 'group') await onSave({ id: `group:${crypto.randomUUID()}`, kind: 'group', matterId: 'firm_home', name: groupName.trim() || 'Private clients', visibility: 'restricted', updatedAt: at }); if (kind === 'permission') await onSave({ id: `permission-policy:${crypto.randomUUID()}`, kind: 'permissionPolicy', matterId: 'firm_home', defaultRole: 'member', canViewRestrictedClients: false, updatedAt: at }); if (kind === 'service') await onSave({ id: `service-policy:${crypto.randomUUID()}`, kind: 'servicePolicy', matterId: 'firm_home', name: 'Standard service', schedulingLinkUrl: schedulingLink.trim(), updatedAt: at }); setSaved(kind); };
  return <section data-testid="crm-firm-access-setup" style={panelStyle}><h3 style={{ marginTop: 0 }}>Members and access</h3><p style={mutedStyle}>Set the firm’s starting roles, teams, private groups, and scheduling link here. These choices stay with this workspace.</p><div style={{ display: 'grid', gap: 12 }}><div><label>Member name<input value={memberName} onChange={(event) => { setMemberName(event.target.value); }} /></label><div style={{ display: 'flex', gap: 8, marginTop: 6 }}><Button data-testid="crm-firm-member-invite" variant="secondary" onClick={() => { setMemberName('New firm member'); }}>Invite member</Button><Button data-testid="crm-firm-member-role-save" onClick={() => { void save('member'); }}>Save member role</Button></div></div><div><label>Team name<input value={teamName} onChange={(event) => { setTeamName(event.target.value); }} /></label><div style={{ display: 'flex', gap: 8, marginTop: 6 }}><Button data-testid="crm-firm-team-new" variant="secondary" onClick={() => { setTeamName('Client service'); }}>New team</Button><Button data-testid="crm-firm-team-save" onClick={() => { void save('team'); }}>Save team</Button></div></div><div><label>Private group name<input value={groupName} onChange={(event) => { setGroupName(event.target.value); }} /></label><div style={{ display: 'flex', gap: 8, marginTop: 6 }}><Button data-testid="crm-firm-group-new" variant="secondary" onClick={() => { setGroupName('Private clients'); }}>New private group</Button><Button data-testid="crm-firm-group-save" onClick={() => { void save('group'); }}>Save group</Button></div></div><div><p style={{ ...mutedStyle, marginBottom: 6 }}>New members can view ordinary firm records. Private client access stays off until it is deliberately granted.</p><Button data-testid="crm-firm-permission-save" onClick={() => { void save('permission'); }}>Save default permissions</Button></div><div><label>Scheduling link<input data-testid="crm-service-policy-scheduling-link" value={schedulingLink} onChange={(event) => { setSchedulingLink(event.target.value); }} /></label><Button data-testid="crm-service-policy-save" style={{ marginTop: 6 }} onClick={() => { void save('service'); }}>Save scheduling link</Button></div></div>{saved && <p role="status">Saved {saved === 'service' ? 'the scheduling link' : `${saved} settings`}.</p>}</section>;
}
...
function RecordValues({ records, onSave }: { records: readonly LiveCrmRecord[]; onSave: (record: LiveCrmRecord) => Promise<unknown> }) {
  ...
  return <section data-testid="crm-record-values" style={panelStyle}><h3 style={{ marginTop: 0 }}>Values and tags</h3><p style={mutedStyle}>Choose a saved record, then add only the fields that apply to it.</p>{editableRecords.length === 0 ? <p data-testid="crm-record-values-empty" style={mutedStyle}>There are no saved household, person, account, task, or opportunity records yet.</p> : <><label style={{ display: 'block', marginBottom: 10 }}>Record<select data-testid="crm-record-values-select" value={recordId} onChange={(event) => { setRecordId(event.target.value); }}><option value="">Choose a record</option>{editableRecords.map((record) => <option key={record.id} value={record.id}>{recordLabel(record)} ({record.kind})</option>)}</select></label>{selected && <>{applicable.length === 0 ? <p style={mutedStyle}>No active custom fields apply to this record type.</p> : applicable.map((field) => <label key={field.id} style={{ display: 'block', marginBottom: 8 }}>{field.label}{field.fieldType === 'bool' ? <select data-testid={`crm-record-value-${field.key}`} value={draft[field.key] ?? 'false'} onChange={(event) => { setDraft((current) => ({ ...current, [field.key]: event.target.value })); }}><option value="false">No</option><option value="true">Yes</option></select> : field.fieldType === 'enum' ? <select data-testid={`crm-record-value-${field.key}`} value={draft[field.key] ?? ''} onChange={(event) => { setDraft((current) => ({ ...current, [field.key]: event.target.value })); }}><option value="">Choose one</option>{field.options?.map((option) => <option key={option}>{option}</option>)}</select> : <input data-testid={`crm-record-value-${field.key}`} type={field.fieldType === 'date' ? 'date' : field.fieldType === 'number' || field.fieldType === 'money' ? 'number' : 'text'} value={draft[field.key] ?? ''} placeholder={field.fieldType === 'multi-enum' ? 'Separate choices with commas' : undefined} onChange={(event) => { setDraft((current) => ({ ...current, [field.key]: event.target.value })); }} />}</label>)}<fieldset style={{ border: 0, padding: 0 }}><legend>Tags</legend>{tags.length === 0 ? <p style={mutedStyle}>Create a tag first.</p> : tags.map((tag) => <label key={tag.id} style={{ marginRight: 12 }}><input data-testid={`crm-record-tag-${tag.id}`} type="checkbox" checked={tagIds.includes(tag.id)} onChange={(event) => { setTagIds((current) => event.target.checked ? [...current, tag.id] : current.filter((id) => id !== tag.id)); }} /> {tag.name}</label>)}</fieldset><Button data-testid="crm-record-values-save" style={{ marginTop: 12 }} iconLeft={Save} onClick={() => { void save(); }}>Save values and tags</Button></>}</>}</section>;
}

export function FirmSetupSurface({ initialTab = 'setup', onNavigate }: { initialTab?: Tab; onNavigate?: (route: string) => void }) {
  const live = useLiveCrmRecords();
  const [tab, setTab] = useState<Tab>(initialTab);
  const [showAdmin, setShowAdmin] = useState(false);
  useEffect(() => { setTab(initialTab); }, [initialTab]);
  return <div data-testid="crm-firm-surface" style={{ padding: 'var(--kp-space-xl)', overflow: 'auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 'var(--kp-space-md)' }}><SurfaceHeader Icon={Users} title="Firm" description="Shared firm setup and record labels" actions={<Button data-testid="crm-firm-open-admin" iconLeft={ExternalLink} onClick={() => { setShowAdmin((open) => !open); }}>{showAdmin ? 'Close firm administration' : 'Open firm administration'}</Button>} />{showAdmin && <section data-testid="crm-firm-admin-rails" style={panelStyle}><FirmAdminConsole /></section>}{live.error && <p role="alert">Could not load firm setup: {live.error}</p>}<nav aria-label="Firm sections" style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>{([{ id: 'setup', label: 'Members and access' }, { id: 'fields', label: 'Custom fields' }, { id: 'tags', label: 'Tags' }, { id: 'values', label: 'Values and tags' }] as const).map((item) => <Button key={item.id} data-testid={`crm-firm-tab-${item.id}`} size="sm" variant={tab === item.id ? 'primary' : 'secondary'} onClick={() => { setTab(item.id); }}>{item.label}</Button>)}</nav>{tab === 'setup' && <><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><Button data-testid="crm-firm-route-intake-links" variant="secondary" onClick={() => { onNavigate?.('intake-links'); }}>Intake links</Button><Button data-testid="crm-firm-route-migration" variant="secondary" onClick={() => { onNavigate?.('migration'); }}>Migration</Button></div><FirmAccessSetup onSave={live.save} /><MemberDirectory records={live.records} /><EthicalWallNotice /></>}{tab === 'fields' && <FieldsAdmin records={live.records} onSave={live.save} />}{tab === 'tags' && <TagsAdmin records={live.records} onSave={live.save} />}{tab === 'values' && <RecordValues records={live.records} onSave={live.save} />}</div>;
}
