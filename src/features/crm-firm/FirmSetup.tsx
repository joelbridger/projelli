...
function seedRecordDraft(selected: RecordWithMetadata, applicable: readonly LiveCustomFieldDef[]) {
  const values = customValues(selected);
  return Object.fromEntries(applicable.map((field) => [field.key, inputValue(values[field.key]?.value)]));
}

function RecordValueEditor({ selected, applicable, tags, onSave }: { selected: RecordWithMetadata; applicable: readonly LiveCustomFieldDef[]; tags: readonly LiveTag[]; onSave: (record: LiveCrmRecord) => Promise<unknown> }) {
  const [draft, setDraft] = useState<Record<string, string>>(() => seedRecordDraft(selected, applicable));
  const [tagIds, setTagIds] = useState<string[]>(() => Array.isArray(selected.tagIds) ? selected.tagIds : []);
  const dirtyFieldKeysRef = useRef<Set<string>>(new Set());
  const tagIdsDirtyRef = useRef(false);
  // This editor is keyed by record ID. Refreshes within the same record merge
  // untouched values, while selecting another record destroys this whole state.
  // eslint-disable-next-line react-hooks/set-state-in-effect -- this effect synchronizes the selected record into its edit form.
  useEffect(() => { const seedDraft = seedRecordDraft(selected, applicable); const seedTags = Array.isArray(selected.tagIds) ? selected.tagIds : []; setDraft((current) => { if (dirtyFieldKeysRef.current.size === 0) return seedDraft; const next = { ...seedDraft }; for (const key of dirtyFieldKeysRef.current) { const edited = current[key]; if (edited !== undefined) next[key] = edited; } return next; }); setTagIds((current) => !tagIdsDirtyRef.current ? seedTags : current); }, [selected.updatedAt, applicable.map((field) => field.id).join('|')]);
  const save = async () => { const existing = customValues(selected); const updatedAt = new Date().toISOString(); const nextValues = { ...existing, ...Object.fromEntries(applicable.map((field) => [field.key, { value: parseValue(draft[field.key] ?? '', field), updatedAt, source: source() }])) }; const tagNames = tags.filter((tag) => tagIds.includes(tag.id)).map((tag) => tag.name); await onSave({ ...selected, customFields: nextValues, tagIds, tags: tagNames, updatedAt, updatedBy: actor() }); };
  const updateDraft = (key: string, value: string) => { dirtyFieldKeysRef.current.add(key); setDraft((current) => ({ ...current, [key]: value })); };
  const updateTagIds = (tagId: string, checked: boolean) => { tagIdsDirtyRef.current = true; setTagIds((current) => checked ? [...current, tagId] : current.filter((id) => id !== tagId)); };
  return <>{applicable.length === 0 ? <p style={mutedStyle}>No active custom fields apply to this item.</p> : applicable.map((field) => <label key={field.id} style={{ display: 'block', marginBottom: 8 }}>{field.label}{field.fieldType === 'bool' ? <select data-testid={`crm-record-value-${field.key}`} value={draft[field.key] ?? 'false'} onChange={(event) => { updateDraft(field.key, event.target.value); }}><option value="false">No</option><option value="true">Yes</option></select> : field.fieldType === 'enum' ? <select data-testid={`crm-record-value-${field.key}`} value={draft[field.key] ?? ''} onChange={(event) => { updateDraft(field.key, event.target.value); }}><option value="">Choose one</option>{field.options?.map((option) => <option key={option}>{option}</option>)}</select> : <input data-testid={`crm-record-value-${field.key}`} type={field.fieldType === 'date' ? 'date' : field.fieldType === 'number' || field.fieldType === 'money' ? 'number' : 'text'} value={draft[field.key] ?? ''} placeholder={field.fieldType === 'multi-enum' ? 'Separate choices with commas' : undefined} onChange={(event) => { updateDraft(field.key, event.target.value); }} />}</label>)}<fieldset style={{ border: 0, padding: 0 }}><legend>Tags</legend>{tags.length === 0 ? <p style={mutedStyle}>Create a tag first.</p> : tags.map((tag) => <label key={tag.id} style={{ marginRight: 12 }}><input data-testid={`crm-record-tag-${tag.id}`} type="checkbox" checked={tagIds.includes(tag.id)} onChange={(event) => { updateTagIds(tag.id, event.target.checked); }} /> {tag.name}</label>)}</fieldset><Button data-testid="crm-record-values-save" style={{ marginTop: 12 }} iconLeft={Save} onClick={() => { void save(); }}>Save details and tags</Button></>;
}
