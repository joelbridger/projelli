...
function RecordValues({ records, onSave }: { records: readonly LiveCrmRecord[]; onSave: (record: LiveCrmRecord) => Promise<unknown> }) {
  ...
  // Only selecting another record is a new form. Live refreshes of this record
  // refresh untouched values but cannot replace the advisor's unsaved edits.
  // eslint-disable-next-line react-hooks/set-state-in-effect -- this effect synchronizes the selected record into its edit form.
  useEffect(() => { const contextChanged = seedContextRef.current !== recordId; seedContextRef.current = recordId; if (contextChanged) { dirtyFieldKeysRef.current = new Set(); tagIdsDirtyRef.current = false; } if (!selected) { setDraft({}); setTagIds([]); return; } const values = customValues(selected); const seedDraft = Object.fromEntries(applicable.map((field) => [field.key, inputValue(values[field.key]?.value)])); const seedTags = Array.isArray(selected.tagIds) ? selected.tagIds : []; setDraft((current) => { if (contextChanged || dirtyFieldKeysRef.current.size === 0) return seedDraft; const next = { ...seedDraft }; for (const key of dirtyFieldKeysRef.current) { const edited = current[key]; if (edited !== undefined) next[key] = edited; } return next; }); setTagIds((current) => contextChanged || !tagIdsDirtyRef.current ? seedTags : current); }, [recordId, selected?.updatedAt, applicable.map((field) => field.id).join('|')]);
