...
vi.mock('./trashClient', () => ({
  ...
  restoreTrashedCrmRecord: (...args: unknown[]) =>
    restoreTrashedCrmRecord(...args),
}));
...
describe('TrashRecoverySurface', () => {
  ...
  it('matches the Trash & recovery table: search, type filter, recovery meter, guard, and recover action', async () => {
    ...
    expect(
      screen.getByTestId('crm-trash-meter-household-1')
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.queryByTestId('crm-trash-row-household-1')
