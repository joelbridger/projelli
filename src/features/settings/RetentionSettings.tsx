  ...
  return (
    <section data-testid="retention-settings">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span data-testid="retention-summary" style={{ fontSize: 13, color: 'var(--kp-navy)' }}>
          {modeLabel(policy.mode, policy.audioRetentionDays, t)}
        </span>
        <Button
          size="sm"
          variant="ghost"
          data-testid="retention-change"
          aria-expanded={editing}
          onClick={() => { setEditing((value) => !value); }}
        >
          {t('privacy.retention.change')}
        </Button>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <IconButton
              ...
              size="sm"
              variant="ghost"
              data-testid="retention-more-actions"
            />
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-44">
            <DropdownMenuItem
              data-testid="retention-run-now"
              disabled={running}
              onSelect={() => {
                if (running) return;
                runCleanup();
      ...
      {editing && (
        <div data-testid="retention-editor" style={{ marginTop: 10 }}>
          {MODES.map((mode) => (
