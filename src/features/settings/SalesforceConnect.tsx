...
function scrubSalesforceFromMatters() {
  ...
  for (const matter of getMatters()) {
    ...
    const nonSalesforceKeys = (matter.crmHouseholdKeys ?? []).filter((key) =>
      !key.startsWith(SALESFORCE_KEY_PREFIX),
    );
    if (matter.createdFromCrm && nonSalesforceKeys.length === 0 && (matter.folderPaths ?? []).length === 0) {
...
export function SalesforceConnect() {
  ...
  async function connect() {
    ...
    try {
      ...
      setConnectError(
        typeof err === 'string' ? err : err instanceof Error ? err.message : 'Could not connect to Salesforce. Try again.',
      );
    } finally {
      setConnecting(false);
    }
  ...
  async function runSync() {
    ...
    try {
      ...
      if (households.length === 0) {
        setSyncError('This Salesforce login did not return any FSC household accounts.');
        return;
      }
      ...
      setSyncError(
        typeof err === 'string' ? err : err instanceof Error ? err.message : 'Salesforce sync could not complete. Please try again.',
      );
    } finally {
      setSyncing(false);
    }
  ...
  async function disconnect() {
    ...
    try {
      ...
      if (!remains && result.tokenDeleted) {
        ...
        setLastSyncReport(null);
        setDisconnectNote('Disconnected Salesforce and deleted imported CRM data from this device.');
      } else {
      ...
      setConnectError(
        typeof err === 'string' ? err : err instanceof Error ? err.message : 'Could not disconnect Salesforce. Please try again.',
      );
      crmIsConnected(PROVIDER).then(setConnected).catch(() => {});
    }
  }
    ...
    return (
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Connect Salesforce Financial Services Cloud to bring household and contact data into Client Maps. Requires the Keepance desktop app.
        </p>
  ...
  return (
    <>
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h3 className="text-sm font-semibold text-slate-900">Salesforce FSC</h3>
        <p className="mt-1 text-sm text-slate-600">
          Connect Salesforce Financial Services Cloud to import the households and contacts this login can read. This connector is read-only.
        </p>
