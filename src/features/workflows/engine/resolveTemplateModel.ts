 ...
 *
 *   'lantern-local'   — run on the embedded Lantern Local AI (on-device,
...
export interface ResolveWorkflowProviderInput {
  ...
  /**
   * F-503 — true when the embedded Lantern Local AI model is downloaded and
...
export function resolveWorkflowProvider(
  ...
  //
  // WHICH local engine: the embedded Lantern Local AI is preferred when it is
