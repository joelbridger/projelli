...
function PipelineHome({ data, onNavigate }: { data: PipelineData; onNavigate: (route: PipelineRoute) => void }) {
  ...
  return <main data-testid="crm-pipeline-surface" style={{ flex: 1, overflow: 'auto', padding: 'var(--kp-space-lg)', display: 'grid', alignContent: 'start', gap: 'var(--kp-space-md)' }}>
    ...
    {(creating || editing) && selectedPipeline && <OpportunityEditor opportunity={editing} pipeline={selectedPipeline} stages={pipelineStages} households={households} onCancel={() => { setCreating(false); setEditing(null); }} onSave={async (draft) => { const target = pipelineStages.find((stage) => stage.id === draft['stageId'])!; if (!editing || editing.stageId !== draft['stageId']) { await move({ ...draft, stageId: editing?.stageId ?? '__new__' } as StoredOpportunity, target); } else { await save(saved(editing, draft)); setMessage('Opportunity saved.'); } setCreating(false); setEditing(null); }} />}
  </main>;
}
...
function OpportunityEditor({ opportunity, pipeline, stages, households, onCancel, onSave }: { opportunity: StoredOpportunity | null; pipeline: StoredPipeline; stages: readonly StoredStage[]; households: readonly LiveCrmRecord[]; onCancel: () => void; onSave: (record: LiveCrmRecord) => Promise<void> }) {
...
export function CrmPipelineSurface({ route, onNavigate, data }: { route: PipelineRoute; onNavigate: (route: PipelineRoute) => void; data?: PipelineData }) {
  const live = useLiveCrmRecords();
  const effective = data ?? live;
  return route === 'pipeline-settings' ? <PipelineSettings data={effective} onNavigate={onNavigate} /> : <PipelineHome data={effective} onNavigate={onNavigate} />;
}
